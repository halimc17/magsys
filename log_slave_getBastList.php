...
//==================
  ...
  while($bar=mysql_fetch_object($res))
  {
	...
	//====================ambil sub unit
	  ...
	  $resSub=mysql_query($strSub);
	  while($barSub=mysql_fetch_object($resSub))
	  {
	...
	if($bar->post<1)
	{
		...
		$add="<img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"editXBast('".$bar->notransaksi."','".$bar->untukunit."','".$subunit."','".tanggalnormal($bar->tanggal)."','".$bar->namapenerima."','".$bar->keterangan."');\">";
