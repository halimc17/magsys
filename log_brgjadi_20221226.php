...
<?php
	...
	$sBarang="select a.kodebarang,b.namabarang,a.saldoakhirqty,b.satuan from ".$dbname.".log_5saldobulanan a 
			...
			and (left(a.kodebarang,3) in ('311','312','384','385','386','387') or left(a.kodebarang,1)='4') 
			and b.satuan not in ('Lusin','Pack','Pcs','Rit','Set','Transaksi','Unit') 
	...
	$sBarang="select distinct(a.kodebarang),a.namabarang,a.satuan from ".$dbname.".log_5masterbarang a where (left(a.kodebarang,3) in ('311','312','384','385','386','387') or left(a.kodebarang,1)='4') and satuan not in ('Lusin','Pack','Pcs','Rit','Set','Transaksi','Unit') order by a.namabarang";
	$qBarang= mysql_query($sBarang) or die (mysql_error($conn));
	while($dBarang=mysql_fetch_assoc($qBarang)){
