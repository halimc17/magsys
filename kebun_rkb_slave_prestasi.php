...
switch($proses) {
    case 'add':
	$cols = array(
	    'kodekegiatan','kodeorg','kodesegment','hasilkerja','jumlahhk','upahkerja','umr',
	...
	$query = insertQuery($dbname,'kebun_rkb_prestasi',$data,$cols);
	...
	$query = updateQuery($dbname,'kebun_rkb_prestasi',$data,$where);
	if(!mysql_query($query)) {
	    echo "DB Error : ".mysql_error();
	    exit;
	}
	...
	$query = "delete from `".$dbname."`.`kebun_rkb_prestasi` where ".$where;
	if(!mysql_query($query)) {
	    echo "DB Error : ".mysql_error();
	    exit;
	}
