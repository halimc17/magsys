                          ...
                          if($bar['dibuat']==$_SESSION['standard']['userid'])
                                  {
                                        ...
                                        echo"<img onclick=\"previewDetail('".$bar['nopp']."',event);\" title=\"Detail PP\" class=\"resicon\" src=\"images/zoom.png\">"
                                                . "<img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('log_prapoht','".$bar['nopp']."','','log_slave_print_log_pp',event);\">
                                            </td>
                                 ";
                                  }
                                  else
                    ...
                    if($res2<1)
                    {
                            foreach($_POST['kdbrg'] as $row=>$Act)
                            {
								setIt($_POST['nmbrg'][$row],'');
                                            ...
                                            $rkd_vhc=$_POST['rkd_vhc'][$row];
                                            
                                            $sqp="insert into ".$dbname.".log_prapodt(`nopp`, `kodebarang`, `jumlah`,`kd_anggran`,`tgl_sdt`,`keterangan`,`kodevhc`) values('".$nopp."','".$kdbrg."','".$rjmlhDiminta."','".$rkd_angrn."','".$rtgl_sdt."','".$ketrng."','".$rkd_vhc."')"; //echo $sqp; exit();
//                                            exit("error: ".$sqp);
                                            if(!mysql_query($sqp))
                                                    {
                                                            //echo $sqp; 
