...
switch($proses)
{
	...
	echo $rDt['notiket']."###".$rDt['kodefraksi']."###".$rDt['jmlasli']."###".tanggalnormal($rTgl['tanggal']);
	break;

	case'LoadData':
		$where="";
		if($noTiket!=''){
			$where=" and a.notiket like '%".$noTiket."%' ";
		}
	echo"
	...
	$offset=$page*$limit;
	$ql2="select distinct a.notiket from ".$dbname.".pabrik_sortasi a left join ".$dbname.".pabrik_timbangan b on a.notiket=b.notransaksi where millcode='".$_SESSION['empl']['lokasitugas']."' and kodebarang='40000003' ".$where." order by a.notiket desc";
	$query2=mysql_query($ql2) or die(mysql_error());
	$jsl=mysql_num_rows($query2);
	$jlhbrs= $jsl;


	$sNotiket="select distinct a.notiket from ".$dbname.".pabrik_sortasi a left join ".$dbname.".pabrik_timbangan b on a.notiket=b.notransaksi where millcode='".$_SESSION['empl']['lokasitugas']."' and kodebarang='40000003' ".$where." order by a.notiket desc limit ".$offset.",".$limit." ";
	//echo $ql2;
	...
	while($rNotiket=mysql_fetch_assoc($qNotiket))
	{
			...
			$qFraksi=mysql_query($sFraksi) or die(mysql_error());
			$sJjg="select jjgsortasli,tanggal,persenBrondolan,kgpotsortasli,beratbersih,nokendaraan,jumlahtandan1 from ".$dbname.".pabrik_timbangan 
				   where notransaksi='".$rNotiket['notiket']."'";
			$qJjg=mysql_query($sJjg) or die(mysql_error());
			$rJjg=mysql_fetch_assoc($qJjg);
			echo"<td>".$rJjg['nokendaraan']."</td>";
			echo"<td align='center'>".tanggalnormal(substr($rJjg['tanggal'],0,10))."</td>";
			echo"<td align=right>".number_format($rJjg['beratbersih'],0)."</td>";
			echo"<td align=right>".number_format($rJjg['jumlahtandan1'],0)."</td>";
...
//                    echo"</pre><br />";
	foreach ($kdFraksi as $rt =>$isi)
	{
		...
		if($rCek<1)
		{
				...
				if(mysql_query($sIns))
				{
				   $sCekDt="select jjgsortasli from ".$dbname.".pabrik_timbangan where notransaksi='".$noTiket."'";
				   $qCekDt=mysql_query($sCekDt) or die(mysql_error());
				   $rCekDt=mysql_fetch_assoc($qCekDt);
				   if($rCekDt['jjgsortasli']==0)
				   {
				   $sDt="update ".$dbname.".pabrik_timbangan set jjgsortasli='".$jmlhJJg."',kgpotsortasli='".$kgPtngan."' where notransaksi='".$noTiket."'";
				   if(mysql_query($sDt))
					   echo"";
				   else
...
//                    echo"</pre><br />";
	...
	$sDel="update ".$dbname.".pabrik_sortasi set jmlasli=0 where ".$where."";
	if(mysql_query($sDel))
	{
	   $sUpd="update ".$dbname.".pabrik_timbangan set jjgsortasli=0,kgpotsortasli=0 where notransaksi='".$noTiket."'";
	   if(mysql_query($sUpd))
		   echo"";
	   else
	...
	$ql2="select a.notiket from ".$dbname.".pabrik_sortasi a  left join ".$dbname.".pabrik_timbangan b
		  on a.notiket=b.notransaksi ".$where." and kodebarang='40000003' group by a.notiket order by a.notiket desc";
	$query2=mysql_query($ql2) or die(mysql_error());
	$jsl=mysql_num_rows($query2);
	$jlhbrs= $jsl;
	...
	$sNotiket="select a.notiket from ".$dbname.".pabrik_sortasi a  left join ".$dbname.".pabrik_timbangan b
		  on a.notiket=b.notransaksi ".$where." and kodebarang='40000003' group by a.notiket order by a.notiket desc limit ".$offset.",".$limit." ";
	//echo $sNotiket;exit();
	$qNotiket=mysql_query($sNotiket) or die(mysql_error());
	$a=0;
	while($rNotiket=mysql_fetch_assoc($qNotiket))
	{
			...
			$sJjg="select jjgsortasli,tanggal,persenBrondolan,kgpotsortasli,beratbersih,nokendaraan,jumlahtandan1 from ".$dbname.".pabrik_timbangan where notransaksi='".$rNotiket['notiket']."'";
			$qJjg=mysql_query($sJjg) or die(mysql_error());
			$rJjg=mysql_fetch_assoc($qJjg);
			echo"<td>".$rJjg['nokendaraan']."</td>";
			echo"<td align='center'>".tanggalnormal(substr($rJjg['tanggal'],0,10))."</td>";
...
//                         <tfoot></tfoot></table>";
	...
	$optNotiket="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
	if($_POST['kdOrg']!=''){
	$sTim="select notransaksi,nokendaraan from ".$dbname.".pabrik_timbangan where 
...
//echo "warning:".$sTim;exit();
	...
	$table .= "<thead>";
	$table .= "<tr>";
	$table .= "	<td>".$_SESSION['lang']['noTiket']."</td>
				<td>Netto</td>
   ...
   // $brs=count($rHead);
	...
	$table .= "</thead><tbody>";
	$table.="<tr class=rowcontent><td><select style='width:150px;' id=noTkt name=noTkt onchange=getNetto(this.options[this.selectedIndex].value)>".$optNotiket."</select></td>";          
	$table.="<td id=nettox></td>";
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=jmlJJg  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"4\" value=0  onblur=hitungBJR(this.value,".mysql_num_rows($zd).") disabled></td>";
	$table.="<td id=bjrx></td>";
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=jmlhJJg  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"4\" value=0  onblur=hitungPotongan(this.value,'BRD',".mysql_num_rows($zd).")></td>";
	$a=0;
	$arr="";
	...
	$table .="<td align=right>
		<input type=text class=myinputtextnumber style='width:65px;' id=inputan_".$a." name=frak".$isi2['kode']." onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"5\" value=0 onblur=hitungPotongan(this.value,'".$isi2['kode']."',".mysql_num_rows($zd).")><input type=hidden id=fraksi_".$a." value=".$isi2['kode']." /></td>";
	}
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=kgPtngan  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"8\" value=0 readonly /></td>";
	...
	$tanggal=$thn."-".$bln."-".$hari;   
	$optNotiket="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
	$sTim="select notransaksi,nokendaraan from ".$dbname.".pabrik_timbangan "
	...
	$sJjg="select jjgsortasli,tanggal,persenBrondolan,kgpotsortasli,beratbersih,jumlahtandan1 from ".$dbname.".pabrik_timbangan where notransaksi='".$noTiket."'";
	$qJjg=mysql_query($sJjg) or die(mysql_error());
	$rJjg=mysql_fetch_assoc($qJjg);
					//=========================
	#ambil potongan sortasi
...
//                    }
//                    echo"</tr></thead>
//                         <tbody><tr class=rowcontent><td>Standar Potongan*100(%)</td><td id=nettox>".$rJjg['beratbersih']."</td><td id=bjrx>".number_format(($rJjg['beratbersih']/$rJjg['jjgsortasli']),2,".","")."</td>";
...
//                         <tfoot></tfoot></table>";
	...
	$table .= "</thead><tbody>";
	$table.="<tr class=rowcontent><td><select style='width:150px;' id=noTkt name=noTkt disabled onchange=getNetto(this.options[this.selectedIndex].value)>".$optNotiket."</select></td>";    
	$table.="<td id=nettox>".$rJjg['beratbersih']."</td>";
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=jmlJJg  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"4\" value='".$rJjg['jumlahtandan1']."'  onblur=hitungBJR(this.value,".mysql_num_rows($zd).") disabled></td>";
	@$dtbjr=$rJjg['beratbersih']/$rJjg['jumlahtandan1'];
	$table.="<td id=bjrx>".number_format($dtbjr,2)."</td>";
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=jmlhJJg  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"4\" value='".$rJjg['jjgsortasli']."'  onblur=hitungPotongan(this.value,'BRD',".mysql_num_rows($zd).")></td>";
	...
	{
	   $listData[$dt['kodefraksi']]=$dt['jmlasli'];
	}
	$a=0;
	foreach($rHead as $row2 =>$isi2)
	{
		...
		$table .="<td align=right>
		<input type=text class=myinputtextnumber style='width:65px;' id=inputan_".$a." onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"5\" value=".$listData[$isi2['kode']]." onblur=hitungPotongan(this.value,'".$isi2['kode']."',".mysql_num_rows($zd).")><input type=hidden  id=fraksi_".$a." value=".$isi2['kode']." /></td>";
	}
	$table.="<td><input type=text class=myinputtextnumber style='width:65px;' id=kgPtngan  onkeypress=\"return angka_doang(event)\" size=\"10\" maxlength=\"8\" value='".$rJjg['kgpotsortasli']."' readonly /></td>";
	$table .="<td><img id='detail_add' title='".$_SESSION['lang']['save']."' class=zImgBtn onclick=\"addDetail('".$a."')\" src='images/save.png'/></td>";
	$table.="</tr></tbody></table><input type=hidden id=jmlhBaris value=".$a." />";
	echo $table;
	break;
	...
	echo"<td align='center'>".$_SESSION['lang']['potongankg']."</td><td align='center'>Action</td></tr></thead><tbody>";
	...
	foreach($rData as $brs =>$dt)
	{
	   $listData[$dt['notiket']][$dt['kodefraksi']]=$dt['jmlasli'];
	}
	...
	while($rNotiket=mysql_fetch_assoc($qNotiket))
	{
			...
			echo"<td>".$rNotiket['notiket']."</td>";
			echo"<td>".$rJjg['nokendaraan']."</td>";
			echo"<td align=right>".number_format($rJjg['beratbersih'],2)."</td>";
			echo"<td align=right>".number_format($rJjg['jumlahtandan1'],0)."</td>";
			...
			echo"<td align=right>".number_format($rJjg['kgpotsortasli'],2)."</td><td>
	...
	case'getNetto':
		$str="select beratbersih,jumlahtandan1,kgpotsortasli,jjgsortasli from ".$dbname.".pabrik_timbangan where notransaksi='".$_POST['noticket']."'";
		$res=mysql_query($str);
		$netto=0;
		while($bar=mysql_fetch_object($res))
		{
			...
			$kgpotsortasli=$bar->kgpotsortasli;
			$jjgsortasli=$bar->jjgsortasli;
		}
		echo $netto.'###'.$jmljjg.'###'.$kgpotsortasli.'###'.$jjgsortasli;
	break;
