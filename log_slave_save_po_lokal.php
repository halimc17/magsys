...
switch($proses)
{
	...
	case 'insert':
		...
		$strx="update ".$dbname.".log_poht set `kodesupplier`='".$supplier_id."',`subtotal`='".$sub_total."',`diskonpersen`='".$disc."',`nilaidiskon`='".$nilai_dis."',`pbbkb`='".$pbbkb."',`pph`='".$npph."',`chkppn`='".$chkppn."',`ppn`='".$nppn."',`nilaipo`='".$nilai_po."',`tanggalkirim`='".$tanggl_kirim."',
			  ".$field."='".$lokasi_kirim."',`syaratbayar`='".$cr_pembayaran."',`uraian`='".$ketUraian."',`purchaser`='".$purchaser."',`matauang`='".$mtUang."',`kurs`='".$Kurs."',`persetujuan1`='".$persetujuan."',`hasilpersetujuan1`='1',
			  `tglp1`='".$thisDate."',`statuspo`='2',`persetujuan2`='".$ttd2."',`hasilpersetujuan2`='1',`tglp2`='".$thisDate."',tgledit='".$thisDate."',ongkosangkutan='".$ongkirim."',lokalpusat='1' 
			   where nopo='".$nopo."'";
		if(!mysql_query($strx)) {
	...
	case 'update_data' :
		...
		if($_SESSION['empl']['kodejabatan']=='5') {
			$sql2="select count(*) as jmlhrow from ".$dbname.".log_poht where lokalpusat='1' ".$where." order by tanggal desc ";
			$sql="select * from ".$dbname.".log_poht where lokalpusat='1' ".$where." order by tanggal desc limit ".$offset.",".$limit."";
		} else {
			$sql2="select count(*) as jmlhrow from ".$dbname.".log_poht where lokalpusat='1' and purchaser='".$_SESSION['standard']['userid']."' ".$where." order by tanggal desc ";
			$sql="select * from ".$dbname.".log_poht where lokalpusat='1' and purchaser='".$_SESSION['standard']['userid']."' ".$where." order by tanggal desc limit ".$offset.",".$limit."";
		}
	...
	case 'get_form_approval' :
		$sql="select nopo from ".$dbname.".log_poht where nopo='".$nopo."' and lokalpusat='1'";
		$query=mysql_query($sql) or die(mysql_error());
	...
	case'getKoreksi':
		$sql="select  catatanrelease from ".$dbname.".log_poht where nopo='".$nopo."' and lokalpusat='1'";
		//echo $sql;
		$query=mysql_query($sql) or die(mysql_error());
		$rCek=mysql_num_rows($query);
		if($rCek>0)
