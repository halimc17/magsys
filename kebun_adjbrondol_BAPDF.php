...
if($jenis=='Afkir'){
	$sebab="terdapat Brondolan Afkir";
	$judul="BERITA ACARA BRONDOLAN AFKIR";
	...
	$sebab="terdapat Brondolan Borongan";
	$judul="BERITA ACARA BRONDOLAN BORONGAN";
	$str="select a.*,b.namaorganisasi as namablok from ".$dbname.".kebun_adjbrondol a 
		left join ".$dbname.".organisasi b on b.kodeorganisasi=a.kodeorg
		where a.jenis='Borongan' ".$where." 
		order by a.tanggal,a.waktu,a.kodeorg,a.jenis";
}else{
	$sebab="terjadi pencurian brondolan";
	$judul="BERITA ACARA HILANG BRONDOLAN";
	$str="select a.*,b.namaorganisasi as namablok from ".$dbname.".kebun_adjbrondol a 
		left join ".$dbname.".organisasi b on b.kodeorganisasi=a.kodeorg
...
$pdf=new PDF('P','mm','A4');
    ...
    $pdf->Cell(100,5,'Keterangan',1,1,'C');
	for ($x = 1; $x <= $no; $x++) {
		$pdf->SetX(20);	
		$pdf->SetFont('Arial','',10);
		...
		$pdf->Cell(100,5,$jenis2[$x].' '.$waktu[$x],1,1,'L');
		$pdf->SetX(20);	
		$pdf->Cell(07,5,'',1,0,'C');
		$pdf->Cell(17,5,'',1,0,'C');
		$pdf->Cell(25,5,'',1,0,'L');
		$pdf->Cell(22,5,'',1,0,'R');
		$pdf->Cell(100,5,$keterangan[$x],1,1,'L');
	}
	$pdf->Ln();
	$pdf->SetX(20);
	$pdf->SetFont('Arial','',12);
