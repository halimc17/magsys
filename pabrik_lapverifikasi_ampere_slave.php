<?php
	...
	#ambil data Verifikasi Ampere
	if($stasiun!=''){
		$whr=" and a.kodemesin like '".$stasiun."%'";
	}else{
		$whr=" ";
	}
	$strtb="select kodemesin,a.ukur1,a.ukur2,a.ukur3,a.standard from ".$dbname.".pabrik_verifikasi_ampere a 
	...
	$str="select a.kodeorg,a.kodemesin,a.tanggal,avg(a.standard) as standard,avg(a.ukur1) as ukur1,avg(a.ukur2) as ukur2,avg(a.ukur3) as ukur3
		,b.namaorganisasi as namamesin,c.namaorganisasi as namastasiun,d.ampere from ".$dbname.".pabrik_verifikasi_ampere a 
	...
	$bgclr="align='center'";
	if($proses=='excel'){
		$stream="<h2>Laporan Verifikasi Ampere ".$kdOrg." Periode: ".$periode."</h2>";
		$brd=1;
	...
	for ($x = 1; $x <= 12; $x++){
		...
		//$qwe=date('D', strtotime($isi));
		//if($qwe=='Sun'){ 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."Amp Mtr"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."R"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."S"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."T"."</font></td>"; 
	...
	foreach($kodemesin as $mdid=>$mdval)
    {
		...
		for ($tangval = 1; $tangval <= 12; $tangval++){
			...
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($standard[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur1[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur2[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur3[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukurrata2[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($selisih[$mdid][$bulan],2)."</td>";
		}
	...
	*/
	switch($proses){
        ...
        case 'excel':
            $nop_="Laporan_Verifikasi_Ampere_".$kdOrg."_".$periode."__".date("His");
            if(strlen($stream)>0)
            {
