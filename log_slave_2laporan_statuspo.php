...
$ptId=		empty($_POST['ptId'])? (isset($_GET['ptId'])? $_GET['ptId']: ''): $_POST['ptId'];
...
//get nopo
$sdtPo="select distinct nopo,tanggal,lokalpusat from ".$dbname.".log_po_vw
        where nopo!='' ".$whr." order by nopo asc";
...
if($brsdt!=0)
{
	...
	foreach($data as $dataNopo)
	{
		...
		$sdtPo="select distinct nopo,namabarang,kodebarang,lokalpusat,sum(jumlahpesan) as jumlahpesan,tanggal,satuan from ".$dbname.".log_po_vw
			where nopo='".$dataNopo."'
			group by nopo,namabarang,kodebarang,lokalpusat,tanggal,satuan";
		$qdtPo=mysql_query($sdtPo) or die(mysql_error($conn));
		while($rdtPo=mysql_fetch_assoc($qdtPo))
		{
            $sRow="select distinct nopo,kodebarang from ".$dbname.".log_podt where nopo='".$dataNopo."'";
			...
			if(!isset($lstNotrans[$rdtPo['nopo']][$rdtPo['kodebarang']])) $lstNotrans[$rdtPo['nopo']][$rdtPo['kodebarang']]='';
			if(!isset($lstJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']])) $lstJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']]=0;
			if(!isset($sisaJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']])) $sisaJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']]=0;
