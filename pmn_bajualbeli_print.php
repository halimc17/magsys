<?php
	...
	while($dBrg=mysql_fetch_assoc($qBrg)){
		...
		$baris+=1;
		$pdf->Cell(85,5,$dBrg['namabarang'],'',0,'L');
		$pdf->Cell(17,5,$dBrg['jumlah'],'',0,'R');
		$pdf->Cell(17,5,$dBrg['satuan'].' @ '.$optMtSim[$bar['matauang']],'',0,'L');
		$pdf->Cell(18,5,number_format($dBrg['hargasatuan']),'',1,'R');
	...
	$pdf->SetFont('Arial','','10');
	if($bar['kualitas10']!=''){
		$pdf->Cell(35,5,$bar['kualitas10'],'',0,'L');
		if($bar['kualitas11']!='0'){
			$pdf->Cell(5,5,':','',0,'L');
	...
	if($bar['kualitas20']!=''){
		...
		$pdf->Cell(39,5,'','',0,'L');
		$pdf->Cell(5,5,'','',0,'L');
		$pdf->Cell(35,5,$bar['kualitas20'],'',0,'L');
		if($bar['kualitas21']!='0'){
			$pdf->Cell(5,5,':','',0,'L');
	...
	if($bar['kualitas30']!=''){
		...
		$pdf->Cell(39,5,'','',0,'L');
		$pdf->Cell(5,5,'','',0,'L');
		$pdf->Cell(35,5,$bar['kualitas30'],'',0,'L');
		if($bar['kualitas31']!='0'){
			$pdf->Cell(5,5,':','',0,'L');
	...
	$pdf->Ln(5);
	$pdf->Cell(90,5,$_SESSION['lang']['penjual'].',','',1,'L');
	...
	setIt($nmPtS[1],'');
	$pdf->Cell(90,5,$nmPtS[0].".".ucwords(strtolower($nmPtS[1])),'',1,'L');
	...
	$pdf->Cell(90,5,ucwords(strtolower($bar['penandatangan'])),'',1,'L');
	//$pdf->Cell(90,5,ucwords(strtolower($namaTtdBeli[$bar['koderekanan']])),'',1,'C');

	$pdf->SetFont('Arial','B','10');
	$pdf->Cell(90,5,$jabatanTtd[$bar['penandatangan']],'',1,'L');
