<?php
// Test SDM Menu Items - File Existence Check
// This script tests if SDM menu files exist (no session required)
...
$successCount = 0;
$failCount = 0;
$basePath = 'C:/XAMPP/xampp/htdocs/erpmill/';
...
th { background-color: #1E3A8A; color: white; padding: 8px; text-align: left; position: sticky; top: 0; }
td { border: 1px solid #ddd; padding: 6px; }
tr:nth-child(even) { background-color: #f2f2f2; }
.success { color: green; font-weight: bold; }
.failed { color: red; font-weight: bold; }
.summary { background-color: #f9f9f9; padding: 15px; margin: 20px 0; border-left: 4px solid #1E3A8A; }
.transaksi { background-color: #e3f2fd; }
.laporan { background-color: #fff3e0; }
.proses { background-color: #e8f5e9; }
.setup { background-color: #fce4ec; }
</style></head><body>";

echo "<h1>SDM MODULE TEST REPORT</h1>";
echo "<p><strong>Testing Date:</strong> " . date('Y-m-d H:i:s') . "</p>";
echo "<p>Testing all SDM menu items for file existence...</p>";
...
foreach($menuItems as $index => $item) {
    $num = $index + 1;
    $filePath = $basePath . $item['url'];
    $fileExists = file_exists($filePath);

    // Determine row class based on category
    $rowClass = '';
    if(strpos($item['category'], 'Transaksi') !== false) $rowClass = 'transaksi';
    elseif(strpos($item['category'], 'Laporan') !== false) $rowClass = 'laporan';
    elseif(strpos($item['category'], 'Proses') !== false) $rowClass = 'proses';
    elseif(strpos($item['category'], 'Setup') !== false) $rowClass = 'setup';
...
if($failCount > 0) {
    ...
    foreach($menuItems as $index => $item) {
        ...
        if(!file_exists($filePath)) {
            echo "<li><strong>{$item['category']} > {$item['name']}</strong>: {$item['url']} - File not found at {$filePath}</li>";
        }
    }
    echo "</ul>";
    echo "</div>";
...
// Count by category
$categoryCount = array('Transaksi' => 0, 'Laporan' => 0, 'Proses' => 0, 'Setup' => 0);
foreach($menuItems as $item) {
    if(strpos($item['category'], 'Transaksi') !== false) $categoryCount['Transaksi']++;
    elseif(strpos($item['category'], 'Laporan') !== false) $categoryCount['Laporan']++;
    elseif(strpos($item['category'], 'Proses') !== false) $categoryCount['Proses']++;
    elseif(strpos($item['category'], 'Setup') !== false) $categoryCount['Setup']++;
}

echo "<div class='summary'>";
echo "<h2>Items by Category</h2>";
echo "<ul>";
echo "<li><strong>Transaksi:</strong> {$categoryCount['Transaksi']} items</li>";
echo "<li><strong>Laporan:</strong> {$categoryCount['Laporan']} items</li>";
echo "<li><strong>Proses:</strong> {$categoryCount['Proses']} items</li>";
echo "<li><strong>Setup:</strong> {$categoryCount['Setup']} items</li>";
echo "</ul>";
echo "</div>";
