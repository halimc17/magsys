          ...
          if($thnBudget=='')
          {
           ...
           //$sCek="select sum(jumlahhk) as totalJmlhHk from ".$dbname.".bgt_hkperkegiatan_vw where tahunbudget='".$thnBudget."' and substring(kodeorg,1,4)='".$_SESSION['empl']['lokasitugas']."'";
            ...
            while($res=mysql_fetch_assoc($qCek2))
            {
				...
				$sHk="SELECT sum(jumlah) as jumlahhk FROM ".$dbname.".bgt_budget 
                   where tahunbudget='".$thnBudget."' and 
                   kodeorg = '".$res['kodeorg']."' and kegiatan is not null";
				$qHk = mysql_query($sHk);
				$rHk = mysql_fetch_assoc($qHk);
				
				
                // $nilai=(1/$rTot)*$jmlhPerson*$hkEfektif;
                $nilai=$jmlhPerson*$rHk['jumlahhk'];
                // $nilai2=(1/$rTot)*$totUpah;                
                // $nilai2=($uphSupervisi*$jmlhPerson*$rHk['jumlahhk']);                
                $nilai2=($totUpah/$hkEfektif)*$rHk['jumlahhk'];                
               ...
               // $tab.="<td align=right  id=jmlhHk_".$no.">".$res['jumlahhk']."</td>";
                // $tab.="<td align=right  id=jmlhHk_".$no.">".number_format($nilai)."</td>";
