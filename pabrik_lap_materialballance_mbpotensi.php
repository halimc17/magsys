<?php
	...
	if(strlen($periode)==4){
		...
		$xbln=substr($periode,5,2);
		$awalbln=number_format($xbln);
		$akhirbln=number_format($xbln);
	}
	...
	for ($xbln=$awalbln; $xbln<=$akhirbln; $xbln++){
		$xper=substr($periode,0,4).($xbln<=9 ? '-0'.$xbln : '-'.$xbln);
		//----- OER -----
