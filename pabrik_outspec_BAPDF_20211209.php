...
while($bar=mysql_fetch_object($res)){
	...
	$nokontrakkirim=$bar->nokontrakkirim;
	$nosipbkirim=$bar->nosipbkirim;
...
$pdf=new PDF('P','mm','A4');
    ...
    $pdf->Cell(170,5,'BERITA ACARA OUTSPEC '.strtoupper($komoditi),0,1,'C');
	$pdf->SetX(20);
	...
	$pdf->Cell(3,5," : ",0,0,'L');
	$pdf->Cell(55,5,number_format($beratmasukkirim,0,'.',',').' Kg '.$tanggal.' '.$jammasukkirim,0,1,'R');	
	$pdf->SetX(20);	
	$pdf->Cell(25,5,'Timbang I',0,0,'L');	
	$pdf->Cell(3,5," : ",0,0,'L');
	$pdf->Cell(55,5,number_format($beratkeluarkirim,0,'.',',').' Kg '.$tanggal.' '.$jamkeluarkirim,0,1,'R');	
	$pdf->SetX(20);	
