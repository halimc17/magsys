...
while($rPanen=  mysql_fetch_assoc($qPanen))
{
	...
	$dtBJRPan[$rPanen['kodeorg']]=$rPanen['kg']/$rPanen['jjg'];
}
        ...
        foreach($dtKdOrg as $isi)
        {
            ...
            foreach($arrTgl as $dtTgl=>$ertDt)
            {
                ...
                if($ard==1)
                {
					...
					//if($isi=='DUKE05068A' and $ertDt=='2021-11-10'){
					//	exit('isi='.$dtBJRPan[$isi.$ertDt].' ertDt='.$ertDt);
					//}
					//$kgkg=$dtBJRPan[$isi.$ertDt]*$rest[$isi.$ertDt];
					$kgkg=$dtBJRPan[$isi]*$rest[$isi.$ertDt];
