...
switch($method){
	case'insert':
		...
		if(mysql_query($i)){
			$n="insert into ".$dbname.".pmn_traderht values ('".$nokontrakext."','".$tanggalext."','".$kdUnit."','".$kdCust."','".$kdBrg."','".$jmlExt."','IDR','1','".$hrgExt."','".$nilaiExt."','".$ppnExt."','".$catatan."','".$nokontrakpembanding."','".$uangmuka."','".$franco."','".$ffa."','".$dobi."','".$mdani."','".$moist."','".$dirt."')";
			if(!mysql_query($n)){
	...
	case'loadData':
		...
		$str="select a.*,b.namabarang,c.namacustomer,if(isnull(d.jmlkirim),0,d.jmlkirim) as jmlkirim,if(a.qtykontrak-d.jmlkirim=0,1,0) as stsselisih from ".$dbname.".pmn_traderht a 
				...
				limit ".$offset.",".$limit." ";
		*/
		$str="select a.*,b.namabarang,c.namacustomer,if(isnull(d.jmlkirim),0,d.jmlkirim) as jmlkirim,if(a.qtykontrak-d.jmlkirim=0,1,0) as stsselisih
				,upper(e.penjualan) as penyerahan
				from ".$dbname.".pmn_traderht a 
				left join ".$dbname.".log_5masterbarang b on b.kodebarang=a.kodebarang 
				left join ".$dbname.".pmn_4customer c on c.kodecustomer=a.kodecustomer
				left join (select nokontrakext,sum(beratbersih) as jmlkirim from ".$dbname.".pmn_traderdt group by nokontrakext) d on d.nokontrakext=a.nokontrakext 
				left join ".$dbname.".pmn_5franco e on e.id_franco=a.franco
				where true ".$whrd."
		...
		if($oow==0){
			...
			while($bar=mysql_fetch_assoc($res)){
				...
				echo"	<td align=right>".number_format($bar['uangmuka'],0)."</td>
						<td align=right>".$bar['penyerahan']."</td>";
				echo"	<td>".$bar['nokontrakpembanding']."</td>
						<td align=right>".number_format($bar['jmlkirim'],0)."</td>";
				//echo"	<td>".$bar['catatan']."</td>";
				if($bar['jmlkirim']>0){
					//echo"<td></td>";
					echo"<td align=center>
							<img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".$bar['kodeorg']."','".$bar['kodecustomer']."','".$bar['nokontrakext']."','".tanggalnormal($bar['tanggalext'])."','".$bar['kodebarang']."','".$bar['qtykontrak']."','".$bar['hargaext']."','".$bar['nilaikontrakext']."','".$bar['nilaippnext']."','".$bar['catatan']."','".$bar['nokontrakpembanding']."','".$bar['uangmuka']."','".$bar['franco']."','".$bar['ffa']."','".$bar['dobi']."','".$bar['mdani']."','".$bar['moist']."','".$bar['dirt']."');\">
						</td>";
				}else{
					echo"<td align=center>
							<img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".$bar['kodeorg']."','".$bar['kodecustomer']."','".$bar['nokontrakext']."','".tanggalnormal($bar['tanggalext'])."','".$bar['kodebarang']."','".$bar['qtykontrak']."','".$bar['hargaext']."','".$bar['nilaikontrakext']."','".$bar['nilaippnext']."','".$bar['catatan']."','".$bar['nokontrakpembanding']."','".$bar['uangmuka']."','".$bar['franco']."','".$bar['ffa']."','".$bar['dobi']."','".$bar['mdani']."','".$bar['moist']."','".$bar['dirt']."');\">
...
//		break;
	case'updateData':
		...
		,matauang='IDR',kurs='1',hargaext='$hrgExt',nilaikontrakext='$nilaiExt',nilaippnext='$ppnExt',catatan='$catatan',nokontrakpembanding='$nokontrakpembanding',uangmuka='$uangmuka',franco='$franco',ffa='$ffa',dobi='$dobi',mdani='$mdani',moist='$moist',dirt='$dirt'
