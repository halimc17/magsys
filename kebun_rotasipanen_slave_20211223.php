...
$dtKodeBlok=array();
//Setup Blok
$sBlok="select kodeorg,tahuntanam,luasareaproduktif,jumlahpokok from ".$dbname.".setup_blok 
		where statusblok='TM' and kodeorg like '".$kodeunit."%' order by kodeorg";
//exit("Warning: ".$sBlok);
...
while($rBlok=mysql_fetch_assoc($qBlok)){
	...
	//$sPanen2="select distinct kodeorg,tanggal,datediff('".$tglAwal."', tanggal) as selisih from ".$dbname.".kebun_prestasi_vw 
	//		where kodeorg='".$rBlok['kodeorg']."' and tanggal<'".$tglAwal."'
	//		order by tanggal desc,kodeorg limit 1";
	//$sPanen2="select distinct a.kodeorg,b.tanggal,datediff('".$tglAwal."', b.tanggal) as selisih from ".$dbname.".kebun_prestasi a
	//		left join ".$dbname.".kebun_aktifitas b on b.notransaksi=a.notransaksi
	//		where a.notransaksi like '%/PNN/%' and a.kodeorg='".$rBlok['kodeorg']."' and b.tanggal<'".$tglAwal."'
	//		order by b.tanggal desc,a.kodeorg limit 1";
	//$qPanen2=mysql_query($sPanen2) or die(mysql_error($conn));
	//$rPanen2=mysql_fetch_assoc($qPanen2);
	//$dtDateLalu[$rBlok['kodeorg']]=$rPanen2['tanggal'];
	//$tanggal1 = new DateTime($rPanen2['tanggal']);
	//$tanggal2 = new DateTime($tglAwal);
	//$dtDataAwal[$rBlok['kodeorg']]=$tanggal2->diff($tanggal1)->d;
	//$dtDataAwal[$rBlok['kodeorg']]=$rPanen2['selisih'];
	//$dtDataAwal[$rBlok['kodeorg']]=1;
	//exit("Warning: ".$sPanen2.' '.$rBlok['kodeorg'].'-'.$optNamaBlok[$rBlok['kodeorg']].'='.$dtDataAwal[$rBlok['kodeorg']]);
}
...
$sPanen3="select distinct a.kodeorg,b.tanggal from ".$dbname.".kebun_prestasi a
			left join ".$dbname.".kebun_aktifitas b on b.notransaksi=a.notransaksi
			where a.notransaksi like '%/PNN/%' and a.kodeorg like '".$kodeunit."%' and b.tanggal<'".$tglAwal."' 
			order by b.tanggal ,a.kodeorg";
...
while($rPanen3=mysql_fetch_assoc($qPanen3)){
	...
	$tanggal2 = new DateTime($tglAwal);
	$dtDataAwal[$rPanen3['kodeorg']]=$tanggal2->diff($tanggal1)->d;
}
...
if($proses=='excel'){
	...
	$brd=1;
        ...
        foreach($dtKodeBlok as $blok){
			...
			$nu=$dtDataAwal[$blok]-1;
			$ard=$dtDataAwal[$blok]-1;
