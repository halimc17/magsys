<?php
	...
	if($proses!='excel'){
		...
		for ($tangval = 1; $tangval <= 12; $tangval++){
			$bln=$periode.'-'.sprintf("%02d",$tangval);
			$bulan=sprintf("%02d",$tangval);
			$stream.="<td title='Click untuk cetak detail.' align=center style=\"cursor: pointer\" 	onclick=showpopup('','".$bln."','".$kdOrg."','','excel',event)><img src=images/skyblue/excel.jpg></td>";
			$stream.="<td title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('','".$bln."','".$kdOrg."','','',event)><img src=images/zoom.png></td>";
		}
		$stream.="<td title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('','".$periode."','".$kdOrg."','','excel',event)><img src=images/skyblue/excel.jpg></td>";
		...
		$stream.="<td title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('','".$periode."','".$kdOrg."','','',event)>&nbsp;</td>";
	}
	switch($proses){
        case'preview':
          echo $stream;
