<?php
	...
	while($bar=mysql_fetch_object($res)){
		...
		if($libur == 'LIBUR'){
			$premibersih[$bar->mandorid][$bar->karyawanid][$bar->tanggal]=($bar->upahpremi-($bar->rupiahpenalty))/2*1.5;
		}else{
			//$premibersih[$bar->mandorid][$bar->karyawanid][$bar->tanggal]=$bar->upahpremi-($bar->upahpenalty+$bar->rupiahpenalty);
			$premibersih[$bar->mandorid][$bar->karyawanid][$bar->tanggal]=($bar->upahpremi-($bar->rupiahpenalty))*1.5;
		}
    ...
    $stream.="<table cellspacing='1' border='".$brd."' class='sortable'>
        ...
        foreach($tgltgl as $ar => $isi)
        {
			...
			if($libur){
                ...
                $stream.="<td width=5px rowspan='2' ".$bgclr."><font color=red>".$_SESSION['lang']['premi'].' '.$_SESSION['lang']['mandorpanen']."</font></td>";
			}else{
                ...
                $stream.="<td width=5px rowspan='2' ".$bgclr.">".$_SESSION['lang']['premi'].' '.$_SESSION['lang']['mandorpanen']."</td>";
			}
		}
		$stream.="<td width=5px rowspan='2' ".$bgclr.">".$_SESSION['lang']['upah']."</td>"; 
		$stream.="<td width=5px colspan='3' ".$bgclr.">".$_SESSION['lang']['premi']."</td>"; 
		$stream.="<td width=5px colspan='3' ".$bgclr.">".$_SESSION['lang']['denda']."</td>"; 
		$stream.="<td width=5px rowspan='2' ".$bgclr.">".$_SESSION['lang']['total']."</td>";
		$stream.="<td width=5px rowspan='2' ".$bgclr.">".$_SESSION['lang']['premi'].' '.$_SESSION['lang']['mandorpanen']."</td></tr><tr>";
        foreach($tgltgl as $ar => $isi)
        {
			// Cek Hari Libur
			$qLibur = "select * from ".$dbname.".sdm_5harilibur where tanggal='".$isi."' and keterangan='libur' and kebun in ('GLOBAL','".$kdOrg."')";
        ...
        foreach($karyawannik as $mdid=>$mdval)
        {
			...
			foreach($mdval as $kpid=>$kpval)
			{
				...
				foreach($tgltgl as $key=>$tangval)
				{
                    ...
                    $stream.="<td title='Click untuk melihat notransaksi.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$kpid."','".$tangval."',event)>".@number_format($premibersih[$mdid][$kpid][$tangval]/$jmlpemanen[$mdid][$tangval],2)."</td>";
