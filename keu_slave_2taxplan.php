...
$pt=	isset($_POST['pt'])? $_POST['pt']:	(isset($_GET['pt'])? $_GET['pt']: '');
$unit=	!empty($_POST['unit'])? $_POST['unit']: (isset($_GET['unit'])? $_GET['unit']: '');

$tanggaldr=		isset($_POST['tanggaldari'])? tanggalsystem($_POST['tanggaldari']): '';
$tanggaldari=	substr($tanggaldr,0,4).'-'.substr($tanggaldr,4,2).'-'.substr($tanggaldr,6,2);
$tanggalsd=		isset($_POST['tanggalsampai'])? tanggalsystem($_POST['tanggalsampai']): '';
...
if(!empty($isidata)){
	...
	{        
		if(isset($_GET['type']) and $_GET['type']!='excel')$tampiltanggal=tanggalnormal($baris['tang']); else $tampiltanggal=$baris['tang'];
		$no+=1; $total=0;
		$stream.="<tr class=rowcontent>
			...
			<td align=right>".number_format($baris['harg'],2)."</td>
			<td align=right>".number_format($baris['tota'],2)."</td>
			</tr>";
	}
}
else{
...
if(isset($_GET['type']) and $_GET['type']=='unit'){
    ...
    $s_unit="select * from ".$dbname.".organisasi where induk='".$pt."'  and kodeorganisasi='".$_SESSION['empl']['lokasitugas']."' order by kodeorganisasi asc";
...
elseif(isset($_GET['type']) and $_GET['type']=='excel')
{
