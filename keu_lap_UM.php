<?php
	...
	//Pilih No Akun
	$optNoakun="<option value=''>".$_SESSION['lang']['all']."</option>";
	$i="select noakun,namaakun from ".$dbname.".keu_5akun where noakun in ('1180300','1180400') order by noakun";
	$n=mysql_query($i) or die (mysql_error($conn));
	while($d=mysql_fetch_assoc($n)){
		$optNoakun.="<option value='".$d['noakun']."'>".$d['noakun'].' - '.$d['namaakun']."</option>";
	}
	...
	$arr="##kodept##kodeunit##noakun##tanggal1##tanggal2##tipereal";
?>
...
<div>
	<fieldset style="float: left;">
		<legend><b><?php echo 'Laporan Uang Muka';?></b></legend>
		<table cellspacing="1" border="0" >
			...
			<tr>
				...
				<td><select id="noakun" name="noakun"><?php echo $optNoakun;?></select></td>
			</tr>
			<tr>
			...
			<tr><td colspan="2">
					<button onclick="zPreview('keu_lap_UM_slave','<?php echo $arr;?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button>
					<button onclick="zExcel(event,'keu_lap_UM_slave.php','<?php echo $arr;?>')" class="mybutton" name="preview" id="preview">Excel</button>
