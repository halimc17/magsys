...
if(isTransactionPeriod())//check if transaction period is normal
{
	...
	$str="select count(*) as jlhbrs from ".$dbname.".log_transaksiht where tipetransaksi=7 and gudangx='".$gudang."' and tanggal like '".$tanggalDt[2]."%' 
	     ".$add."  order by jlhbrs desc ";
	//echo $str;
	$res=mysql_query($str);
	...
	$str="select * from ".$dbname.".log_transaksiht where tipetransaksi=7 and gudangx='".$gudang."' and tanggal like '".$tanggalDt[2]."%' 
	     ".$add."  order  by notransaksi desc limit ".$offset.",20";
	 $res=mysql_query($str);  
	...
	//status apakah sudah diterima
		...
		if(($bar->notransaksireferensi!='')||!is_null($bar->notransaksireferensi!=''))
		{
			...
			$barCek=mysql_fetch_object($resCek);
			...
			$barCek=mysql_fetch_object($resCekBpb);
		...
		$qSj=mysql_query($sSj) or die(mysql_error($conn));
		$rSj=mysql_fetch_object($qSj);
