...
// cari kegiatan
$kegiatan="SELECT kodekegiatan, namakegiatan,namakegiatan1,satuan FROM ".$dbname.".setup_kegiatan WHERE `kelompok` = 'TBM' order by kodekegiatan";
...
##setahun
$ifPtaThn="select sum(volume) as volume,kodekegiatan from ".$dbname.".pta_dt where "
        . " tanggal like '%".$tahun."%' and alokasibiaya like '%".$sortUnit."%' group by kodekegiatan";
$nfPtaThn=  mysql_query($ifPtaThn) or die (mysql_error($conn));
while($dfPtaThn=  mysql_fetch_assoc($nfPtaThn))
{
    $dzArr[$dfPtaThn['kodekegiatan']][fptathn]=$dfPtaThn['volume'];
}

##bulan ini
$ifPtaBln="select sum(volume) as volume,kodekegiatan from ".$dbname.".pta_dt where "
        . " tanggal like '%".$periode."%' and alokasibiaya like '%".$sortUnit."%' group by kodekegiatan";
$nfPtaBln=  mysql_query($ifPtaBln) or die (mysql_error($conn));
while($dfPtaBln=  mysql_fetch_assoc($nfPtaBln))
{
    $dzArr[$dfPtaBln['kodekegiatan']][fptabln]=$dfPtaBln['volume'];
}

##bulan ini
$ifPtaSdBln="select sum(volume) as volume,kodekegiatan from ".$dbname.".pta_dt where "
        . " tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15') and "
        . " alokasibiaya like '%".$sortUnit."%' group by kodekegiatan ";
$nfPtaSdBln=  mysql_query($ifPtaSdBln) or die (mysql_error($conn));
while($dfPtaSdBln=  mysql_fetch_assoc($nfPtaSdBln))
{
    $dzArr[$dfPtaSdBln['kodekegiatan']][fptasdbln]=$dfPtaSdBln['volume'];
}
...
$tab.="<table border=0>
     <tr>
        <td colspan=15 align=left><font size=3>11. ".$_SESSION['lang']['pemeltanaman']." ".$_SESSION['lang']['tbm']."(HA-HK)</font></td>
...
if($proses!='excel')$tab.=$judul;
    ...
    <td align=center colspan=10 ".$bg.">".$_SESSION['lang']['fisik']." (HA-UNIT)</td>
...
// excel array content =========================================================================
    ...
    if(!empty($listkegiatan))foreach($listkegiatan as $keg){
        ...
        $tab.= "<td align=right>".numberformat($dzArr[$keg][fptathn],0)."</td>";
        $tab.= "<td align=right>".numberformat($dzArr[$keg][fptabln],0)."</td>";
        $tab.= "<td align=right>".numberformat($dzArr[$keg][fptasdbln],0)."</td>";
