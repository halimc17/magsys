	...
	case'preview':
		...
		echo" <table class=sortable cellspacing=1 border=0 style='width:1200px;'>
			...
			<tr class=rowheader>
				...
				<td align=center colspan=5>".$_SESSION['lang']['klaim'].' '.$_SESSION['lang']['kualitas'].' (Rp)'."</td>
			...
			$sql="select a.nokontrak,a.tanggalkontrak,a.koderekanan,b.namacustomer,a.kodebarang,c.namabarang,a.kuantitaskontrak,a.kdtermin,d.qty,d.nodo
						...
						,e.rupiah4*if(e.nilaippn=0,1,1.1) as rupiah4,e.rupiah6*if(e.nilaippn=0,1,1.1) as rupiah6
						,(e.rupiah1+e.rupiah2+e.rupiah3+e.rupiah4+e.rupiah6)*if(e.nilaippn=0,1,1.1) as totalklaim
						,((a.kuantitaskontrak*a.hargasatuan)-((e.rupiah1+e.rupiah2+e.rupiah3+e.rupiah4+e.rupiah6)*if(e.nilaippn=0,1,1.1))) as total
			...
			if($row>0)
			{
				while($res=mysql_fetch_assoc($query))
				{
					...
					echo"<tr class=rowcontent>
							...
							<td style='text-align:right;'>".number_format($res['totalkontrak'])."</td>
							...
							<td style='text-align:right;'>".number_format($res['totalklaim'],2)."</td>
							...
							<td style='text-align:right;'>".number_format($jumlahinvoice)."</td>
					...
					if($res['hasilkerjarealisasi']>0){
						echo number_format($res['jumlahrealisasippn']/$res['hasilkerjarealisasi']);
					}else{
				...
				echo"<tr class=rowcontent style='font-weight:bold;'>
					...
					if($gtotalJmlTransport>0){
						echo number_format($gtotalTransport/$gtotalJmlTransport);
					}else{
						echo number_format(0);
					}
				echo "</td>
