...
if($_SESSION['empl']['tipelokasitugas']=='HOLDING'){
	...
	$sThnTanam="select distinct tahuntanam from ".$dbname.".setup_blok where kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['induk']."') order by tahuntanam";
}else{
	$optPT="<option value='".$_SESSION['org']['kodeorganisasi']."'>".$_SESSION['org']['namaorganisasi']."</option>";
	...
	$sThnTanam="select distinct tahuntanam from ".$dbname.".setup_blok where kodeorg like '".$_SESSION['empl']['lokasitugas']."%' order by tahuntanam";
}
...
$sKegiatan="select kodekegiatan,namakegiatan from ".$dbname.".setup_kegiatan where status='1' and (kodekegiatan like '126%' or (kodekegiatan between 128000000 and 128129999) or kodekegiatan like '61%' or kodekegiatan like '62%' or kodekegiatan like '7%') order by kodekegiatan";
...
$optStatus.="<option value='PNN'>PNN</option>";
$sStatus="select kode from ".$dbname.".kebun_5sttanam where kode like '%T%' order by kode desc";
$qStatus=mysql_query($sStatus) or die(mysql_error($conn));
while($rStatus=mysql_fetch_assoc($qStatus)){
	$optStatus.="<option value=".$rStatus['kode'].">".$rStatus['kode']."</option>";
}
...
echo "<fieldset style='width:750px;'>
				...
				<td valign=top>
					<table>
						...
						<tr>
							...
							<td><select id='kdDivisi' name='kdDivisi' style='width:235px' onchange='loadData(0)'>".$optDivisi."</select></td>
						</tr>
						<tr>
				            <td><label>".$_SESSION['lang']['tahuntanam']."</label></td>
							<td><select id='ThnTanam' name='ThnTanam' style='width:80px' onchange='loadData(0)'>".$optThnTanam."</select></td>
						</tr>
						<tr>
				            <td><label>".$_SESSION['lang']['tahun'].' '.$_SESSION['lang']['budget']."</label></td>
							<td><select id='Tahun' name='Tahun' style='width:55px' onchange='loadData(0)'>".$optTahun."</select></td>
				...
				<td valign=top>
					<table>
						<tr>
							...
							<td><select id='kdStatus' name='kdStatus' style='width:300px' onchange='getKegiatan()'>".$optStatus."</select></td>
						</tr>
						<tr>
		...
		<button class=mybutton onclick=salinData()>".$_SESSION['lang']['copy']."</button>
