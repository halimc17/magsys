<?php
	...
	//============= Data Pelatihan
	$str="	select a.*,b.nik,b.namakaryawan,d.namaorganisasi,e.namajabatan from ".$dbname.".sdm_karyawantraining a 
	...
	$sstr="	select a.*,b.nik,b.namakaryawan,d.namaorganisasi,e.namajabatan from ".$dbname.".sdm_karyawantraining a 
			left join ".$dbname.".datakaryawan b on b.karyawanid=a.karyawanid
			left join ".$dbname.".organisasi c on c.kodeorganisasi=if(b.subbagian='',b.lokasitugas,left(b.subbagian,4))
			left join ".$dbname.".organisasi d on d.kodeorganisasi=c.induk
			left join ".$dbname.".sdm_5jabatan e on e.kodejabatan=b.kodejabatan
	...
	while($dbar=mysql_fetch_object($qres)){
		$no+=1;
		if($dbar->jenistraining=='SOFT'){
			$dkategori='Soft Skill';
		}elseif($dbar->jenistraining=='TECH'){
			$dkategori='Technical Skill';
		}else{
			$dkategori=$dbar->jenistraining;
		}
		$pdf->SetX(10);
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(6,5,$no,1,0,'C');
		$pdf->Cell(21,5,$dkategori,1,0,'L');
