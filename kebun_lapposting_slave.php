<?php
	...
	if($kdPT!='' and $kdOrg==''){
		if($_SESSION['empl']['tipelokasitugas']=='HOLDING'){
			...
			$tipelok=" and kodeorganisasi not like '%HO' ";
		}else{
			$tipelok=" and kodeorganisasi='".$_SESSION['empl']['lokasitugas']."' ";
		}
		$whr.=" and a.kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where detail='1' ".$tipelok." and induk='".$kdPT."') ";
		$whr1.=" and left(a.kodegudang,4) in (select kodeorganisasi from ".$dbname.".organisasi where detail='1' ".$tipelok." and induk='".$kdPT."') ";
		$whr2.=" and left(a.kodeblok,4) in (select kodeorganisasi from ".$dbname.".organisasi where detail='1' ".$tipelok." and induk='".$kdPT."') ";
		$whr3.=" and a.kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where detail='1' ".$tipelok." and induk='".$kdPT."') ";
	}
	if($kdOrg!=''){
	...
	if($jenistransaksi=='' or $jenistransaksi=='kasbank'){
		$str="select 'KASBANK' as jenistr,a.notransaksi,a.tanggal,a.kodeorg,b.namaorganisasi,if(substr(notransaksi,15,2)='BK','Bank Keluar',if(substr(notransaksi,15,2)='BM','Bank Masuk',if(substr(notransaksi,15,2)='KK','Kas Keluar',if(substr(notransaksi,15,2)='KM','Kas Masuk','Lain-lain')))) as tipe_tr,if(a.posting='1',a.lastupdate,'') as tglpost
	...
	if($jenistransaksi=='' or $jenistransaksi=='baspk'){
		...
		$str="select 'BASPK' as jenistr,a.notransaksi,a.tanggal,left(a.kodeblok,4) as kodeorg,b.namaorganisasi,a.kodekegiatan as tipe_tr
				,if(a.statusjurnal=1,a.tanggal,'') as tglpost
	...
	if($jenistransaksi=='' or $jenistransaksi=='bkm'){
		$str="select if(tipetransaksi='TB','BKM Pembukaan Lahan',if(tipetransaksi='BBT','BKM Pembibitan',if(tipetransaksi='TBM','BKM Pemelharaan TBM',if(tipetransaksi='TM','BKM Pemelharaan TM',if(tipetransaksi='PNN','BKM Panen','BKM Lain-lain'))))) as jenistr,a.notransaksi,a.tanggal,a.kodeorg,b.namaorganisasi,if(isnull(left(e.kodeorg,6)),d.namakaryawan,left(e.kodeorg,6)) as tipe_tr,if(a.jurnal='1',a.lastupdate,'') as tglpost
				from ".$dbname.".kebun_aktifitas a
				LEFT JOIN ".$dbname.".organisasi b on b.kodeorganisasi=a.kodeorg
				LEFT JOIN ".$dbname.".datakaryawan d on d.karyawanid=a.updateby
				LEFT JOIN ".$dbname.".kebun_prestasi e on e.notransaksi=a.notransaksi
				where true ".$whr3."
				ORDER BY a.kodeorg,tipe_tr,a.notransaksi";
		//exit('Warning : '.$str);
		$res=mysql_query($str);
		while($bar=mysql_fetch_object($res)){
			$jenistr[$bar->notransaksi]=$bar->jenistr;
	...
	if($jenistransaksi=='' or $jenistransaksi=='spatbs'){
		$str="select 'SPATBS' as jenistr,a.nospb as notransaksi,a.tanggal,a.kodeorg,b.namaorganisasi,if(isnull(left(e.blok,6)),d.namakaryawan,left(e.blok,6)) as tipe_tr
				,if(a.posting='1',a.updatetime,'') as tglpost
				from ".$dbname.".kebun_spbht a
				LEFT JOIN ".$dbname.".organisasi b on b.kodeorganisasi=a.kodeorg
				LEFT JOIN ".$dbname.".datakaryawan d on d.karyawanid=a.updateby
				LEFT JOIN ".$dbname.".kebun_spbdt e on e.nospb=a.nospb
				where true ".$whr."
	...
	if($jenistransaksi=='' or $jenistransaksi=='traksi'){
		$str="select 'TRAKSI' as jenistr,a.notransaksi,a.tanggal,a.kodeorg,b.namaorganisasi,a.kodevhc as tipe_tr,if(a.posting='1',a.updatetime,'') as tglpost
    ...
    $stream.="<table cellspacing='1' border='".$brd."' class='sortable'>
        ...
        <td ".$bgclr.">".$_SESSION['lang']['posting']."</td>
	...
	foreach($notrans as $notransid=>$notransval){
		...
		<td align='center'>".$tglpost[$notransid]."</td>
	...
	switch($proses){
		...
		case 'excel':
            $nop_="Laporan_Transaksi_Belum_Posting_".date("His");
            if(strlen($stream)>0){
