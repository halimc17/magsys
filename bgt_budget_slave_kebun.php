...
//        $sKeg="select distinct kodekegiatan,namakegiatan,kelompok from ".$dbname.".setup_kegiatan where  kelompok in ('PNN','TBM','TM','BBT','TB')  order by kodekegiatan asc";
    ...
    case'saveKendaran':
		...
		$sAlokasi="select sum(jumlah) as jumlah from ".$dbname.".bgt_budget where tahunbudget='".$thnBudget."' and kodevhc='".$kdVhc.
			"' and tipebudget<>'TRK' and left(kodeorg,4)='".$kebun."' group by left(kodeorg,4)";
        $qAlokasi=mysql_query($sAlokasi) or die(mysql_error($conn));
        $rAlokasi=mysql_fetch_assoc($qAlokasi);
		 
		setIt($rAlokasi['jumlah'],0);
          $sisa=$rCekJam['jumlahjam']-$rAlokasi['jumlah'];
          if($jmlhJam>$sisa)
          {
              exit("Error: Vehicle ".$kdVhc." has been allocated: ".$rAlokasi['jumlah']." from total hours :".$rCekJam['jumlahjam']." can only allocate as remains:".$sisa."");
          }
	...
	break;
      ...
      case'loadDataTool':
        ...
        {
        $optPage.="<option value='".$starAwal."' ".(isset($_POST['page']) and $starAwal==$_POST['page']?'selected':'').">".$starAwal."</option>";
        }
        if($rAngka!=0)
        {
        $limit=50;
        ...
        while($res=mysql_fetch_assoc($qLoad))
        {
            ...
            $tab.="<td align='center' ".$dtClik.">".(isset($optNmBrg[$res['kodebarang']])? $optNmBrg[$res['kodebarang']]: '')."</td>";
