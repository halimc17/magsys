...
//                oFCKeditor.BasePath = "http://localhost/fckeditor/";
    ...
    <tr>
        ...
        <td><input type='text' class='myinputtext' id='html' onkeypress="return tanpa_kutip();"  size='10' style="width:200px;" value="help/en/" /></td>
    </tr>
    <tr>
    <td colspan="3" id="tmblHeader">
        <button class=mybutton id=saveForm onclick=saveForm_en()><?php echo $_SESSION['lang']['save']?></button>
        <button class=mybutton id=cancelForm onclick=cancelForm_en()><?php echo $_SESSION['lang']['cancel']?></button>
    </td>
    </tr>
...
<fieldset style='float:left;'>
     ...
     <table cellspacing="1" border="0">
         <tr>
            ...
            <td><input type='text' class='myinputtext' id='cariindex' onkeypress="return tanpa_kutip();"  size='10' maxlength='35' style="width:150px;" />
                <button class=mybutton id='cari' onclick=cariHelp_en()><?php echo $_SESSION['lang']['find']?></button></td>
         </tr>
     </table>
    <table cellspacing="1" border="0" class="sortable">
        ...
        <?php
            ...
            {
                $no+=1;
                echo"<script>loadNData_en()</script>";
                echo"<td><img src=images/edit.png class=resicon  title='Edit' onclick=\"editRow_en('".$row['kode']."','".$row['tentang']."','".$row['modul']."','".str_replace(array("\r", "\n"), '\n', $row['isi'])."','".$row['tujuan']."');\" ></td>";
                echo"<td><img onclick=\"detailHelp_en(event,'".str_replace(" ","",$row['kode'])."','".$row['modul']."');\" title=\"Detail Help\" class=\"resicon\" src=\"images/zoom.png\"></td>";
                echo"<td><img src=images/delete1.jpg class=resicon  title='Delete' onclick=\"delData_en('".$row['kode']."','".$row['tentang']."','".$row['modul']."','".str_replace(array("\r", "\n"), '\n', $row['isi'])."')></td></tr>";
            }
