	...
	$data = $_POST;
        ...
        { 
            if($menitmulai>$menitselesai)
            {
                ...
                $jamisi=($jamselesai+23)-$jammulai;
                //$menitisi=  number_format((($menitselesai+60)-$menitmulai)/60,2);
                $menitisi=  number_format(($menitmulai-$menitselesai)/60,2);
            }
            else
            {
                //exit("Error:MASUK");
        ...
        //echo $jammulai._.$menitmulai.______.$jamselesai._.$menitselesai;
        
        $selisih=$jamisi+$menitisi;
        
        //echo $selisih._.$data['jamdinasbruto']._.$data['jamstagnasi'];
        //echo $jamisi._.$menitisi;
		$jamx=$data['jamdinasbruto']+$data['jamstagnasi'];
		$selisiherr=abs($jamx-$selisih);
        if(($data['jamdinasbruto']+$data['jamstagnasi'])!=$selisih)
        {
            exit("Warning: Jam proses masih salah,  masih terdapat selisih,  selisih : ".$selisiherr."	");
        }
...
//    }
    ...
    */
    ...
    $whereKaryMandor=" lokasitugas='".$_SESSION['empl']['lokasitugas']."' and tanggalkeluar='0000-00-00' and "
            . " kodejabatan in (select kodejabatan from ".$dbname.".sdm_5jabatan "
            . " where namajabatan like '%Group Leader%' or namajabatan like '%Senior%') ";
    ...
    $whereKaryAst=" lokasitugas='".$_SESSION['empl']['lokasitugas']."' and tanggalkeluar='0000-00-00' and "
            . " kodejabatan in (select kodejabatan from ".$dbname.".sdm_5jabatan "
            . " where namajabatan like '%Head%' or namajabatan like '%Coordinator%' or kodejabatan='46' or kodejabatan='194') ";
	...
	makeElement('asisten','select',$data['asisten'],
	    array('style'=>'width:150px'),$optKaryAst)
    );
    $els[] = array(
