...
if($afdId!='')
{
    $str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.kodeorg like '".$afdId."%' and b.noakun like '621%'
    GROUP BY b.noakun";
}
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
...
$iptarpthn="select sum(rupiah) as rupiah,sum(volume) as volume,noakun from ".$dbname.".pta_dt_blok_vw where "
        . " tahun='".$tahun."' and statusblok='TM'  and noakun like '621%' group by noakun";
...
$iptarpbln="select sum(rupiah) as rupiah,sum(volume) as volume,noakun from ".$dbname.".pta_dt_blok_vw where "
        . " bulan='".$periode."' and statusblok='TM'  and noakun like '621%' group by noakun";
...
$iptarpsdbln="select sum(rupiah) as rupiah,sum(volume) as volume,noakun from ".$dbname.".pta_dt_blok_vw where "
        . " (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and statusblok='TM'  and noakun like '621%' group by noakun";
...
// excel array content =========================================================================
    ...
    if(!empty($dzArr))foreach($dzArr as $keg){
        if(strlen($keg['noakun'])==5){
            ...
            $gkeg[810]+=$keg[810];
            $gkeg[811]+=$keg[811]/1000;
            $gkeg[812]+=$total[812];
            
            $gkeg[820]+=$keg[820];
            $gkeg[821]+=$keg[821]/1000;
            $gkeg[822]+=$total[822];
            
            $gkeg[830]+=$keg[830];
            $gkeg[831]+=$keg[831]/1000;
            $gkeg[832]+=$total[832];
    ...
    $tab.= "<td align=right>".numberformat($gkeg[810],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[811],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[812],0)."</td>";
    
    $tab.= "<td align=right>".numberformat($gkeg[820],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[821],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[822],0)."</td>";
    
    $tab.= "<td align=right>".numberformat($gkeg[830],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[831],0)."</td>";
    $tab.= "<td align=right>".numberformat($gkeg[832],0)."</td>";
