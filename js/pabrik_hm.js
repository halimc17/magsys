...
function simpanData(){
	...
	if(kodeorg=='' ||  stasiun=='' ||  mesin=='' ||  tanggal==''){
		...
		if (parseFloat(hmakhir)>=parseFloat(hmawal) && parseFloat(jam)>=0){
			...
			param+='&hmawal='+hmawal+'&hmakhir='+hmakhir+'&jam='+jam+'&tipeservice='+tipeservice+'&tipesrvlama='+tipesrvlama;
...
function hitungjam(num){
	...
	if (num==1){
		if (hmakhir>=hmawal){
			document.getElementById('jam').value = (hmakhir-hmawal).toFixed(2);
		}else{
			if(hmakhir!=0){
	...
	if (num==2){
		if (jam>=0){
			document.getElementById('hmakhir').value = (hmawal+jam).toFixed(2);
		}else{
