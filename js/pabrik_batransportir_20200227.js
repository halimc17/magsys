...
function fillfield(kodeorg,notransaksi,tanggal,koderekanan,nokontrak,keterangan,hasilkerjarealisasi,jumlahrealisasi,nilaippn){
...
function simpanData(){
	...
	if(notransaksi=='' || kodeorg=='' || tanggal=='' || trpcode=='' || nospp=='' || jumlahkg=='' || jumlahkg==0 || hargakg=='' || hargakg==0 || jmlharga=='' || jmlharga==0){
		...
		param+='&trpcode='+trpcode+'&nospp='+nospp+'&jumlahkg='+jumlahkg+'&hargakg='+hargakg+'&jmlharga='+jmlharga;
		tujuan='pabrik_slave_save_batransportir.php';
		post_response_text(tujuan, param, respog);		
	}
	function respog(){
