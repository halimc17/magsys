...
{
	if(typeof kodeasset=='undefined') kodeasset = "";
	...
	if (kodeasset!='') param += '&kodeasset='+kodeasset;
		...
		if(con.readyState==4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					var res = con.responseText, opt,
						fAsset = document.getElementById('kodeasset');
					res = res.split('#####');
					opt = JSON.parse(res[1]);
					document.getElementById('container').innerHTML=res[0];
					fAsset.options.length = 0;
					//fAsset.options[0] = new Option("","");
					selIndex = 0;
					for(i in opt) {
						if(i==kodeasset) selIndex=fAsset.options.length;
						fAsset.options[fAsset.options.length] = new Option(opt[i],i);
					}
					fAsset.selectedIndex = selIndex;
				}
			} else {
				busy_off();
				error_catch(con.status);
...
kodebarang,kepemilikan,kodetraksi,tglakhirstnk,tglakhirkir,tglakhirijinbm,tglakhirijinang,kodeasset,detailvhc)
{
