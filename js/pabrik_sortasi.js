...
function addDetail(brs)
{
    ...
    pros=document.getElementById('proses').value;
    param="proses="+pros+"&noTiket="+noTkt+"&jmlhJJg="+jmlh+'&persenBrnd='+prsn+'&kgPtngan='+kgPtngan;
    param+=strUrl;
...
function hitungPotongan(val,kodefraksi,field)
{
    if(document.getElementById('bjrx').innerHTML=='' || document.getElementById('bjrx').innerHTML==0)
       ...
       for(x=1;x<=field;x++)
        ...
        persenBrnd=document.getElementById('persenBrnd').value;
        persenBrnd=parseFloat(persenBrnd)
        if(persenBrnd<12.5)
            {
               totpot+=((12.5/100*nettox)-(persenBrnd/100*nettox))*(brd);
            }
    }
