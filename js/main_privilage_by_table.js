...
function saveMenu(parent,caption,action,showlink,hideinput,type,newfile)
{
        ...
        param='id_parent='+id_parent+'&caption='+_caption+'&action='+_action+'&class='+clas+'&create='+createFile;
        //alert(param);
        if (clas == 'Type...') {
           alert('Choose type..!');
           objType.focus();
                           ...
                           if (con.responseText.lastIndexOf('Warning') > -1) {
                                alert('ERROR TRANSACTION,\n'+con.responseText);
                                    ...
                                    arr=con.responseText.split(',');
                                        ...
                                        //_in += "<center>";
                                        _in += "<input type=button class=mybutton value=Save onclick=saveMenu('master_menu" + _id + "','newCaption" + _id + "','newAction" + _id + "','link" + _id + "','inputmenu" + _id + "','type" + _id + "','newFile"+ _id +"');>";
...
// 	Get id submenu 
    param='parent='+id+'&sub='+sub;
        post_response_text('slave_main_privilage_by_table.php?proses=getForm', param, respog);
...
//jamhari
function addData(brske,idmen)
{
    idmen=document.getElementById('id_menu').value;
    stat=document.getElementById('adddt_'+brske);
    ...
    usrnm=document.getElementById('usr_nm_'+brske).innerHTML;
    param='id_menu='+idmen+'&usernm='+usrnm+'&stat='+dstat;
    post_response_text('slave_main_privilage_by_table.php?proses=addData', param, respog);
    function respog(){
              if(con.readyState==4)
              {
                        if (con.status == 200) {
