        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('afdeling2').innerHTML = con.responseText;
                }
            } else {
                busy_off();
                error_catch(con.status);
...
function getpreview2(){
    ...
    inti=document.getElementById('inti2').options[document.getElementById('inti2').selectedIndex].value;
    param='pt='+pt+'&unit='+unit+'&afdeling='+afdeling+'&periode='+periode+'&inti='+inti+'&proses=preview';
    tujuan='mr_slave_biayaProduksiTbsDetail.php';
     function respon() {
        if (con.readyState == 4) {
            if (con.status == 200) {
                busy_off();
