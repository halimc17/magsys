...
function loadData(num){
	...
	param+='&page='+num;
	tujuan='kebun_adjpanen_slave.php';
...
function simpanData(){
	...
	supirlangsir=document.getElementById('supirlangsir').value;
	keterangan 	=document.getElementById('keterangan').value;
	catatan  	=document.getElementById('catatan').value;
	addedit		=document.getElementById('addedit').value;
	if(kodeorg=='' ||  tanggal=='' ||  jenis=='' ||  janjang==0){
		alert('Fields are required');
	}else{
		param='kodeorg='+kodeorg+'&tanggal='+tanggal+'&jenis='+jenis+'&waktu='+waktu+'&janjang='+janjang;
		param+='&supirlangsir='+supirlangsir+'&keterangan='+keterangan+'&catatan='+catatan;
...
function fillfield(kodeorg,tanggal,jenis,waktu,janjang,supirlangsir,keterangan,catatan){
	...
	document.getElementById('supirlangsir').value=supirlangsir;
		...
		if(con.readyState==4){
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('catatan').value=con.responseText;
				}
			}else{
				busy_off();
				error_catch(con.status);
...
function bersihkanForm(){
	...
	document.getElementById('supirlangsir').value='';
...
function preview_BAPDF(kodeorg,tanggal,jenis,ev){
	param='kodeorg='+kodeorg+'&tanggal='+tanggal+'&jenis='+jenis;
	tujuan='kebun_adjpanen_BAPDF.php?'+param;
...
function cariinventaris(type,ev){
	...
	tujuan='kebun_adjpanen_showpopup.php?'+param;
	title='Adjustment Panen '+carikodeorg+' '+carijenis;
	width='720';
	height='400';
	content="<iframe frameborder=0 width=100% height=100% src='"+tujuan+"'></iframe>"
	showDialog1(title,content,width,height,ev);
