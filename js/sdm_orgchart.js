 ...
 */
 ...
 function saveOrg()
 {
		...
		_karyName = document.getElementById('karyId').options[document.getElementById('karyId').selectedIndex].text;
...
//response++++++++++++++++++++++++++++++++++++++++
           function respog(){
                //save active org on memory incase slow server response
                        id         = activeOrg;
                        newCaption = _kdstruktur + ": " + _karyName;
              if(con.readyState==4)
              {
                        if (con.status == 200) {
                                        busy_off();
                                        if (!isSaveResponse(con.responseText)) {
                                                ...
                                                else if(id.lastIndexOf('_new')>-1)
                                                {
                                                  if (clos<9) {
                                                        nex=clos+1;
                                                        ne = "<li class=mmgr>";
                                                        ne += "<img title=expand class=arrow src='images/foldc_.png' height=17px  onclick=show_sub('gr" + _kdstruktur + "',this);>";
                                                        ne += "<a class=elink id='el" + _kdstruktur + "'  onclick=\"javascript:activeOrg=this.id;orgVal='" + orgVal + "';getCurrent('" + _kdstruktur + "');setpos('inputorg',event);\">" + _kdstruktur + ': '+ _karyName + "</a>";
                                                        ne += "<ul id=gr" + _kdstruktur + " style='display:none;'>";
                                                        ne += "<div id=main" + _kdstruktur + ">";
                                                        ne += "</div>";
                                                        ne += "<li class=mmgr>";
                                                        ne += "<a id='" + _kdstruktur + "_new' class=elink title='Create Child'  onclick=\"javascript:orgVal='" + _kdstruktur + "';clos="+nex+";activeOrg='" + _kdstruktur + "_new';setpos('inputorg',event);\">New Org<a>";
                                                        ne += "</li>";
                                                        ne += "</ul>";
                                                        ne += "</li>";
                                                  }
                                                  else
                                                  {
                                                        ...
                                                        ne += "<a class=elink id='el" + _kdstruktur + "'  onclick=\"javascript:activeOrg=this.id;orgVal='" + orgVal + "';getCurrent('" + _kdstruktur + "');setpos('inputorg',event);\">" + _kdstruktur + "</a>";
                            ne += "</li>";					  	
                                                  }						
                          //alert('main'+orgVal);
