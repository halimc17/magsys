...
function saveDiagnosa()
{
	...
	kodekelompok=document.getElementById('kodekelompok').options[document.getElementById('kodekelompok').selectedIndex].value;
	name=document.getElementById('name').value;
	param='idx='+idx+'&kodekelompok='+kodekelompok+'&name='+name;
	if (name != '') {
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                //else {
					...
					document.getElementById('kodekelompok').options[0].selected=true;
					document.getElementById('tbody').innerHTML=con.responseText;
				//}
            }
            else {
...
function editDiagnosa(id,kodekelompok,name)
{
	document.getElementById('idx').value=id;
	document.getElementById('name').value=name;	
	l=document.getElementById('kodekelompok');
    
    for(a=0;a<l.length;a++){
		if(l.options[a].value==kodekelompok){
