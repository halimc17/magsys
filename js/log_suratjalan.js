        ...
        if(con.readyState==4) {
            if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					x.cells[9].innerHTML="<img class='zImgBtn' title=Lengkap' src='images/skyblue/posted.png'>";
					x.cells[8].childNodes[0].onclick='';
...
function saveDetail(num) {
	var nosj = document.getElementById('nosj').value,
		...
		nopp = getById('t_nopp_'+num),
		param='nosj='+nosj+'&kodept='+getValue('kodept')
			+'&kodebarang='+getInner('t_kodebarang_'+num)
			+'&nopo='+nopo.getAttribute('value')
			+'&nopp='+nopp.getAttribute('value')+'&jumlah='+jml.value;
		body = document.getElementById('bodySearch'),
...
function deleteDetail(num) {
	var nosj = document.getElementById('nosj').value,
		...
		param='nosj='+nosj+'&kodept='+getValue('kodept')
			+'&kodebarang='+getInner('t_kodebarang_'+num)
			+'&nopo='+(getById('el_nopo_'+num)? getValue('el_nopo_'+num) : getInner('t_nopo_'+num))
			+'&nopp='+(getById('el_nopp_'+num)? getValue('el_nopp_'+num) : getInner('t_nopp_'+num)),
		body = document.getElementById('bodySearch'),
