...
function getInvNameSaldo(event,id,idGudang,tSaldo) {
    var cont = "<fieldset><legend><b>Search</b></legend>";
    cont += "<input id='invSearch' type='text' onkeypress=\"if(getKey(event)==13){searchInvNameSaldo('invSearch','"+id+"','"+idGudang+"','"+tSaldo+"');} else {return tanpa_kutip(event)}\" />";
    cont += "<img src='images/search.png' onclick=\"searchInvNameSaldo('invSearch','"+id+"','"+idGudang+"','"+tSaldo+"')\" style='cursor:pointer'>";
    cont += "</fieldset>";
