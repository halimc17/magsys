...
function get_blok_penghuni()
{
	kd_org=document.getElementById('kode_org_penghuni').value;
	tujuan='log_get_data_header_rumah.php'
	param='method=get_blok_penghuni'+'&code_org='+kd_org;
	post_response_text(tujuan, param, respog);		
	//alert(param);
	function respog(){
			if (con.readyState == 4) {
...
//						document.getElementById('blok_rmh_penghuni').innerHTML=con.responseText;
						pisah=con.responseText.split('###');
                                                document.getElementById('blok_rmh_penghuni').innerHTML=pisah[0];
                                                document.getElementById('kode_karyawan').innerHTML=pisah[1];
                                                load_data_penghuni();
					}
				}
				else {
