...
function goToPages(page,shows,where) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=showHeadList', param, respon);
}
...
function showAdd(tipe) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=showAdd', param, respon);
}
...
function showEditFromAdd(tipe) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=showEdit', param, respon);
}
...
function addDataTable(tipe) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=add&tipe='+tipe, param, respon);
}
...
function editDataTable(tipe) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=edit', param, respon);
}
...
function showDetail() {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					for(var i=0; i<tbody.length; i++) {
						...
						if (color) {
							...
							tbody[i].setAttribute('title','Karyawan Bongkar Muat lebih dari 1 blok');
						}
					}
                }
            } else {
    ...
    post_response_text('kebun_rkb_slave_detail.php?proses=showDetail', param, respon);
}
...
function deleteData(num) {
    ...
    post_response_text('kebun_rkb_slave_operasional.php?proses=delete', param, respon);
}
...
function getLaporanRKB_1()
{
    ...
    tujuan='kebun_laporanRKB_tanggal.php';
...
function getLaporanRKB_2()
{
    ...
    tujuan='kebun_laporanRKB_orang.php';
...
function getLaporanRKB_3()
{
    ...
    tujuan='kebun_laporanRKB_spbwb.php';
...
function postingData(numRow) {
    ...
    if(confirm('Akan dilakukan posting untuk transaksi '+notrans+
        ...
        post_response_text('kebun_rkb_slave_posting.php', param, respon);
    }
}
...
function updTahunTanam() {
    ...
    post_response_text('kebun_rkb_slave_detail.php?proses=updTahunTanam', param, respon);
}
...
function updUpah() {
    ...
    post_response_text('kebun_rkb_slave_detail.php?proses=updUpah2', param, respon);
}

function detailPDF(numRow,ev) {
    // Prep Param
    var notransaksi = document.getElementById('notransaksi_'+numRow).getAttribute('value');
    param = "proses=pdf&tipe=RKB"+"&notransaksi="+notransaksi;
    
    showDialog1('Print PDF',"<iframe frameborder=0 style='width:795px;height:400px'"+
        " src='kebun_rkb_slave_operasional_print_detail.php?"+param+"'></iframe>",'800','400',ev);
...
function detailExcel(numRow,ev) {
    // Prep Param
    var notransaksi = document.getElementById('notransaksi_'+numRow).getAttribute('value');
    param = "proses=excel&tipe=RKB"+"&notransaksi="+notransaksi;
...
function printPDF(ev,tipe) {
    // Prep Param
    param = "proses=pdf&tipe=RKB";
    
    showDialog1('Print PDF',"<iframe frameborder=0 style='width:795px;height:400px'"+
        " src='kebun_rkb_slave_operasional_print.php?"+param+"'></iframe>",'800','400',ev);
    var dialog = document.getElementById('dynamic1');
    dialog.style.top = '50px';
    dialog.style.left = '15%';
}
   ...
   param='afd='+val+'&tipe=afdeling';
    ...
    {        
     ...
     param='afd='+val+'&tipe=unit';
       ...
       post_response_text('kebun_rkb_slave_detail.php?proses=gatKarywanAFD', param, respon);     
...
function countPremi() {
	...
	timer = setTimeout(function() {
		var blokCont = getById('ftPrestasi_kodeorg').firstChild,
			...
			hasilkerjakg = getValue('hasilkerjakg'),
			tanggal = document.getElementById('tanggal').value,
			tahun=tanggal.substr(6, 4),
			param = "";
		param += "blok="+blok+"&hasilkerjakg="+hasilkerjakg+"&tahun="+tahun;
		param += "&basis="+getValue('norma');
		param += "&tanggal="+getValue('tanggal');
		param += "&nik="+getValue('nik');
		post_response_text('kebun_rkb_slave_detail.php?proses=countPremi', param, respon);
	},500);
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					setValue('upahkerja',res.upahkerja);
					setValue('upahpremi',res.upahpremi);
