    ...
    //	return;
        ...
        function respog()
        {
            ...
            if (con.status == 200) {
				...
				if (jnsIjin=='CUTI'){
				   if ((sisa-hk) < 0){
				      alert('Jumlah HK(Hari) melebihi jumlah sisa cuti untuk periode '+periodec+' dan akan dipotong gaji sebanyak '+(hk-sisa)+' hari.');
				   }
				}
                            cancelForm();
                            loadNData();
...
function jumlahijin(){
    ...
    var tglEnd=document.getElementById('tglEnd').value;
    jnsIjin=document.getElementById('jnsIjin').options[document.getElementById('jnsIjin').selectedIndex].value;
    if((jnsIjin=='CUTI' || jnsIjin=='MELAHIRKAN' || jnsIjin=='KAWIN/SUNATAN/WISUDA') && (tglAwal!='' || tglEnd!='')){
       jumlahhari();
    }else{
	   document.getElementById("jumlahhk").value = 0;
    }
...
function masterPDF(tgl,karywn,ev)
{
        tglijin=tgl;
        krywnId=karywn;
