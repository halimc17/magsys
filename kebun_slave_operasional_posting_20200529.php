...
while($bar1=mysql_fetch_object($resc)){
        ...
        $str="select max(notransaksi) as notransaksi from ".$dbname.".log_transaksiht where tipetransaksi=5 and kodegudang='".$gudang."' and
            tanggal>=".$_SESSION['org']['period']['start']." and tanggal<=".$_SESSION['org']['period']['end']." and notransaksireferensi!=''
...
while($bar=mysql_fetch_object($resp)){ 
  ...
  else
  {
                ...
                $errSal='';
                  ...
                  foreach($gud as $keygud=>$valgud){
                        ...
                        $sth="update ".$dbname.".log_5saldobulanan set saldoakhirqty=".$saldo[$valgud][$valbrg].",
                                ...
                                qtykeluar=".$qtykeluar[$valgud][$valbrg].",qtykeluarxharga=".$xkeluar[$valgud][$valbrg]."
                                where periode='".$periode[$valgud]."' and kodegudang='".$valgud."' and kodebarang='".$valbrg."'"; 
                        mysql_query($sth);
                        }  
                    }
	...
	if(empty($errorDB)) { // Update Setup Blok, jika kegiatan sisip, completering, tanam baru
		$where = "nilai = '".$kodekegiatan."' and kodeaplikasi = 'TN'"; // kalo kodeparameter SISIP
		...
		if(!empty($resSisip)) {
			...
			// Update Blok Baru
			$dataUpd = array(
				'jumlahpokok' => $resBlok[0]['jumlahpokok'] + $hasilkerja
			);
