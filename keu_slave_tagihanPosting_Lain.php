<?//@Copy nangkoelframework
	...
	while($res=mysql_fetch_assoc($qry)){
		$uangmuka=$res['uangmuka'];
		$tipeinvoice=$res['tipeinvoice'];
	}
	if($tipeinvoice=='p'){
		$str="select notransaksi from ".$dbname.".log_transaksiht where statusjurnal='1' and post='1' and nopo='".$nopo."'";
		$qry=mysql_query($str) or die(mysql_error($conn));
		$noBAPB="";
		while($res=mysql_fetch_assoc($qry)){
			$noBAPB=$res['notransaksi'];
		}
		//exit("Warning: ".$str.' '.$noBAPB);
		if($noBAPB==""){
			$str="select * from ".$dbname.".keu_jurnaldt where left(noakun,1)='2' and (nodok='".$nopo."' or keterangan like '%".$nopo."%')";
		}else{
			$str="select * from ".$dbname.".keu_jurnalht where noreferensi='".$noBAPB."'";
		}
	}else if($tipeinvoice=='s'){
		//$str="select * from ".$dbname.".keu_jurnaldt where nodok='".$nopo."'";
		$str="select * from ".$dbname.".keu_jurnaldt where left(noakun,1)='2' and (nodok='".$nopo."' or keterangan like '%".$nopo."%')";
	}else if($tipeinvoice=='t'){
		//$str="select * from ".$dbname.".keu_jurnaldt where nodok='".$nopo."'";
		$sparajrn="select * from ".$dbname.".keu_5parameterjurnal where jurnalid='PMB'";
		$qparajrn=mysql_query($sparajrn) or die ("SQL ERR : ".mysql_error($conn));
		$rparajrn=mysql_fetch_assoc($qparajrn);
		$noakundebet=$rparajrn['noakundebet'];
		$noakunkredit=$rparajrn['noakunkredit'];
		$str="select * from ".$dbname.".keu_jurnaldt where noakun='".$noakundebet."' and (nodok='".$nopo."' or keterangan like '%".$nopo."%')";
	}else if($tipeinvoice=='b'){
		$str="select * from ".$dbname.".keu_jurnalht where noreferensi like 'EXP01-".$nopo."%'";
	}else{
		$str="select * from ".$dbname.".keu_jurnalht where noreferensi='".$nopo."'";
	}
	$qry=mysql_query($str) or die ("SQL ERR : ".mysql_error($conn));
	$row=mysql_num_rows($qry);
	if($uangmuka==0 and $row==0){
		exit("Warning: Belum ada penerimaan barang...!");
	}
