<?php
    ...
    $stream.="<table cellspacing='1' border='".$brd."' class='sortable'>
				<thead class=rowheader>
					<tr>
						<td rowspan=2 width='70px'".$bgclr.">".$_SESSION['lang']['divisi']."</td>";
	for ($x = 1; $x <= 12; $x++){
	...
	foreach($xunit as $xun=>$vun){
		$x+=1;
		if($x==1){
			$whr.=" and (kodeorganisasi like '".$vun."0%' or kodeorganisasi like '".$vun."1%'";
		}else{
	...
	if($whr!=""){
		...
		$whr=" and induk in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$kodept."')";
	}
	#ambil data Adjustmen Panen
	$str  ="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where tipe='AFDELING' ".$whr." ORDER BY kodeorganisasi";
	//exit('Warning: '.$str);
	$res=mysql_query($str);
	$stlbt=array();
	$stlbrd=array();
