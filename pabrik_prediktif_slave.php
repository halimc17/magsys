...
switch($method){
    ...
    case 'insert':  //$komMain=$_POST['komMain']; //$komPros=$_POST['komPros'];
            ...
            $iSave="insert into ".$dbname.".pabrik_predictiveht (`notransaksi`, `pabrik`, `tanggal`, `jam`,
    ...
    case'update':
        //exit("Error:MASUK");
        $iUpdate="update ".$dbname.".pabrik_predictiveht set namapemohon='".$namaPemohon."',statuspemohon='".$statusPemohon."',
    ...
    case'getNodok':
        $iList="select notransaksi,tanggal,statasiun  from ".$dbname.".pabrik_predictiveht where statasiun ='".$station."' "
    ...
    case'loadData':
       //exit("Error:ASDASDAS");
           ...
           //exit("Error:$schTgl");
            ...
            $ql2="select count(*) as jmlhrow from ".$dbname.".pabrik_predictiveht where pabrik='".$_SESSION['empl']['lokasitugas']."' ".$nodokSch." ".$tglSch." order by `notransaksi` desc";// echo $ql2;notran
            //exit("Error:$ql2");
            ...
            $iList="select * from ".$dbname.".pabrik_predictiveht where  pabrik='".$_SESSION['empl']['lokasitugas']."' ".$nodokSch." ".$tglSch."  order by `tanggal` desc, `notransaksi` desc limit ".$offset.",".$limit."";
            ...
            while($dList=mysql_fetch_assoc($nList)){
                ...
                <td>".$dList['kegiatan']."</td>  

                <td align=center>";

                echo"<img src=images/application/application_edit.png class=resicon  title='Edit Detail' 
                ...
                <img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('pabrik_predictiveht','".$dList['notransaksi']."','','pabrik_prediktif_slave_pdf',event)\">
	...
	##########case delete
	case 'deleteHead':
		$iDel="delete from ".$dbname.".pabrik_predictiveht where notransaksi='".$nodok."' ";
		if(mysql_query($iDel)){
		}else{
			echo " Gagal,".addslashes(mysql_error($conn));
		}			
	...
	########### case insert detail
	case 'saveBarang':
		$iBarang="insert into ".$dbname.".pabrik_predictivedt (`notransaksi`,`kodebarang`,`satuan`,`jumlah`,`keterangan`,`harga`)
	...
	case 'loadDetailBarang':	
            ...
            $no=0;
            $iListBarang="select * from ".$dbname.".pabrik_predictivedt where notransaksi='".$nodok."' ";
	...
	break;
        ...
        case 'deleteBarang':
                $iDelBarang="delete from ".$dbname.".pabrik_predictivedt where notransaksi='".$nodok."' and kodebarang='".$kodeBarang."' ";
        ...
        ##pekerjaan
        case 'savePekerjaan':
            $iKaryawan="insert into ".$dbname.".pabrik_predictivedt_pekerjaan (`notransaksi`,`nomor`,`rincian`,`kondisi`,`updateby`)
	...
	case 'loadDetailPekerjaan':
            ...
            <td>".$_SESSION['lang']['action']."</td></tr></thead>";
            $no=0;
            $iListPekerjaan="select * from ".$dbname.".pabrik_predictivedt_pekerjaan where notransaksi='".$nodok."'"
                    . " order by nomor asc ";
	...
	break;//
        ...
        case 'deletePekerjaan':
            $iDelPekerjaan="delete from ".$dbname.".pabrik_predictivedt_pekerjaan where notransaksi='".$nodok."' and nomor='".$nomor."' ";
        ...
        #karyawan
        case 'saveKaryawan':
            $iKaryawan="insert into ".$dbname.".pabrik_predictivedt_karyawan (`notransaksi`,`karyawanid`,`updateby`)
	...
	break;
        ...
        case 'deleteKaryawan':
            $iDelPekerjaan="delete from ".$dbname.".pabrik_predictivedt_karyawan where notransaksi='".$nodok."' 
	...
	case 'loadDetailKaryawan':
            ...
            <td>".$_SESSION['lang']['action']."</td></tr></thead>";
            $no=0;
            $iListKaryawan="select * from ".$dbname.".pabrik_predictivedt_karyawan where notransaksi='".$nodok."' ";
