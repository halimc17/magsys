...
$iList="SELECT *,round(TIMESTAMPDIFF(MINUTE,jammulai,jamselesai)/60,2) as jamjalan 
		FROM ".$dbname.".pabrik_predictiveht where tanggal between '".$tgl1."' and '".$tgl2."' and"
...
$arrTipePerbaikan=array("prev"=>"Preventive Maintenance","kalibrasi"=>"Kalibrasi","project"=>"Project",
    "pabrikasi"=>"Pabrikasi","corrective"=>"Corrective Maintenance","service"=>"Service","predictive"=>"Predictive Maintenance");
...
if(isset($notransaksi)){
	foreach ($notransaksi as $notran){
		...
		$stream.="<tr class=rowcontent>";
			...
			$stream.="<td align=right valign=top rowspan=".$rowspan.">".$jumlahjamperbaikan[$notran]."</td>";
			...
			if(empty($barang[$notran]) and empty($kar[$notran])){
				...
				if($proses!='excel'){
					$stream.="<td valign=top rowspan=".$rowspan.">
								...
								<img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('pabrik_predictiveht','".$notran."','','pabrik_prediktif_slave_pdf',event)\">
							</td>";  
				}
			}else{
				for($i=0;$i<$rowspan;$i++) {
...
switch($proses){
    case'getListBarangLaporan':
        $isi="<table cellspacing='1' class='sortable'><thead class=rowheader>
              ...
              <tbody>";
        $iBrg="SELECT * FROM ".$dbname.".pabrik_predictivedt where notransaksi='".$_POST['nodok']."'";
...
######EXCEL	
	case 'excel':
		...
		$tglSkrg=date("Ymd");
		$nop_="LAPORAN_PREDIKTIF_I_".$tglSkrg;
		if(strlen($stream)>0){
