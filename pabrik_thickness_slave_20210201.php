...
switch($proses){
	case'loadData':
		...
		$strtb="select a.tebal1,a.tebal2,a.tebal3,a.tipeservice from ".$dbname.".pabrik_thickness a where a.kodemesin like '".$mesin."%' and a.tipeservice<>0 and tanggal<'".$periodetb."-01'
		...
		while($bar=mysql_fetch_object($res)){
			...
			$tebalkecil=($bar->tebal1<$bar->tebal2 ?  $bar->tebal1 : $bar->tebal2);
			...
			echo"<tr class=rowcontent >
					...
					<td ".$drcl." align=left width='10%'>".$jenisservice."</td>
					<td ".$drcl." align=right width='5%'>".number_format($bar->tebal1,2,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($bar->tebal2,2,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($bar->tebal3,2,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format(($tebalkecil/$tebalsisa*100),2,'.',',')."</td>
					<td ".$drcl." align=left>".$bar->keterangan."</td>
					<td align=center width='7%'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodeorg."','".$bar->kodemesin."','".tanggalnormal($bar->tanggal)."','".$bar->tipeservice."','".round($bar->tebal1,2)."','".round($bar->tebal2,2)."','".round($bar->tebal3,2)."','".$bar->keterangan."')\">&nbsp
	...
	case'saveData':
		...
		while($bara=mysql_fetch_object($resa)){
			if($bara->tipeservice=='0' and ($tebal1>$bara->tebal1 or $tebal2>$bara->tebal2 or $tebal3>$bara->tebal3)){
		...
		if($addedit=='update'){
			...
			if($kodeorglama==$kodeorg and $kodemesinlama==$mesin and $tanggallama==$tanggal){
				...
				$strs="select * from ".$dbname.".pabrik_thickness
						where (kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal>'".$tanggal."') 
						   or (kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal='".$tanggal."' and tipeservice<>'".$tipesrvlama."')";
				$ress=mysql_query($strs);
			...
			$strx="update ".$dbname.".pabrik_thickness set kodeorg='".$kodeorg."',kodemesin='".$mesin."',tanggal='".$tanggal."'
					,tebal1=".round($tebal1,2).",tebal2=".round($tebal2,2).",tebal3=".round($tebal3,2).",tipeservice='".$tipeservice."',keterangan='".$keterangan."'
					,lastuser='".$_SESSION['standard']['username']."',lastdate=now()
					where kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal='".$tanggal."' and tipeservice='".$tipesrvlama."'";
		}else{
			$strx="insert into ".$dbname.".pabrik_thickness (kodeorg,kodemesin,tanggal,tebal1,tebal2,tebal3,tipeservice,keterangan,lastuser,lastdate)
					values('".$kodeorg."','".$mesin."','".$tanggal."',".round($tebal1,2).",".round($tebal2,2).",".round($tebal3,2)."
							,'".$tipeservice."','".$keterangan."','".$_SESSION['standard']['username']."',now())";
		}
		if(!mysql_query($strx)){
