...
<?
	...
	$strz="select a.*,b.namaorganisasi from ".$dbname.".kebun_adjpanen a 
	...
	$stream2="";
	if($_GET['type']=='excel'){
		$stream2.="<h2>ADJUSTMENT PANEN</h2>";
	...
	if($rowz==0){
		...
		while($barz=mysql_fetch_object($resz)){
            ...
            $stream2.="<tr class=rowcontent>
						...
						<td align=left>".$barz->supirlangsir."</td>
						<td align=left>".$barz->keterangan."</td>
						<td align=left>".$barz->catatan."</td>
					</tr>";
	...
	if($_GET['type']!='excel'){
		...
		$nop_="Adjustment_Panen_".date("YmdHis");
        if(strlen($stream2)>0){
