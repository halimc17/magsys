...
switch($proses){
	case'loadData':
		...
		if(substr($periodetb,5,2)=='01'){
			//$periodetb2=sprintf("%04d",substr($periodetb,0,4)-1)."-12-01";
			$periodetb2=sprintf("%04d",substr($periodetb,0,4)-2)."-12-01";
		}else{
			//$periodetb2=substr($periodetb,0,4)."-".sprintf("%02d",substr($periodetb,5,2)-1)."-01";
			$periodetb2=(substr($periodetb,0,4)-1)."-".sprintf("%02d",substr($periodetb,5,2)-1)."-01";
		}
		//$periodetb2=date('Y-m-d',strtotime('-1 month',$periodetb));
		//exit('Warning: '.$periodetb2);
		$strtb="select a.hmakhir,a.tipeservice from ".$dbname.".pabrik_hm a where a.kodemesin like '".$mesin."%' and a.tipeservice<>0 and tanggal<'".$periodetb2."' 
				order by a.kodemesin,a.tanggal,a.tipeservice";
		//$strtb="select a.hmakhir,a.tipeservice from ".$dbname.".pabrik_hm a where a.kodemesin like '".$mesin."%' and a.tipeservice<>0 and tanggal<'".$periodetb."-01'
		//		order by a.kodemesin,a.tanggal,a.tipeservice";
		//$strtb="select periode from ".$dbname.".setup_periodeakuntansi where tutupbuku=0 and kodeorg='".$kodeorg."' order by periode limit 1";
		$restb=mysql_query($strtb);
		$hmsisa1=0;
		...
		$str="select a.*,b.namaorganisasi from ".$dbname.".pabrik_hm a 
				...
				where a.kodemesin like '".$mesin."%' and tanggal>='".$periodetb2."' 
				order by a.kodemesin,a.tanggal,a.tipeservice";
		//$str="select a.*,b.namaorganisasi from ".$dbname.".pabrik_hm a 
		//		left join ".$dbname.".organisasi b on b.kodeorganisasi=a.kodemesin
		...
		while($bar=mysql_fetch_object($res)){
			if($awalmesin!=$bar->kodemesin){
				...
				$jamganti=10000;
				$jamjalan1=$bar->hmawal-$hmsisa1;
				$jamjalan2=$bar->hmawal-$hmsisa2;
				$jamjalan3=$bar->hmawal-$hmsisa3;
			...
			echo"<tr class=rowcontent >
					...
					<td align=center width='7%'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodeorg."','".$bar->kodemesin."','".tanggalnormal($bar->tanggal)."','".$bar->tipeservice."','".round($bar->hmawal,2)."','".round($bar->hmakhir,2)."','".round($bar->jam,2)."','".$bar->keterangan."')\">&nbsp
						...
						<img src=images/zoom.png class=resicon title='Detail' onclick=\"showpopup('".$bar->kodemesin."','".$bar->tanggal."','".$bar->kodeorg."','".substr($bar->kodemesin,0,6)."','preview',event);\">&nbsp
						<img src=images/skyblue/excel.jpg class=resicon title='Detail' onclick=\"showpopup('".$bar->kodemesin."','".$bar->tanggal."','".$bar->kodeorg."','".substr($bar->kodemesin,0,6)."','excel',event);\">
					</td>
				</tr>";	
		}
	break;
	...
	case'delData':
		...
		if(!mysql_query($strx)){
			...
			$strz="select hmakhir from ".$dbname.".pabrik_hm where kodemesin='".$mesin."' order by kodemesin,tanggal desc limit 1";
			$qtrz=mysql_query($strz);
			$hmakhirst=0;
			while($rtrz=mysql_fetch_object($qtrz)){
				$hmakhirst=$rtrz->hmakhir;
			}
			$strz="update ".$dbname.".pabrik_hm_setup set hmakhir=".round($hmakhirst,2)." where kodemesin='".$mesin."'";
			if(!mysql_query($strz)){
	...
	case'saveData':
		...
		if($addedit=='update'){
			$strs="select * from ".$dbname.".pabrik_hm
					where kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal='".$tanggal."' and tipeservice='".$tipesrvlama."'";
			...
			while($bars=mysql_fetch_object($ress)){
				...
				$hmawallama=round($bars->hmawal,2);
				$hmakhirlama=round($bars->hmakhir,2);
				$jamlama=round($bars->jam,2);
			}
			if($kodeorglama==$kodeorg and $kodemesinlama==$mesin and $tanggallama==$tanggal and $hmawallama==round($hmawal,2) and $hmakhirlama==round($hmakhir,2) and $jamlama==round($jam,2)){
				//exit('Warning: masuk'.$kodeorglama.'=='.$kodeorg.' '.$kodemesinlama.'=='.$mesin.' '.$tanggallama.'=='.$tanggal.' '.$hmawallama.'=='.round($hmawal,2).' '.$hmakhirlama.'=='.round($hmakhir,2).' '.$jamlama.'=='.round($jam,2));
			}else{
				$strs="select * from ".$dbname.".pabrik_hm
			...
			$strx="update ".$dbname.".pabrik_hm set kodeorg='".$kodeorg."',kodemesin='".$mesin."',tanggal='".$tanggal."'
					,hmawal=".round($hmawal,2).",hmakhir=".round($hmakhir,2).",jam=".round($jam,2).",tipeservice='".$tipeservice."'
					,jamganti=".round($jamganti,0).",keterangan='".$keterangan."',lastuser='".$_SESSION['standard']['username']."',lastdate=now()
					where kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal='".$tanggal."' and tipeservice='".$tipesrvlama."'";
		}else{
			$strx="insert into ".$dbname.".pabrik_hm (kodeorg,kodemesin,tanggal,hmawal,hmakhir,jam,tipeservice,jamganti,keterangan,lastuser,lastdate)
					values('".$kodeorg."','".$mesin."','".$tanggal."',".round($hmawal,2).",".round($hmakhir,2).",".round($jam,2).",'".$tipeservice."'
					,'".round($jamganti,0)."','".$keterangan."','".$_SESSION['standard']['username']."',now())";
		}
		if(!mysql_query($strx)){
			echo " Gagal, ".$strx,' '.addslashes(mysql_error($conn));
		}else{
			if($rowa>0){
				$strz="update ".$dbname.".pabrik_hm_setup set hmakhir=".round($hmakhir,2)." where kodemesin='".$mesin."'";
			}else{
				$strz="insert into ".$dbname.".pabrik_hm_setup (kodemesin,jamganti1,jamganti2,jamganti3,hmakhir,lastuser,lastdate)
						values('".$mesin."',0,0,0,'".round($hmakhir,2)."','".$_SESSION['standard']['username']."',now())";
			}
			if(!mysql_query($strz)){
				echo " Gagal, ".$strz,' '.addslashes(mysql_error($conn));
			}
