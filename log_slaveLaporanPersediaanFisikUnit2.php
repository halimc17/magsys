	...
	while($barx=mysql_fetch_object($resx))
	{
		...
		$pdf->Cell(25,5,$kamustipe[$barx->tipetransaksi],0,0,'C');
                if(substr($kodebarang,0,3)=='312'){
                    $pdf->Cell(25,5,number_format($sawal,3,'.',','),0,0,'R');
                    $pdf->Cell(25,5,number_format($masuk,3,'.',','),0,0,'R');		
                    $pdf->Cell(25,5,number_format($keluar,3,'.',','),0,0,'R');
                    $pdf->Cell(25,5,number_format($saldo,3,'.',','),0,1,'R');
                    }
                else{
