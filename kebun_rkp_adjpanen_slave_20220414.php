<?php
	...
	$str  ="select left(kodeorg,6) as divisi,jenis,left(tanggal,7) as periode,SUM(janjang) as jjg,SUM(kg) as kg,count(tanggal) as jml
			,count(DISTINCT tanggal) as BA
	...
	foreach($xjenis as $xjns=>$vjns){
		for ($x = 1; $x <= 12; $x++){
			...
			$stream.="		<td width='50px'".$bgclr.">BA</td>";
		}
		$stream.="			<td width='50px'".$bgclr.">Jjg</td>";
		$stream.="			<td width='50px'".$bgclr.">Kg</td>";
		$stream.="			<td width='50px'".$bgclr.">BA</td>";
