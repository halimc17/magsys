...
for ($i=1; $i<=$bulan; $i++)
{
    ...
    while($rData=mysql_fetch_assoc($qData))
    {
		setIt($resData[$rData['kodebarang']]['salak'],0);
        $resData[$rData['kodebarang']]['kobar']=$rData['kodebarang'];
        $terima=$per."R";  
        $resData[$rData['kodebarang']][$terima]=$rData['saldo'];
		if(!isset($resData[$rData['kodebarang']]['totem'])) $resData[$rData['kodebarang']]['totem']=0;
    ...
    while($rData=mysql_fetch_assoc($qData))
    {
		...
		if(!isset($resData[$rData['kodebarang']]['tokel'])) $resData[$rData['kodebarang']]['tokel']=0;
...
//    $tab.="<td rowspan=4 bgcolor='AAAAFF' align=right>".number_format($ar[sallu])."</td>";
    ...
    for ($i=1; $i<=$bulan; $i++)
    {
		...
		setIt($ar[$harga],0);
		setIt($ar[$terima],0);
        $tampilan=(empty($ar[$terima])? 0: $ar[$terima]); // penerimaan bulan ini
...
//        $tab.="<td bgcolor='AAFFAA' align=right style='cursor:pointer;' onclick=getDetailGudang('GR','".$ar['kobar']."','".$per."',event) title='Klik untuk melihat detail.'>".number_format($tampilan)."</td>"; 
		...
		if(!isset($ar['totalharga'])) $ar['totalharga']=0;
		if(!isset($ar['totalbarang'])) $ar['totalbarang']=0;
		if(!isset($ar['totalnilai'])) $ar['totalnilai']=0;
		$ar['totalharga']+= (empty($ar[$terima])? 0: $ar[$terima]) * (empty($ar[$harga])? 0: $ar[$harga]);
        $ar['totalbarang']+=(empty($ar[$terima])? 0: $ar[$terima]);
        $ar['totalnilai']+=(empty($ar[$terima])? 0: $ar[$terima]) * $ar[$harga];
    }
	setIt($ar['totem'],0);
...
//    $tab.="<td bgcolor='AAFFAA' align=right style='cursor:pointer;' onclick=getDetailGudang('GR','".$ar['kobar']."','".$tahun."',event) title='Klik untuk melihat detail.'><b>".number_format($tampilan)."</b></td>"; 
    ...
    for ($i=1; $i<=$bulan; $i++)
    {
		...
		setIt($ar[$kasih],0);
		setIt($ar[$harga],0);
        $tampilan=(empty($ar[$kasih])? 0: $ar[$kasih]); // pengeluaran bulan ini
        ...
        $ar['totalharga']+=(empty($ar[$kasih])? 0: $ar[$kasih]) *$ar[$harga];
        $ar['totalbarang']+=(empty($ar[$kasih])? 0: $ar[$kasih]);
        $ar['totalnilai']+=(empty($ar[$kasih])? 0: $ar[$kasih]) *$ar[$harga];
    }
	setIt($ar['tokel'],0);
