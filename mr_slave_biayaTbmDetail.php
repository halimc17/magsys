...
$_POST['intiplasma1']==''?$intiplasma=$_GET['intiplasma1']:$intiplasma=$_POST['intiplasma1'];
...
if($intiplasma!=''){
    $addInti=" and intiplasma='".$intiplasma."'";
}
...
// preview excel pdf    
    ...
    #cek apakah memiliki kebun atau tidak 
    $str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi 
        where induk='".$pt."' and tipe='KEBUN'";
    $query=mysql_query($str) or die(mysql_error($conn));
    $rowcek=mysql_num_rows($query);
    if($rowcek==0){
        exit('warning : PT yang dipilih tidak memiliki kebun');
    }
    // luas blok per tahuntanam
    $str="SELECT tahuntanam, sum(luasareaproduktif) as luas FROM ".$dbname.".setup_blok
        WHERE substr(kodeorg,1,4) ".$kodeorg." ".$addInti." group by tahuntanam order by tahuntanam"; //statusblok in('TBM','TB') and 
    $tt[0]=0;
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    ...
    $str="SELECT tahuntanam, substr(noakun,1,5) as noakun, sum(debet) as debet, sum(kredit) as kredit, periode FROM ".$dbname.".keu_jurnalsum_blok_vw
          WHERE ((substr(kodeorg,1,4) ".$kodeorg.") or (substr(kodeblok,1,4) ".$kodeorg.")) and periode <= '".$periode."' and noakun like '126%'  ".$addInti."
          group by tahuntanam, substr(noakun,1,5)";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {
...
//        $adatt[$tatanam]=$tatanam; 
    ...
    $str="SELECT tahuntanam, noakun, namaakun, sum(debet) as debet, sum(kredit) as kredit, periode FROM ".$dbname.".keu_jurnalsum_blok_vw
          WHERE substr(kodeorg,1,4) ".$kodeorg." and periode <= '".$periode."' and length(noakun)=7 and noakun like '126%'  ".$addInti."
          group by tahuntanam, noakun";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {
//        $ttoke=false;
