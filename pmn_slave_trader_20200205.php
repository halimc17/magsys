...
switch($method){
	...
	case'loadData':
		...
		$str="select a.*,b.namabarang,c.namacustomer,if(isnull(d.jmlkirim),0,d.jmlkirim) as jmlkirim,if(a.qtykontrak-d.jmlkirim=0,1,0) as stsselisih from ".$dbname.".pmn_traderht a 
				...
				where true ".$whrd."
				order by stsselisih,a.kodeorg,a.tanggalext,a.nokontrakext
				limit ".$offset.",".$limit." ";
        //exit('Warning : '.$str);
		$no=$maxdisplay;
		$res=mysql_query($str);
...
//		break;
	case'updateData':
		...
		,matauang='IDR',kurs='1',hargaext='$hrgExt',nilaikontrakext='$nilaiExt',nilaippnext='$ppnExt',catatan='$catatan' where nokontrakext='".$nokontrakext."'";
		if(!mysql_query($upd)){
