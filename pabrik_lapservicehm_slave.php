<?php
	...
	while($bar=mysql_fetch_object($res)){
		if($bar->jamganti1!=0){
			$where="";
			if($hmtgl1[$bar->kodemesin]!='' and !is_null($hmtgl1[$bar->kodemesin])){
				$where=" and a.tanggal>='".$hmtgl1[$bar->kodemesin]."'";
			}
			$str2="select a.kodemesin,sum(a.jam) as hmakhir from ".$dbname.".pabrik_hm a 
					where a.kodemesin='".$bar->kodemesin."' ".$where."
			...
			if($hmakhir[$bar->kodemesin]>=$bar->jamganti1*0.80){
		...
		if($bar->jamganti2!=0){
			$where="";
			if($hmtgl2[$bar->kodemesin]!='' and !is_null($hmtgl2[$bar->kodemesin])){
				$where=" and a.tanggal>='".$hmtgl2[$bar->kodemesin]."'";
			}
			$str2="select a.kodemesin,sum(a.jam) as hmakhir from ".$dbname.".pabrik_hm a 
					where a.kodemesin='".$bar->kodemesin."' ".$where."
					ORDER BY a.kodemesin,a.tanggal desc limit 1";
			...
			if($hmakhir[$bar->kodemesin]>=$bar->jamganti2*0.85){
				$stream.="<tr clas=rowcontent title='Click untuk melihat detail..!' align=left style=\"cursor: pointer\" onclick=showpopup('".substr($bar->kodemesin,0,4)."','".substr($bar->kodemesin,0,6)."','".$bar->kodemesin."','',event)>";
				$stream.="<td align=center>".substr($bar->kodemesin,0,6)."</td>";
				$stream.="<td>".$bar->namastasiun."</td>";
				$stream.="<td align=center>".$bar->kodemesin."</td>";
		...
		if($bar->jamganti3!=0){
			$where="";
			if($hmtgl3[$bar->kodemesin]!='' and !is_null($hmtgl3[$bar->kodemesin])){
				$where=" and a.tanggal>='".$hmtgl3[$bar->kodemesin]."'";
			}
			$str2="select a.kodemesin,sum(a.jam) as hmakhir from ".$dbname.".pabrik_hm a 
					where a.kodemesin='".$bar->kodemesin."' ".$where."
					ORDER BY a.kodemesin,a.tanggal desc limit 1";
			...
			if($hmakhir[$bar->kodemesin]>=$bar->jamganti3*0.85){
				$stream.="<tr clas=rowcontent title='Click untuk melihat detail..!' align=left style=\"cursor: pointer\" onclick=showpopup('".substr($bar->kodemesin,0,4)."','".substr($bar->kodemesin,0,6)."','".$bar->kodemesin."','',event)>";
				$stream.="<td align=center>".substr($bar->kodemesin,0,6)."</td>";
				$stream.="<td>".$bar->namastasiun."</td>";
				$stream.="<td align=center>".$bar->kodemesin."</td>";
