...
if($tanggal==''){
	...
	if($proses=='preview' or $proses=='excel'){
		...
		$sql="select if(substr(a.nospb,9,6)='' or substr(a.nospb,9,6)='0','zzzzzz',substr(a.nospb,9,6)) as unit
				...
				,sum(z.A)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as apersen,sum(z.B)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as bpersen,sum(z.C)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as cpersen
				,sum(z.D)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as dpersen,sum(z.E)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as epersen,(1-((sum(z.A)+sum(z.B)+sum(z.C)+sum(z.D)+sum(z.E))/if(z.kodeorg='',sum(z.jumlahtandan1),100)))*100 as fpersen
				,sum(z.F)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as gpersen,sum(z.G)/if(z.kodeorg='',sum(z.jumlahtandan1),100)*100 as hpersen
				from (select x.notransaksi,x.nospb,x.kodeorg,x.kodecustomer,x.beratbersih,x.millcode,x.tanggal,x.kodebarang,x.jumlahtandan1,w.A,w.B,w.C,w.D,w.E,w.F,w.G 
		...
		if($row>0){
			while($res=mysql_fetch_assoc($query)){
				...
				$totalgradE += $res['e'];
				$totalgradF += $res['f'];
				$totalsample += $res['a']+$res['b']+$res['c']+$res['d']+$res['e']+$res['f'];
				$totalgradG += $res['g'];
				$totalgradH += $res['h'];
				$gtotalgradA += $res['a'];
				$gtotalgradB += $res['b'];
				...
				$gtotalgradE += $res['e'];
				$gtotalgradF += $res['f'];
				$gtotalsample += $res['a']+$res['b']+$res['c']+$res['d']+$res['e']+$res['f'];
				$gtotalgradG += $res['g'];
				$gtotalgradH += $res['h'];
		...
		$tab2[$no2].="
			...
			<td colspan=2 style='text-align:right;'>".number_format($jamstagnasisd/$jamolahsd*100,2)."</td>
			<td colspan=2 style='background-color:DarkGrey;'></td>";
		$no2+=1;
		$tab2[$no2].="
