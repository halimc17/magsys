...
if($_SESSION['empl']['tipelokasitugas'] == 'HOLDING' and ($_SESSION['empl']['bagian'] == 'HHRD' || $_SESSION['empl']['bagian'] == 'HHRS')){
	...
	$sKary="select distinct karyawanid,namakaryawan from ".$dbname.".datakaryawan where tipekaryawan in(0,1,2,6,7,8,9) order by namakaryawan asc";
	...
	$optKary="<option value=''>".$_SESSION['lang']['all']."</option>";
	$optJenis=$optKary;
	$sKary="select distinct karyawanid,namakaryawan from ".$dbname.".datakaryawan where substr(lokasitugas,3,2)!='HO' and tipekaryawan in(0,1,2,3,7,8) and kodeorganisasi = '".$_SESSION['empl']['kodeorganisasi']."' order by namakaryawan asc";
	$qKary=mysql_query($sKary) or die(mysql_error($sKary));
	while($rKary=mysql_fetch_assoc($qKary))
	{
		$optKary.="<option value='".$rKary['karyawanid']."'>".$rKary['namakaryawan']."</option>";
