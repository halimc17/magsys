...
$arrTanggal=makeOption($dbname, 'log_pengiriman_ht', 'nosj,tanggalkirim');
...
{
    exit("Error: ".$_SESSION['lang']['periode']." ".$_SESSION['lang']['dari']." tidak boleh kosong".$periode);
}


if($kdUnit!='')
{
        $where.=" and d.kodeorg='".$kdUnit."'";
        $whereb.=" and d.kodeorg='".$kdUnit."'";
...
$sListData="select distinct a.kodebarang,a.notransaksi as nolpbjkt,b.tanggal as tgllpbJkt,a.satuan,a.jumlah as jmlhlpbjkt,c.nosj,b.nopo 
            from ".$dbname.".log_lpbdt a
            left join ".$dbname.".log_lpbht b on a.notransaksi=b.notransaksi
            left join ".$dbname.".log_pengiriman_dt c on b.notransaksi=c.notransaksi
            left join ".$dbname.".log_poht d on b.nopo=d.nopo
            where ".$where." order by a.kodebarang,b.tanggal asc ";
//$qListData=mysql_query($sListData) or die(mysql_error());
//while($rData=mysql_fetch_assoc($qListData))
//{
//    $dtPo[$rData['nopo']]=$rData['nopo'];
//    $dtPenrmaan[$rData['nopo'].$rData['nolpbjkt']]=$rData['nolpbjkt'];
//    $tglPenrmaan[$rData['nopo'].$rData['nolpbjkt']]=$rData['tgllpbJkt'];
//}
//exit("Error:".$sListData);
	...
	<tr>
        ...
        <td ".$bgcoloraja." colspan=3 align=center>Head Office</td>
        <td ".$bgcoloraja." colspan=2 align=center>Delivery Document</td>
...
while($rListData=mysql_fetch_assoc($qListData))
{
    ...
    $tglEdit='';
                ...
                $tab.="<td>".$rListData['satuan']."</td>";
                $tab.="<td>".$rListData['nolpbjkt']."</td>";
                $tab.="<td>".$rListData['tgllpbJkt']."</td>";
                $tab.="<td align=right>".$rListData['jmlhlpbjkt']."</td>";
                $tab.="<td>".$rListData['nosj']."</td>";
                $tab.="<td>".$arrTanggal[$rListData['nosj']]."</td>";
                $sTgl="select distinct tanggal,jumlah,notransaksi from ".$dbname.".log_transaksi_vw 
                          where nopo='".$rListData['nopo']."' and kodebarang='".$rListData['kodebarang']."'";
                $qTgl=mysql_query($sTgl) or die(mysql_error($conn));
                $rTgl=mysql_fetch_assoc($qTgl);
                $tab.="<td>".$rTgl['notransaksi']."</td>";
                $tab.="<td align=right>".$rTgl['tanggal']."</td>";
                $tab.="<td align=right>".number_format($rTgl['jumlah'],0)."</td>";
                $tab.="</tr>";
   }
}
else
