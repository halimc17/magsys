...
// remove AK-%
$sremove="select * from ".$dbname.".log_transaksidt
    where notransaksi in ".$notrantu." ";
$qremove=mysql_query($sremove) or die(mysql_error($conn));
while($rremove=mysql_fetch_assoc($qremove)){    
    $key=$rremove['notransaksi'].$rremove['kodebarang'];
    
    if(substr($rremove['kodeblok'],0,2)=='AK'){
        unset($data[$key]);
        unset($datakey[$key]);
    }
}
