	...
	{ 	
		...
		if($tipe=='STENGINE' or $tipe=='STATION' or $tipe=='MAINTENANCE')
		{
			...
			$strf="select kodekegiatan,kelompok,namakegiatan from ".$dbname.".setup_kegiatan 
			       where kelompok='MIL' and status = '1' order by kelompok,namakegiatan";
		...
		else if($tipe=='BLOK')
		{
			...
			$optKegiatan="<option value=''></option>";
			$strf="select kodekegiatan,kelompok,namakegiatan from ".$dbname.".setup_kegiatan 
			       where kelompok='WSH' or kelompok='KNT' and status = '1' order by kelompok,namakegiatan";	   
			$resf=mysql_query($strf);
			while($barf=mysql_fetch_object($resf))
			{
				 $optKegiatan.="<option value='".$barf->kodekegiatan."'>[".$barf->kelompok."][".$barf->kodekegiatan."]-".$barf->namakegiatan."</option>";
			...
			$optKegiatan="<option value=''></option>";
			$strf="select kodekegiatan,kelompok,namakegiatan from ".$dbname.".setup_kegiatan 
			       where kelompok='SPL' or kelompok='KNT' and status = '1' order by kelompok,namakegiatan";	   
			$resf=mysql_query($strf);
			while($barf=mysql_fetch_object($resf))
			{
				 $optKegiatan.="<option value='".$barf->kodekegiatan."'>[".$barf->kelompok."][".$barf->kodekegiatan."]-".$barf->namakegiatan."</option>";
			...
			$optKegiatan="<option value=''></option>";
			$strf="select kodekegiatan,kelompok,namakegiatan from ".$dbname.".setup_kegiatan 
			       where  kelompok in ('BBT','MN','PN','KNT') and status = '1' order by kelompok,namakegiatan";	   
			$resf=mysql_query($strf);
			while($barf=mysql_fetch_object($resf))
			{
				 $optKegiatan.="<option value='".$barf->kodekegiatan."'>[".$barf->kelompok."][".$barf->kodekegiatan."]-".$barf->namakegiatan."</option>";
			...
			echo $optKegiatan;			
               ...
               else if(substr($blok,0,2)=='AK' or substr($blok,0,2)=='PB')
               {
                ...
                else
                {
                        $optKegiatan="<option value=''></option>";
                        $strf="select kodekegiatan,kelompok,namakegiatan from ".$dbname.".setup_kegiatan 
                               where kelompok='KNT' and status = '1' order by kelompok,namakegiatan";
                        $resf=mysql_query($strf);
                        while($barf=mysql_fetch_object($resf))
                        {
                                 $optKegiatan.="<option value='".$barf->kodekegiatan."'>[".$barf->kelompok."][".$barf->kodekegiatan."]-".$barf->namakegiatan."</option>";
