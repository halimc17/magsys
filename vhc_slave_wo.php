...
//            exit("Error:".$i);
	...
	case 'getBA':
		$param = $_POST;
		$optBA="<option value=''></option>";
		$where = "tanggal = '".tanggalsystem($param['tanggal']).
			"' and kodetraksi = '".$param['kodetraksi'].
			"' and kodealat = '".$param['kodealat'].
			"' and operator = '".$param['operator']."'";
		$sGet=selectQuery($dbname,'vhc_balaka','notransaksi',$where);
		$qGet=mysql_query($sGet) or die(mysql_error());
		$optOperator="<option value=''></option>";
		while($rGet=mysql_fetch_assoc($qGet)){
			$optOperator.="<option value=".$rGet['notransaksi'];
			if(!empty($param['balaka']) and $param['balaka']==$rGet['notransaksi']) $optOperator.= " selected ";
			$optOperator.=">".$rGet['notransaksi']."</option>";
		}
		echo $optOperator;
	break;
	...
	case'printPdf':
            ...
            $pdf->SetY(22);
			$pdf->Cell(180,10,$_SESSION['org']['namaorganisasi'],0,1,'C');
            ...
            $pdf->Cell(45,10,isset($optKarRow[$res['divmanager']])? $optKarRow[$res['divmanager']]: '','',0,'C');
