...
switch($proses)
{
	...
	<td>".$_SESSION['lang']['kg']." ".$_SESSION['lang']['kebun']."</td>
	...
	if($row>0)
	{
		...
		echo"<tr class=rowcontent style='font-weight:bold'>
				...
				<td style='text-align:right'>".number_format($grandTotal['totaLkg'],2)."</td>
			</tr>";
	}
	else
	{
...
*/				$this->SetFont('Arial','',7);
		...
		if($row>0)
		{
			...
			while($rDet=mysql_fetch_assoc($qDet))
			{
				...
				$qSpbDet=mysql_query($sSpbDet) or die(mysql_error());
				$rSpbDet=mysql_fetch_assoc($qSpbDet);
							$srow="select blok from ".$dbname.".kebun_spbdt where nospb='".$rDet['nospb']."'";
							$qrow=mysql_query($srow) or die(mysql_error($conn));
							$rRow=mysql_num_rows($qrow);
							@$bjrR=$rSpbDet['Bjr']/$rRow;
				$arrPost=array($_SESSION['lang']['belumposting'],$_SESSION['lang']['posting']);
	...
	$rOrg=mysql_fetch_assoc($qOrg);
			...
			$resx=mysql_query($strx);
				...
				$qSpbDet=mysql_query($sSpbDet) or die(mysql_error());
				$rSpbDet=mysql_fetch_assoc($qSpbDet);
							$srow="select blok from ".$dbname.".kebun_spbdt where nospb='".$barx['nospb']."'";
							$qrow=mysql_query($srow) or die(mysql_error($conn));
							$rRow=mysql_num_rows($qrow);
							@$bjrR=$rSpbDet['Bjr']/$rRow;
				$arrPost=array($_SESSION['lang']['belumposting'],$_SESSION['lang']['posting']);
