...
<?
	...
	if($tanggal1!='' and $tanggal2!='' ){
		...
		$periodesd="<br>Periode : ".tanggalnormal($tanggal1)." s/d ".tanggalnormal($tanggal2);
	}
	if($pekerjaanx!=''){
	...
	$strz="select a.*,b.nik,b.namakaryawan,c.namakaryawan as namaatasan,datediff(entrydate,tanggal) as selisih from ".$dbname.".sdm_pekerjaanharian a
	...
	$stream2.="
		<thead>
			<tr class=rowcontent align=center bgcolor='#00FF00'>
	...
	if($proses!=''){
		if($type!='excel'){
			$stream2.=" <td ".$bgclr." width='30px'>".$_SESSION['lang']['hari']."</td>";
		}
	}
	$stream2.="</tr>
		</thead>
	...
	if($rowz==0){
		...
		while($barz=mysql_fetch_object($resz)){
			...
			if($proses!=''){
				if($type!='excel'){
					$stream2.="<td align=right>".$barz->selisih."</td>";
				}
			}
			$stream2.="</tr>";
		}
