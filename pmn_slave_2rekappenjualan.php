...
$idCustomer=checkPostGet('idCustomer','');
...
switch($proses)
{
	case'preview':
		...
		echo" <table class=sortable cellspacing=1 border=0 style='width:1200px;'>
			...
			if(!empty($idCustomer)) {
				$sql .= " and a.koderekanan = '".$idCustomer."'";
			}
			...
			if($row>0)
			{
				while($res=mysql_fetch_assoc($query))
				{
					...
					//$selisihtgl=tanggalnormal($res['tglakhir'])->diff(tanggalnormal($res['sdtanggal']));
					$selisihtgl=$res['selisihtgl'];
					$biayaperkg=$res['kuantitaskontrak']==0 ? 0 : $res['nilaiinvoice']/$res['kuantitaskontrak'];
					...
					$totalIncKg[]			= $hHargaSatuan==0 ? 0 : $res['nilaiinvoice']/$hHargaSatuan;
					...
					$gtotalIncKg			+= $hHargaSatuan==0 ? 0 : $res['nilaiinvoice']/$hHargaSatuan;
