...
//=================ambil master barang untuk aset kendaraan (905)
  ...
  $str="select kodebarang,namabarang from ".$dbname.".log_5masterbarang where kelompokbarang='903' or kelompokbarang='904' order by namabarang";
  $res=mysql_query($str);
  $optbarang='';
  while($bar=mysql_fetch_object($res))
  {
...
//$optAsset = makeOption($dbname,'sdm_daftarasset','kodeasset,namaasset');
$optAsset = array(''=>'');
...
echo"<fieldset><table>
    ...
    <tr><td>".$_SESSION['lang']['kodeasset']."</td>
		<td>".makeElement('kodeasset','select','',array(),$optAsset)."</td>
		...
		echo"<tr ".$bgcrcolor.">
			 ...
			 <td>
			     <img src=images/application/application_edit.png class=resicon  caption='Edit' onclick=\"fillMasterField('".$bar1->kodeorg."','".$bar1->kelompokvhc."','".$bar1->jenisvhc."','".$bar1->kodevhc."','".$bar1->beratkosong."',
                                 ...
                                 '".tanggalnormal($bar1->tglakhirkir)."','".tanggalnormal($bar1->tglakhirijinbm)."','".tanggalnormal($bar1->tglakhirijinang)."','".$bar1->kodeasset."','".$bar1->detailvhc."');\">
