...
$where =  isset($_GET['cond'])? $_GET['cond']: '';
...
class masterpdf extends FPDF {
    function Header() {
        ...
        $this->Cell($width,$height+5,$_SESSION['org']['namaorganisasi'],'',1,'R');
...
#		$this->Cell($width,$height,'Tabel : '.$table,'',1,'L');
        ...
        $this->Cell(260,$height,$_SESSION['lang']['tanggal'],'',0,'R');
        $this->Cell(5,$height,':','',0,'L');
        $this->Cell(40,$height,date('d-m-Y H:i'),'',1,'L');
        $this->Cell(420,$height,' ','',0,'R');
        $this->Cell(260,$height,$_SESSION['lang']['page'],'',0,'R');
        $this->Cell(8,$height,':','',0,'L');
        $this->Cell(15,$height,$this->PageNo(),'',1,'L');
#        $this->Ln();
        $this->Cell(420,$height,' ','',0,'R');
        $this->Cell(260,$height,'User','',0,'R');
        $this->Cell(8,$height,':','',0,'L');
...
#        }
        ...
        $this->Cell(500,1.5*$height,$_SESSION['lang']['namaperkiraan'],'TBLR',0,'C');
...
foreach($result as $data) {
        ...
        if($_SESSION['language']=='EN'){
            $pdf->Cell(500,$height,$data['namaakun1'],'',0,'L');
        }else{
            $pdf->Cell(500,$height,$data['namaakun'],'',0,'L');
        }
