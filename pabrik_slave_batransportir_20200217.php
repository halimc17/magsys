...
switch($method)
{
    case'loadData':
		...
		$str="select a.kodeorg,a.notransaksi,a.tanggal,a.koderekanan,d.namasupplier,d.kodetimbangan,e.nokontrak,a.keterangan
			...
			where b.kodekegiatan like '8110102%' and e.nokontrak<>'' and a.kodeorg in (".$kdorg.")
			ORDER BY f.statusjurnal,a.kodeorg,a.tanggal,a.koderekanan desc
			";
		$res=mysql_query($str);
			...
			if($bar->statusjurnal==0){
				echo"
					<td align=center>
						...
						,'".$bar->keterangan."','".$bar->hasilkerjarealisasi."','".$bar->jumlahrealisasi."','".$bar->nilaippn."')\">&nbsp
	...
	case'getSPP':
		...
		$iSPP="select m.*,n.tanggaldo from (select distinct(a.nosipb) as nosipb,if(b.statusjurnal=1,1,0) as stsjrn from ".$dbname.".pabrik_timbangan a 
								LEFT JOIN (select x.*,y.statusjurnal from ".$dbname.".log_spkht x
											...
											where x.keterangan<>'' and koderekanan='".$trpcode."') b on b.keterangan=a.nosipb
								where a.kodecustomer='".$kodeCust."' and a.intex='0') m
	...
	case'getNo':
		...
		$iSPK="select max(left(notransaksi,3)) as maxno from ".$dbname.".log_spkht where notransaksi like '%/".$rmw2[$bln]."/".$thn."'";
