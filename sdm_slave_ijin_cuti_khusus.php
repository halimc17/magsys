        ...
        switch($proses)
        {
                ...
                case'jumlahhari':
					...
					*/
				    ...
				    $tglAwal=tanggalsystem($param['tglAwal']);
					if($param['jnsIjin']=='MELAHIRKAN'){
						$tglEnd=date('Y-m-d',strtotime('+89 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='MENIKAH'){
						$tglEnd=date('Y-m-d',strtotime('+2 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='ANAKNIKAH'){
						$tglEnd=date('Y-m-d',strtotime('+1 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='ISTRILAHIR'){
						$tglEnd=date('Y-m-d',strtotime('+1 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='KLGUTMMATI'){
						$tglEnd=date('Y-m-d',strtotime('+1 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='1RMH_MATI'){
						$tglEnd=date('Y-m-d',strtotime('+0 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='ANAKHITAN'){
						$tglEnd=date('Y-m-d',strtotime('+1 days',strtotime(substr($tglAwal,0,10))));
					}else if($param['jnsIjin']=='BAPTIS'){
						$tglEnd=date('Y-m-d',strtotime('+1 days',strtotime(substr($tglAwal,0,10))));
					}else{
						$tglEnd = $tglAwal;
					}
					...
					$ke=0;
					while ($jalan){
						//$tglEnd=date('Y-m-d',strtotime("+".$ke." days",strtotime(substr($tglEnd,0,10))));
						$ke+=1;
						if($_SESSION['empl']['tipelokasitugas']=='HOLDING'){
						   $strlibur="select * from ".$dbname.".sdm_5harilibur 
									  where kebun in ('HOLDING','GLOBAL') and keterangan='libur' and tanggal='$tglEnd'";
						}else{
						   $strlibur="select * from ".$dbname.".sdm_5harilibur 
									  where kebun in ('".$_SESSION['empl']['lokasitugas']."','GLOBAL') and keterangan='libur' and tanggal='$tglEnd'";
						}
						//exit('Warning: '.$strlibur);
						$reslibur=mysql_query($strlibur);
						$rowlibur=mysql_num_rows($reslibur);
						if($rowlibur==0){
							$jalan=false;
						}else{
							$tglEnd=date('Y-m-d',strtotime("+1 days",strtotime(substr($tglEnd,0,10))));
						}
					}
					echo tanggalnormal($tglEnd);
					break;
                         
                case'insert':
                  //  exit("Error:masuk");
                if(($tglijin=='')||($jnsIjin=='')||($jamDr1=='')||($jamSmp1=='')||($keperluan=='')||($atasan=='')||($hrd==''))
                {
                ...
                case'update':
                    ...
                    //=============================		
                if(($jnsIjin=='')||($jamDr=='')||($jamSmp=='')||($keperluan=='')||($atasan=='')||($hrd==''))
                {
                        echo"warning:Please Complete The Form";
                        exit();
                }
