...
switch($proses){
	case'loadData':
		...
		$strb="select a.kodeorg,a.tanggal,a.jenis from ".$dbname.".kebun_adjbrondol a 
				where ".$where." 
		...
		$offset=$page*$limit;
		//$str="select a.*,c.namasupplier as supplier,d.namakaryawan as karyawan from ".$dbname.".kebun_adjbrondol a 
		...
		$str="select a.*,b.namaorganisasi from ".$dbname.".kebun_adjbrondol a 
		...
		while($bar=mysql_fetch_object($res)){
			...
			if(tanggalsystem(tanggalnormal($bar->tanggal))>=$_SESSION['org']['period']['start']){
				echo"	<img src='images/skyblue/edit.png' class='resicon' title='Edit'	
						onclick=\"fillfield('".$bar->kodeorg."','".tanggalnormal($bar->tanggal)."','".$bar->jenis."','".substr($bar->waktu,0,5)."','".$bar->kg."','".$bar->keterangan."','')\">&nbsp
	...
	case'delData':
		$strx="delete from ".$dbname.".kebun_adjbrondol 
	...
	case'saveData':
		if($addedit=='update'){
			if($kodeorglama!=$kodeorg and $tanggallama!=$tanggal and $jenislama!=$jenis){
				$strs="select * from ".$dbname.".kebun_adjbrondol
						where (kodeorg='".$kodeorg."' and tanggal='".$tanggal."' and jenis='".$jenis."')";
			...
			$strx="update ".$dbname.".kebun_adjbrondol set kodeorg='".$kodeorg."',tanggal='".$tanggal."',jenis='".$jenis."',waktu='".$waktu."'
				,kg='".$kg."',keterangan='".$keterangan."'
				,lastuser='".$_SESSION['standard']['username']."',lastdate=now()
				where kodeorg='".$kodeorglama."' and tanggal='".$tanggallama."' and jenis='".$jenislama."'";
		}else{
			$strx="insert into ".$dbname.".kebun_adjbrondol
				(tanggal,kodeorg,jenis,waktu,kg,keterangan,lastuser,lastdate)
				values('".$tanggal."','".$kodeorg."','".$jenis."','".$waktu."','".$kg."','".$keterangan."'
	...
	case'getCatatan':
		...
		$sData="select catatan from ".$dbname.".kebun_adjbrondol where true ".$whereidx."";
		//exit('Warning: '.$sData);
