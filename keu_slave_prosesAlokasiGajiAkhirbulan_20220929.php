...
function prosesGajiAfdeling(){
  ...
  #pastikan jenis pekerjaan untuk karyawan bersangkutan apakah PNN TM TBM atau BBT
    ...
    $str="select distinct a.tipetransaksi,b.kodeorg,b.kodesegment from ".$dbname.".kebun_aktifitas a left join " 
...
function prosesGajiPabrik(){ 
  ...
  if($param['tipeorganisasi']=='MAINTENANCE'){
		if($param['komponen']==33 or $param['komponen']==32 or $param['komponen']==40 or $param['komponen']==58)#premi dan lembur
		  $group='PKS06';
		else if($param['komponen']==45 or $param['komponen']==56 or $param['komponen']==57) #tunjangan transport dan makan
		  $group='PKS07';
...
# PKS04	Pengobatan
	...
	if($param['komponen']==33){
	    $sAkum="SELECT beban,noakun,SUM(uangmakan+uangtransport+uangkelebihanjam) as uanglembur 
				from ".$dbname.".sdm_lemburdt where karyawanid='".$param['karyawanid']."' and tanggal like '".$param['periode']."%'
				GROUP BY beban,noakun
				ORDER BY beban,noakun";
 ...
 */
            ...
            if($headErr=='') {
                ...
                foreach($dataRes['detail'] as $row) {
                    $insDet = insertQuery($dbname,'keu_jurnaldt',$row);
                    if(!mysql_query($insDet)) {
                        $detailErr .= "Insert Detail Error 1 : ".mysql_error()."\n";
                        break;
                    }
                }
