...
if(trim($_SESSION['empl']['tipelokasitugas'])=='HOLDING'){
   $sOrg="select karyawanid, namakaryawan from ".$dbname.".datakaryawan where (tanggalkeluar = '0000-00-00' or tanggalkeluar > '".date("Y-m-d")."') and tipekaryawan in ('0','2','6','7','8','9') and kodegolongan in ('DIR','KOM','MGR','SR MGR','ASST MGR') and lokasitugas like '%HO' and karyawanid!='".$_SESSION['standard']['userid']."' order by namakaryawan asc";
}else{
   $sOrg="select karyawanid, namakaryawan from ".$dbname.".datakaryawan where (tanggalkeluar = '0000-00-00' or tanggalkeluar > '".date("Y-m-d")."') and tipekaryawan in ('0','2','6','7','8','9') and kodegolongan in ('DIR','KOM','MGR','SR MGR','ASST MGR') and lokasitugas not like '%HO' and kodeorganisasi='".$_SESSION['empl']['kodeorganisasi']."' and karyawanid!='".$_SESSION['standard']['userid']."' order by namakaryawan asc";
}
...
if(trim($_SESSION['empl']['tipelokasitugas'])=='HOLDING'){
   ...
   $sHRD="select karyawanid, namakaryawan from ".$dbname.".datakaryawan where (tanggalkeluar = '0000-00-00' or tanggalkeluar > '".date("Y-m-d")."') and tipekaryawan in ('0','2','6','7','8','9') and bagian in ('HRA','HHRS') and lokasitugas not like '%HO' and kodeorganisasi='".$_SESSION['empl']['kodeorganisasi']."' and karyawanid!='".$_SESSION['standard']['userid']."' order by namakaryawan asc";
}
...
$optJenis="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
                ...
                foreach($arragama as $kei=>$fal)
                {
                    if($_SESSION['language']=='ID'){
                       //if($kei=='CUTI' or $kei=='MELAHIRKAN' or $kei=='KAWIN/SUNATAN/WISUDA'){
...
<td>:</td>
<td><input type='text' class='myinputtext' id='tglIzin' onmousemove='setCalendar(this.id)' onkeypress='return false;' value="<?php echo tanggalnormal(date('Y-m-d'));?>" size='10' maxlength='10' style="width:150px;" /></td>
</tr>
<tr>
<td><?php echo $_SESSION['lang']['jenisijin']?></td>
<td>:</td>
<td><select id="jnsIjin" name="jnsIjin" onchange='jumlahijin()' style="width:150px"><?php echo $optJenis;?></select></td>
</tr>
...
<td>:</td>
<td><input type='text' class='myinputtext' id='tglAwal' onmousemove='setCalendar(this.id)' onchange='jumlahhari()' onkeypress='return false;'  size='10' maxlength='10' style="width:150px;" /><select id="jam1"><?php echo $jm;?></select>:<select id="mnt1"><?php echo $mnt;?></select></td>
</tr>
<tr>
<td><?php echo $_SESSION['lang']['tglcutisampai']."  ".$_SESSION['lang']['tanggal']."&".$_SESSION['lang']['jam']?></td>
<td>:</td>
<td><input type='text' class='myinputtext' id='tglEnd' onmousemove='setCalendar(this.id)' onchange='jumlahhari()' onkeypress='return false;'  size='10' maxlength='10' style="width:150px;" /><select id="jam2"><?php echo $jm;?></select>:<select id="mnt2"><?php echo $mnt;?></select></td>
</tr>
<tr>
<td><?php echo $_SESSION['lang']['jumlahhk']." ".$_SESSION['lang']['diambil'];?></td>
<td>:</td>
<td>
<input type="text" class="myinputtext" id="jumlahhk" name="keperluan" disabled onkeypress="return angka_doang(event);" maxlength="5" value="0" /><?php echo $_SESSION['lang']['hari']; ?> -
(<?php echo $_SESSION['lang']['sisa']; ?> : <span id="sis"><?php echo $sisa; ?></span> <?php echo " ".$_SESSION['lang']['hari'].")"; ?> </td>
</tr>
<tr>
