...
switch($proses){
	case'loadData':
		...
		$strtb="select a.* from ".$dbname.".pabrik_verifikasi_ampere a where a.kodemesin like '".$mesin."%' and a.tipeservice<>0 and a.tanggal<'".$periodetb."-01'
		...
		*/
		...
		//		order by a.kodemesin,a.tanggal,a.tipeservice";
		$str="select a.*,b.namaorganisasi,d.ampere from ".$dbname.".pabrik_verifikasi_ampere a 
				left join ".$dbname.".organisasi b on b.kodeorganisasi=a.kodemesin
				left join ".$dbname.".pabrik_machinery d on d.kodemesin=a.kodemesin and namasubmesin='Elektromotor'
				order by a.kodemesin,a.tanggal desc,a.tipeservice";
		//exit('Warning: '.$str);
		$res=mysql_query($str);
		$awalmesin='';
		while($bar=mysql_fetch_object($res)){
			...
			echo"<tr class=rowcontent >
					...
					<td ".$drcl." align=left width='7%'>".$bar->ampere."</td>
					...
					<td align=center width='7%'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodeorg."','".$bar->kodemesin."','".tanggalnormal($bar->tanggal)."','".$bar->tipeservice."','".round($bar->ukur1,2)."','".round($bar->ukur2,2)."','".round($bar->ukur3,2)."','".$bar->keterangan."')\">&nbsp
	...
	case'saveData':
		...
		while($bara=mysql_fetch_object($resa)){
			if($tipesrvlama=='0' and ($ukur1>$bara->ukur1 or $ukur2>$bara->ukur2 or $ukur3>$bara->ukur3)){
		...
		*/
		if($addedit=='update'){
			...
			$strx="update ".$dbname.".pabrik_verifikasi_ampere set kodeorg='".$kodeorg."',kodemesin='".$mesin."',tanggal='".$tanggal."',ukur1=".round($ukur1,2)."
					,ukur2=".round($ukur2,2).",ukur3=".round($ukur3,2).",tipeservice='".$tipeservice."',keterangan='".$keterangan."'
					,lastuser='".$_SESSION['standard']['username']."',lastdate=now()
					where kodeorg='".$kodeorg."' and kodemesin='".$mesin."' and tanggal='".$tgllama."' and tipeservice='".$tipesrvlama."' 
						and keterangan='".$ketlama."'";
		}else{
			$strx="insert into ".$dbname.".pabrik_verifikasi_ampere
					(kodeorg,kodemesin,tanggal,ukur1,ukur2,ukur3,tipeservice,keterangan,lastuser,lastdate)
