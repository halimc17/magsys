<?php
	...
	$sBlok="select tanggal,noakun,kodeblok,kodebarang,kodevhc,jumlah from ".$dbname.".keu_jurnaldt
			where LENGTH(kodeblok)=10 and left(noakun,1) in ('6','7','8') and kodeblok like '".$kodeunit."%' and tanggal like '".$periode."%' 
	...
	while($rBlok=mysql_fetch_assoc($qBlok)){
		...
		if(substr($rBlok['noakun'],0,1)=='6' and $rBlok['kodebarang']=='' and $rBlok['kodevhc']==''){
			$upah[$xblok][$xbln]+=$rBlok['jumlah'];
		}else if(substr($rBlok['noakun'],0,1)=='6' and $rBlok['kodebarang']!=''){
			$material[$xblok][$xbln]+=$rBlok['jumlah'];
		}else if(substr($rBlok['noakun'],0,1)=='6' and $rBlok['kodevhc']!=''){
