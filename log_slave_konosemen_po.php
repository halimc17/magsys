...
$q2 = "SELECT nopo,kodebarang,sum(jumlah) as jumlah FROM ".$dbname.
	".`log_rinciankono` where nopo like '%".$_POST['po']."%' group by nopo,kodebarang";
$data2 = fetchData($q2);
$optData = array();
	...
	<tbody id=bodySearch>
		...
		<tr class=rowcontent>
			...
			<td id="jumlahkirim_<?php echo $i?>" align=right>
				<?php echo (isset($optData[$row['nopo']][$row['kodebarang']]))? $optData[$row['nopo']][$row['kodebarang']]: 0?>
			</td>
