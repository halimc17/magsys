<?php
	...
	foreach($kodemesin as $mdid=>$mdval)
    {
		...
		foreach($tgltgl as $key=>$tangval){
			...
			$stream.="<td title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$tangval."','".$kdOrg."','".$stasiun."','',event)>".@number_format($tebalkecil[$mdid][$tangval]/$tebalsisa[$mdid]*100,2)."</td>";
		}
