...
else if($periode=='' and $gudang!='')
{
	...
	if($gudang=='')
	{
		$str="select 
			  ...
			  on a.kodebarang=b.kodebarang
			  where kodeorg='".$pt."' 
			  and periode='".$periode."' and (a.qtymasuk!=0 and  a.qtykeluar!=0 or a.saldoakhirqty!=0)
	...
	else
	{
		$str="select
			  ...
			  where kodeorg='".$pt."' 
			  and periode='".$periode."'
			  and kodegudang='".$gudang."' and (a.qtymasuk!=0 and  a.qtykeluar!=0 or a.saldoakhirqty!=0)
			  group by a.kodebarang order by a.kodebarang";		
	}	
}
//=================================================
...
if($periode=='')
{
	...
	$pdf->AddPage();
		...
		$pdf->Cell(15,4,$keluarQTY,0,0,'R');
                if(substr($kodebarang,0,3)=='312'){
                    $pdf->Cell(15,4,number_format($kuantitas,3,'.',','),0,1,'R');
                }
                else
	...
	while($bar=mysql_fetch_object($res))
	{
		...
		$pdf->Cell(7,4,$bar->satuan,0,0,'L');
                if(substr($kodebarang,0,3)=='312'){
                    $pdf->Cell(18,4,number_format($sawalQTY,3,'.',','),0,0,'R');		
                    $pdf->Cell(15,4,number_format($masukqty,3,'.',','),0,0,'R');
                    $pdf->Cell(15,4,number_format($keluarqty,3,'.','.'),0,0,'R');
                    $pdf->Cell(15,4,number_format($salakqty,3,'.',','),0,1,'R');
                }
                else{
                    $pdf->Cell(18,4,number_format($sawalQTY,2,'.',','),0,0,'R');		
                    $pdf->Cell(15,4,number_format($masukqty,2,'.',','),0,0,'R');
                    $pdf->Cell(15,4,number_format($keluarqty,2,'.','.'),0,0,'R');
