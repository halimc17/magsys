...
<?php
	...
	$optTipe=$optDept;
	$sDept="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi 
			where tipe!='HOLDING' and char_length(kodeorganisasi)='4'  order by namaorganisasi asc";
...
<script>
	...
	function showpopup(kdUnit,periode,tipe,tipekaryawan,idkomp,dtsub,ev){
		param='kdUnit='+kdUnit+'&periode='+periode+'&tipe='+tipe+'&tipekaryawan='+tipekaryawan+'&idkomp='+idkomp+'&dtsub='+dtsub;
		//alert(param);
		tujuan='sdm_slave_2realisasiGaji_showpopup.php'+"?"+param;
		width='800';
		height='400';
		content="<iframe frameborder=0 width=100% height=100% src='"+tujuan+"'></iframe>"
		showDialog1('Daftar Upah '+kdUnit+' '+periode+' - '+tipekaryawan,content,width,height,ev); 
	}
...
<div>
	<fieldset style="float: left;">
		...
		<table cellspacing="1" border="0" >
			...
			<tr><td colspan="2">
					<button onclick="zPreview('sdm_slave_2realisasiGaji','<?php echo $arrKry?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button>
	        <!--<button onclick="zPdf('sdm_slave_2realisasiGaji','<?php echo $arrKry?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button>-->
					<button onclick="zExcel(event,'sdm_slave_2realisasiGaji.php','<?php echo $arrKry?>')" class="mybutton" name="preview" id="preview">Excel</button>
