<?
	...
	$optLuasBlok=makeOption($dbname, 'setup_blok', 'kodeorg,luasareaproduktif');
	...
	while($rBlok=mysql_fetch_assoc($qBlok)){
		...
		}else if((substr($rBlok['noakun'],0,1)=='6' or substr($rBlok['noakun'],0,1)=='1') and $rBlok['kodebarang']!='' and $rBlok['kodebarang']!='0'){
	...
	foreach($dtKodeBlok as $xblok){
		...
		if($no!=1 and substr($xblok,0,6)!=$xdiv){
			...
			$tab.="<td bgcolor=#ffcc99 align=right>".number_format($stLuasBlok,2)."</td>";
			$stLuasBlok=0;
			for ($xbln=1; $xbln<=12; $xbln++){
		...
		$tab.="<td align=right>".number_format($optLuasBlok[$xblok],2)."</td>";
		$stLuasBlok+=$optLuasBlok[$xblok];
		$gtLuasBlok+=$optLuasBlok[$xblok];
		for ($xbln=1; $xbln<=12; $xbln++){
	...
	if($nodiv>0){
		...
		$tab.="<td bgcolor=#ffff99 align=right>".number_format($gtLuasBlok,2)."</td>";
		for ($xbln=1; $xbln<=12; $xbln++){
