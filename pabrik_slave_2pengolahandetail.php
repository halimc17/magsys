...
if(!isset($_GET['type']) or $_GET['type']!='excel')$stream="<table class=sortable border=0 cellspacing=1>"; else
...
if(!isset($_GET['type']) or $_GET['type']!='excel')$stream.="<td>Browse</td>";
        $stream.="</tr>
      </thead>
      <tbody>";
    $str="select * from ".$dbname.".pabrik_pengolahan
              where tanggal = '".$tanggal."' and kodeorg='".$kodeorg."'";   
    $res=mysql_query($str);
    $no=0;
    $tdebet=0;
    $tkredit=0;
