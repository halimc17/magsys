...
echo"<table class=sortable cellspacing=1 border=0 width=490px>
    <tr class=rowcontent>
    <td>".$qwe."</td>
...
//    <table class=sortable cellspacing=1 border=0 width=490px>
//    <tbody>";
echo"<marquee height=130 onmouseout=\"this.setAttribute('scrollamount', 1, 0);\" onmouseover=\"this.setAttribute('scrollamount', 0, 0);\" scrolldelay=20 scrollamount=1 behavior=scroll direction=up>
    <table class=sortable cellspacing=1 border=0 width=490px>
    <tbody>";
...
if(!empty($karyawan))foreach($karyawan as $kar){
    ...
    if(($presensi[$kar['id']]['waktumasuk'])or($presensi[$kar['id']]['waktukeluar'])){
        ...
        if(($hariini>='2014-06-30')and($hariini<='2014-07-25')){                // puasa 2014
            if($presensi[$kar['id']]['waktumasuk'])if(substr($presensi[$kar['id']]['waktumasuk'],11,5)>'07:30')$warning=true;
            if($presensi[$kar['id']]['waktukeluar'])if(substr($presensi[$kar['id']]['waktukeluar'],11,5)<'16:00')$warning=true;
        }else
