...
class PDF extends FPDF
{
	...
	global $posting;
	   ...
	   $qryTotBiayaPekerjaan=mysql_query($strTotBiayaPekerjaan) or die(mysql_error());
	   $resTotBiayaPekerjaan=mysql_fetch_assoc($qryTotBiayaPekerjaan);
	   ...
	   $qryTotBiayaOperator=mysql_query($strTotBiayaOperator) or die(mysql_error());
	   $resTotBiayaOperator=mysql_fetch_assoc($qryTotBiayaOperator);
	   ...
	   $valUpah = $resTotBiayaOperator['upah']+$resTotBiayaOperator['premi']+$resTotBiayaOperator['penalty'];
		...
		$this->Cell(40,4,": ".(isset($res5->namakaryawan)? $res5->namakaryawan: ''),0,1,'L');  
...
//ambil kelengkapan
	...
	$pdf->Cell(20,5,$_SESSION['lang']['vhc_kmhm_akhir'],1,0,'L',1);	
  	$pdf->Cell(25,5,$_SESSION['lang']['vhc_berat_muatan'],1,0,'L',1);	
	...
	//$pdf->Cell(25,5,'Total',1,1,'C',1);
		...
		while($res=mysql_fetch_assoc($re))
		{
			...
			$pdf->Cell(20,5,number_format($res['kmhmawal'],2),1,0,'R',1);
			$pdf->Cell(20,5,number_format($res['kmhmakhir'],2),1,0,'R',1);	
    ...
    $pdf->Cell(25,5,$_SESSION['lang']['upahpremi']." sdi",1,1,'L',1);		
	...
	//$pdf->Cell(25,5,'Total',1,1,'C',1);
		...
		while($res=mysql_fetch_assoc($re))
		{
			...
			$pdf->Cell(40,5,$res5->namakaryawan,1,0,'L',1);
                        ...
                        $pdf->Cell(25,5,number_format($rstrd['premisdi'],0),1,1,'R',1);
