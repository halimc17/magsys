...
switch($proses) {
    case 'showDetail':
		...
		if(empty($optKary)) {
			exit("Warning: Data Karyawan KHT dan KHL tidak ada.".
				 "\nTransaksi BM tidak dapat dilanjutkan");
		}
		...
		$cols = "nik,kodeorg,kodesegment,tahuntanam,norma,hasilkerjakg,upahkerja,upahpremi,upahpenalty";
		...
		foreach($dataShow as $key=>$row) {
			...
			// kamus hari libur
			$strorg="select * from ".$dbname.".sdm_5harilibur where tanggal = '".$tanggalx."' and kebun in ('GLOBAL','".$_SESSION['empl']['lokasitugas']."'";
		...
		$theForm2->addEls('kodesegment',$_SESSION['lang']['kodesegment'],'','searchSegment','L',15);
		$theForm2->addEls('tahuntanam',$_SESSION['lang']['tahuntanam'],$thTanam,'textnum','C',5);
		$theForm2->_elements[3]->_attr['disabled'] = 'disabled';
		$theForm2->addEls('norma',$_SESSION['lang']['norma'],'5000','textnum','R',10);
		$theForm2->_elements[4]->_attr['disabled'] = 'disabled';
		$theForm2->_elements[4]->_attr['title'] = 'Basis diambil Basis KG Bongkar Muat';
		$theForm2->addEls('hasilkerjakg',$_SESSION['lang']['hasilkerjakg'],'0','textnum','R',10);
		$theForm2->_elements[5]->_attr['onkeyup'] = "countPremi();";
		$theForm2->addEls('upahkerja',$_SESSION['lang']['upahkerja'],0,'textnum','R',10);
		$theForm2->_elements[6]->_attr['disabled'] = 'disabled';
		$theForm2->_elements[6]->_attr['title'] = 'Upah harian';
		$theForm2->addEls('upahpremi',$_SESSION['lang']['premilebihbasis'],'0','textnum','R',10);
		$theForm2->_elements[7]->_attr['disabled'] = 'disabled';
		$theForm2->_elements[7]->_attr['title'] = 'Hasil Kerja > Basis * Premi Lebih Basis';
		...
		$formTab2->_target = "kebun_bm_slave_detail";
		$formTab2->_noClearField = '##kodeorg##tahuntanam';
		$formTab2->_noEnable = '##tahuntanam##norma##hasilkerjakg##upahkerja##upahpenalty##upahpremi##kodesegment';
	//$formTab2->_noEnable='##tahuntanam##norma##outputminimal##hasilkerjakg##upahkerja##upahpenalty##upahpremi##premibasis##rupiahpenalty##jjgpenalty##kodesegment';
		$formTab2->_defValue = '##upahkerja=0##kodesegment=1';
		$formTab2->_afterCrud = "showDetail";
		$formTab2->_numberFormat = '##upahkerja##upahpremi##upahpenalty';
		...
		// Jumlah Transaksi BM per Karyawan
		$karyTrans = array();
    ...
    case 'add':
		...
		cekPrestasi($param);
            ...
            // cek yang bisa BM berdasarkan taksasi
            $luastaksasi=0;
            $hktaksasi=0;
            $query = "SELECT *
        ...
        if($day=='Sun')$libur=true; else $libur=false;
        // kamus hari libur
        $strorg="select * from ".$dbname.".sdm_5harilibur where tanggal = '".substr($param['notransaksi'],0,8)."' and kebun in ('GLOBAL','".$_SESSION['empl']['lokasitugas']."'";
        $queorg=mysql_query($strorg) or die(mysql_error());
        while($roworg=mysql_fetch_assoc($queorg))
        {
            if($roworg['keterangan']=='libur')$libur=true;
		...
		$cols = array(
			'nik','kodeorg','kodesegment','tahuntanam','norma','hasilkerjakg',
			'upahkerja','upahpremi','upahpenalty','notransaksi',
			'outputminimal','hasilkerja','luaspanen','brondolan',
			'penalti1','penalti2','penalti3','penalti4','penalti5','penalti6','penalti7','penalti8','penalti9','penalti10',
			'rupiahpenalty','jjgpenalty','kodekegiatan','statusblok','pekerjaanpremi'
		);
		$data = $param;
		
		unset($data['numRow']);
		# Additional Default Data
		$data['outputminimal'] = 0;
		$data['hasilkerja'] = 0;
		$data['luaspanen'] = 0;
		$data['brondolan'] = 0;
		$data['penalti1'] = 0;
		...
		$data['penalti9'] = 0;
		$data['penalti10'] = 0;
		$data['rupiahpenalty'] = 0;
		$data['jjgpenalty'] = 0;
		$data['kodekegiatan'] = '611020103';
		$data['statusblok'] = 'TM';
		$data['pekerjaanpremi'] = 0;

        # periksa luas BM hari ini apakah sudah melebihi setup blok
        // cari luas blok
    ...
    if($tanggalnya==''){
        ...
        // cari luas BM yang sudah diinput ditambah inputan
		...
		unset($data['outputminimal']);
		unset($data['hasilkerja']);
		unset($data['luaspanen']);
		unset($data['brondolan']);
		unset($data['penalti1']);
		unset($data['penalti2']);
		unset($data['penalti3']);
		unset($data['penalti4']);
		...
		unset($data['penalti8']);
		unset($data['penalti9']);
		unset($data['penalti10']);
		unset($data['rupiahpenalty']);
		unset($data['jjgpenalty']);
...
//        echo "error:".$query; exit;
    ...
    $data['jjgpenalty']=str_replace(",","",$data['jjgpenalty']);
        ...
        if($day=='Sun')$libur=true; else $libur=false;
        // kamus hari libur
        $strorg="select * from ".$dbname.".sdm_5harilibur where tanggal = '".$tanggalnya."' and kebun in ('GLOBAL','".$_SESSION['empl']['lokasitugas']."'";
        $queorg=mysql_query($strorg) or die(mysql_error());
        while($roworg=mysql_fetch_assoc($queorg))
        {
//            $libur=true;
        ...
        //        echo "error:".$jjgmasak.",".$hasilkerja;
                ...
                // kalo janjang BM>(janjang masak taksasi x 1.1), set premibasis=53000 where premibasis>53000 and notransaksi=notransaksi
	...
	case 'countPremi':
		...
		$totaltbskg=$param['hasilkerjakg'];
		if($totaltbskg<=0){
			$upahkerja=0;
			$upahpenalty=0;
			$upahpremi=0;
		}elseif($totaltbskg<5000){
			$upahkerja=round($upahharian,2);
			$upahpenalty=round(($upahharian-($totaltbskg*10)),2);
			$upahpremi=0;
		}elseif($totaltbskg>=5000 and $totaltbskg<9000){
			$upahkerja=round($upahharian,2);
			$upahpremi=round((($totaltbskg-5000)*10),2);
			$upahpenalty=0;
		}else{
			$upahkerja=round($upahharian,2);
			$upahpremi=round(((4000*10)+(($totaltbskg-9000)*12)),2);
			$upahpenalty=0;
		}
		//exit('Warning: '.$upahkerja.'  '.$upahpremi.'  '.$upahpenalty);
		$res = array(
			'upahkerja' => $upahkerja,
			'upahpremi' => $upahpremi,
	...
	case 'count2Premi':
		...
		// Get Basis BM dan Ketentuan Premi
		...
		if(empty($resBasis)) exit("Warning: Basis BM belum ada untuk\nPT ".
		...
		if($libur) {
			...
			if(empty($resBasis1)) exit("Warning: Basis BM belum ada untuk\nPT ".
									  $_SESSION['org']['kodeorganisasi']."\nJenis Premi KERJA".
									  "\nKelas Pohon ".$kelaspohon."\nTopografi ".
									  $dataBlok['topografi']);
		}
...
function cekPrestasi($param) {
	...
	*/
	// Cek Perawatan
	// Jika sudah ada di perawatan tidak bisa input BM
	// Jika karyawan ada pekerjaan BM dan perawatan, maka harus malekukan input BM terlebih dahulu
	...
	if(!empty($resAbs)) {
		exit("Warning: Karyawan sudah terdaftar di kemandoran lain");
	}
...
function proporsiUpah($param) {
	...
	while($rowPres=mysql_fetch_assoc($resPres))
    {
		...
		$hasilkerjakg=$rowPres['hasilkerjakg'];
		...
		if($totaltbskg<=0){
			...
			$upahkerja=round($upahharian*($hasilkerjakg/$totaltbskg),2);
			$upahpenalty=round(($upahharian-($totaltbskg*10))*($hasilkerjakg/$totaltbskg),2);
			$upahpremi=0;
		}elseif($totaltbskg>=5000 and $totaltbskg<9000){
			$upahkerja=round($upahharian*($hasilkerjakg/$totaltbskg),2);
			$upahpremi=round((($totaltbskg-5000)*10)*($hasilkerjakg/$totaltbskg),2);
			$upahpenalty=0;
		}else{
			$upahkerja=round($upahharian*($hasilkerjakg/$totaltbskg),2);
			$upahpremi=round(((4000*10)+(($totaltbskg-9000)*12))*($hasilkerjakg/$totaltbskg),2);
			$upahpenalty=0;
		}
