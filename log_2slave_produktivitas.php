...
for($ard=1;$ard<=$jumHari;)
{
    ...
    for($awal=$awalitung;$awal<=7;$awal++)
    {
        ...
        $qCount=mysql_query($sCount) or die(mysql_error($conn));
            ...
            if($rCount['purchaser']!='')
            {
				if(!isset($jmlhPo[$rCount['purchaser']][$mingguke])) $jmlhPo[$rCount['purchaser']][$mingguke]=0;
...
{
    if(!isset($totPP[$rDrPP['purchaser']])) $totPP[$rDrPP['purchaser']]=0;
...
{
	if(!isset($outPP[$rOutpp['purchaser']])) $outPP[$rOutpp['purchaser']]=0;
...
if($dtcek!=0)
{
    ...
    foreach($purdt as $dtPur)
    {
        ...
        {
			if(empty($jmlhPo[$dtPur][$awalmngg])) {
