...
if($lapke>0){
	$iList="SELECT kodeorg
			...
			,sum(if(solar_rad<200,0,if(solar_rad<700,1,2))) as jam_solar_rad,sum(solar_rad) as sum_solar_rad
			,sum(if(uv_dose=0,0,1))/(count(waktu)/24) as jam_uv_dose,sum(uv_dose) as uv_dose
	...
	while($dList=mysql_fetch_assoc($nList)){
		...
		$jam_solar_rad=$dList['jam_solar_rad']	;$sum_solar_rad=$dList['sum_solar_rad'];
...
while($dList=mysql_fetch_assoc($nList)){
	...
	$stream.="	<td align='right'>".number_format($dList['temp_out'],2,".","")."</td>
			...
			if($dList['solar_rad']<200){
				...
				$durasi_solar_rad[$dList['tanggal']]+=2;
			}
	...
	if($lapke==1){
		$stream.="<td align='right'>".number_format($dList['jam_solar_rad'],2,".","")."</td>
				<td align='right'>".number_format($dList['sum_solar_rad']/$dList['jam_solar_rad'],2,".","")."</td>
		...
		$stream.="<td align='right'>".number_format($jam_solar_rad[$dList['tanggal']],2,".","")."</td>
				<td align='right'>".number_format($dList['sum_solar_rad']/$jam_solar_rad[$dList['tanggal']],2,".","")."</td>
...
if($lapke==0){
	...
	$stream.="<td align='right'>Durasi: ".number_format(array_sum($durasi_solar_rad),2,".","")."</td>";
	...
	$stream.="<td align='right'>Radiasi: ".number_format(array_sum($solar_rad)/array_sum($durasi_solar_rad),2,".","")."</td>";
	...
	$stream.="<tr bgcolor='#DEFEDE'>
				...
				<td align='right'>".number_format($sum_solar_rad/$ttjam_solar_rad,2,".","")."</td>
