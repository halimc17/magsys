...
$frm[0]="<fieldset>
    ...
    <table>
        ...
        <tr>
			...
			<input type=hidden id=pengurang class=myinputtextnumber  onkeypress=\"return angka_doang(event)\" size=20 />
			</td>
			
			<td>".$_SESSION['lang']['induk']."</td><td>".makeElement('induk','text','',array('maxlength'=>25))."             
			<img src=images/zoom.png title='".$_SESSION['lang']['find']."' id=tmblCariNoInduk class=resicon onclick=cariNoInduk('".$_SESSION['lang']['find']."',event)>
			</td>
		</tr>
        <tr>
			...
			<td>".$_SESSION['lang']['namakelompok']."</td><td><select id=tipe onchange=getSub()>".$optAss."</select>
			Sub : <select id=sub onchange=cek()>".$optSub."</select>
			</td>
		</tr>
        <tr>
		...
		<tr><td>".$_SESSION['lang']['nodokpengadaan']."</td><td>".makeElement('nodokpengadaan','text','',array('maxlength'=>25))."             
...
$str="select a.*,".$ads."
        ...
        while($bar=mysql_fetch_object($res))
        {
			...
			$frm[1].=" <tr class=rowcontent>
				...
				<td width=20 align=center>".$arrstatus[$bar->status]."</td>
