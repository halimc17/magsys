...
<?php
	...
	$arr.="##kodecustomer##nilaiinvoice##stsppn##nilaippn##stspph##nilaipph##matauang##kurs##ttd##jenis##debet##kredit##keterangan";
	...
	$sKomoditi="select distinct(a.kodebarang),a.namabarang from ".$dbname.".log_5masterbarang a where a.kodebarang like '4%' order by a.namabarang";
	...
	echo"			<td>".$_SESSION['lang']['harga']."</td>";
	...
	$sakundbt="select distinct noakun,namaakun from ".$dbname.".keu_5akun where (noakun like '511%' or noakun='9110107') and detail=1
			    order by namaakun asc";
	$qakun=mysql_query($sakundbt) or die(mysql_error($conn));
	$optKredit='';
	while($rakun=  mysql_fetch_assoc($qakun)){
	...
	echo"<td>".$_SESSION['lang']['harga']."</td><td><input type=text id=harga class=myinputtextnumber onblur=getSubTotal(0) style=width:150px; onkeypress='return angka_doang(event)' /></td></tr>";
	echo"<tr><td>".$_SESSION['lang']['nmcust']."</td><td><select id=kodecustomer style=width:150px >".$optCust."</select></td>";
	echo"<td>".$_SESSION['lang']['nilaiinvoice']."</td><td><input type=text id=nilaiinvoice class=myinputtextnumber onblur=getSubTotal(1) style=width:150px; onkeypress='return angka_doang(event)' /></td></tr>";
	...
	echo"<td>".$_SESSION['lang']['kredit']."</td><td><select id=kredit style=width:150px;>".$optKredit."</select></td></tr></table>";
	echo"<table><tr><td>".$_SESSION['lang']['keterangan']."&ensp;&ensp;&ensp;&ensp;&nbsp;</td><td><input type=text id=keterangan class=myinputtext style=width:385px;></td></tr>";
	echo "<tr><td><br></td></tr>";
