...
switch($proses){
	case'loadData':
		...
		while($bar=mysql_fetch_object($res)){
			...
			$jamjalan1+=$bar->jam;
			$jamjalan2+=$bar->jam;
			if($bar->tipeservice=='2'){
				$jenisservice='Major/General Overhaul';
				$jamjalan2=0;
				$jamganti=$bar->jamganti;
			}elseif($bar->tipeservice=='1'){
				$jenisservice='Immediately/Top Overhaul';
				$jamjalan1=0;
				$jamganti=$bar->jamganti;
			}else{
			...
			echo"<tr class=rowcontent >
					...
					<td ".$drcl." align=right width='5%'>".number_format($bar->hmawal,0,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($bar->hmakhir,0,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($bar->jam,0,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($jamjalan1,0,'.',',')."</td>
					<td ".$drcl." align=right width='5%'>".number_format($jamjalan2,0,'.',',')."</td>
					...
					<td align=center width='4%'>
						...
						<img src=images/application/application_delete.png class=resicon title='Delete' onclick=\"deldata('".$bar->kodeorg."','".$bar->kodemesin."','".tanggalnormal($bar->tanggal)."','".$bar->tipeservice."');\">
					</td>
				</tr>";	
		}
		break;
	...
	case'saveData':
		...
		//if($rows>0){
		$jamganti=0;
		$strs="select * from ".$dbname.".pabrik_hm_setup where kodemesin='".$mesin."'";
		$ress=mysql_query($strs);
		while($bars=mysql_fetch_object($ress)){
			if($tipeservice=='2'){
				$jamganti=$bars->jamganti2;
			}elseif($tipeservice=='1'){
				$jamganti=$bars->jamganti1;
			}else{
				$jamganti=0;
			}
		}
