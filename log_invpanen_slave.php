...
$kodeinvlama=checkPostGet('kodeinvlama','');
...
$niklama=checkPostGet('niklama','');
$namakaryawan=checkPostGet('namakaryawan','');
$namakaryawanlama=checkPostGet('namakaryawanlama','');
$tgldiuser=tanggalsystem(checkPostGet('tgldiuser',''));//merubah dari 10-10-2014 menjadi 20141010
$jumlahinv=checkPostGet('jumlahinv',0);
$satuaninv=checkPostGet('satuaninv','');
...
switch($proses){
	case'loadData':
		...
		$strb="select a.kodeinventaris from ".$dbname.".log_invpanen a 
				where ".$where." 
		...
		$offset=$page*$limit;
		//$str="select a.*,c.namasupplier as supplier,d.namakaryawan as karyawan from ".$dbname.".log_invpanen a 
		...
		$str="select a.*,c.namasupplier as supplier from ".$dbname.".log_invpanen a 
				left join ".$dbname.".log_5supplier c on c.supplierid=a.kodesupplier
				where ".$where." 
				order by a.kodeorg,a.kodebarang,a.kodeinventaris limit ".$offset.",".$limit."";
		//exit('Warning: '.$str);
		...
		while($bar=mysql_fetch_object($res)){
			...
			echo"<tr class=rowcontent >
					...
					<td ".$drcl." align=right>".number_format($bar->jumlah,0,'.',',')."</td>
					<td ".$drcl." align=left>".$bar->satuan."</td>
					...
					<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodeorg."','".$bar->kodebarang."','".$bar->kodeinventaris."','".$bar->namainventaris."','".$bar->merkinventaris."','".$bar->tipeinventaris."','".$bar->ketinventaris."','".$bar->ukuran."','".$bar->warna."','".$bar->bahan."','".tanggalnormal($bar->tglperolehan)."','".$bar->hargaperolehan."','".$bar->nopo."','".$bar->kodesupplier."','".$bar->nik."','".$bar->namakaryawan."','".tanggalnormal($bar->tgluserterima)."','".$bar->jumlah."','".$bar->satuan."','".$bar->kondisi."','".$bar->divisi."','".$bar->lokasi."','".$bar->ruangan."')\">&nbsp
					<img src=images/application/application_delete.png class=resicon title='Delete' onclick=\"deldata('".$bar->kodeorg."','".$bar->kodebarang."','".$bar->kodeinventaris."','".$bar->nik."','".$bar->namakaryawan."');\">&nbsp
	...
	case'delData':
		$strx="delete from ".$dbname.".log_invpanen where kodeorg='".$kodeorg."' and kodebarang='".$kodebarang."' and kodeinventaris='".$kodeinv."' and nik='".$nik."' and namakaryawan='".$namakaryawan."'";
		if(!mysql_query($strx)){
	...
	case'saveData':
		...
		if($kodeorglama!=$kodeorg or $kodebrglama!=$kodebarang or $kodeinvlama!=$kodeinv or $niklama!=$nik or $namakaryawanlama!=$namakaryawan){
			$strs="select * from ".$dbname.".log_invpanen where (kodeorg='".$kodeorg."' and kodebarang='".$kodebarang."' and kodeinventaris='".$kodeinv."' and nik='".$nik."' and namakaryawan='".$namakaryawan."')";
		...
		if($addedit=='update'){
			$strx="update ".$dbname.".log_invpanen set
					kodeorg='".$kodeorg."',kodebarang='".$kodebarang."',kodeinventaris='".$kodeinv."',namainventaris='".$namainv."',merkinventaris='".$merkinv."',tipeinventaris='".$tipeinv."',ketinventaris='".$ketinv."',ukuran='".$ukuraninv."',warna='".$warnainv."',bahan='".$bahaninv."',tglperolehan='".$tglbeli."',hargaperolehan=".round($hrgbeli,2).",nopo='".$nopo."',kodesupplier='".$kodesupplier."',namasupplier='".$namasupplier."',nik='".$nik."',namakaryawan='".$namakaryawan."',tgluserterima='".$tgldiuser."',jumlah='".$jumlahinv."',satuan='".$satuaninv."',kondisi='".$kondisi."',divisi='".$divisi."',lokasi='".$lokasi."',ruangan='".$ruangan."'
					,lastuser='".$_SESSION['standard']['username']."',lastdate=now()
					where kodeorg='".$kodeorglama."' and kodebarang='".$kodebrglama."' and kodeinventaris='".$kodeinvlama."' and nik='".$niklama."' and namakaryawan='".$namakaryawanlama."'";
		}else{
			$strx="insert into ".$dbname.".log_invpanen
					...
					,nopo,kodesupplier,namasupplier,nik,namakaryawan,tgluserterima,jumlah,satuan,kondisi,divisi,lokasi,ruangan,aktif,lastuser,lastdate)
					...
					,'".$nik."','".$namakaryawan."','".$tgldiuser."','".$jumlahinv."','".$satuaninv."','".$kondisi."','".$divisi."','".$lokasi."','".$ruangan."'
	...
	case'getBarang':
		$optBarang="";
		if($kodebarang!=''){
			$sBarang="select namabarang,satuan from ".$dbname.".log_5masterbarang where kodebarang='".$kodebarang."'";     
			$qBarang=mysql_query($sBarang) or die(mysql_error($conn));
			while($rBarang=mysql_fetch_assoc($qBarang)){
				$optBarang=$rBarang['namabarang'];
				$optSatuan=$rBarang['satuan'];
			}
		}
		echo $optBarang."###".$optSatuan;
    break;

	case'getKaryawan':
		$optKry="";
		if($nik!=''){
			$sKaryawan="select namakaryawan from ".$dbname.".datakaryawan where nik='".$nik."'";
			//exit('Warning: '.$sKaryawan);
			$qKaryawan=mysql_query($sKaryawan) or die(mysql_error($conn));
			while($rKaryawan=mysql_fetch_assoc($qKaryawan)){
				$optKry=$rKaryawan['namakaryawan'];
			}
		}
		echo $optKry;
    break;
