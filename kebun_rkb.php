...
{
    $whereCont = "tipetransaksi='".$tipeVal."'";
}
else
{
	if(in_array($_SESSION['empl']['kodejabatan'],$postJabatan)) {
       ...
       $whereCont = "tipetransaksi='".$tipeVal."' and updateby='".$_SESSION['standard']['userid']."'";
    }
}
$whereContArr = array();
$whereContArr[] = array('tipetransaksi',$tipeVal);
...
$query = selectQuery($dbname,'kebun_rkb_aktifitas',$cols,$whereCont.
    " and kodeorg='".$_SESSION['empl']['lokasitugas']."'",
    "jurnal, tanggal desc, notransaksi desc",false,10,1);
$data = fetchData($query);
$totalRow = getTotalRow($dbname,'kebun_rkb_aktifitas',$whereCont);
if(!empty($data)) {
...
#=== Display View
OPEN_BOX();
echo "<div align='center'><h3>".$_SESSION['lang']['rkb']."</h3></div>";
