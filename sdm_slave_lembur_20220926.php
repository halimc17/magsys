		...
		switch($proses)
        {
                ...
                if($rCek<1)
                {
                        ...
                        if(mysql_query($sIns))
                        {
							if(($tpLmbr!='')&&($Jam!=''))
                            {
								...
								if($Jam>$batasjam and ($noba=='' or is_null($noba))){
                                ...
                                (`kodeorg`,`tanggal`,`karyawanid`,`tipelembur`,`jamaktual`,`uangmakan`,`uangtransport`,`uangkelebihanjam`,`noba`,`beban`,`noakun`,`posting`,`postby`)
								values ('".$kdOrg."','".$tgl."','".$krywnId."','".$tpLmbr."','".$Jam."','".$ungMkn."','".$ungTrans."','".$ungLbhjm."'
                ...
                else
                {
					if(($tpLmbr!='')&&($Jam!=''))
                    {
						...
						$sDetIns="insert into ".$dbname.".sdm_lemburdt 
                     (`kodeorg`,`tanggal`,`karyawanid`,`tipelembur`,`jamaktual`,`uangmakan`,`uangtransport`,`uangkelebihanjam`,`noba`,`beban`,`noakun`,`posting`,`postby`) values ('".$kdOrg."','".$tgl."','".$krywnId."','".$tpLmbr."','".$Jam."','".$ungMkn."','".$ungTrans."','".$ungLbhjm."','".$noba."','".$beban."','".$noakun."',".$stspost.",'".$by_post."')";
                        //echo"warning:".$sDetIns;exit();
...
$offset=$page*$limit;
		...
		case'updateDetail':
			...
			if(($tpLmbr!='')&&($Jam!='')){
				...
				//exit('Warning: '.$noba);
                $sUp="update ".$dbname.".sdm_lemburdt set tipelembur='".$tpLmbr."',jamaktual='".$Jam."',uangmakan='".$ungMkn."',uangtransport='".$ungTrans."',uangkelebihanjam='".$ungLbhjm."',beban='".$beban."',noakun='".$noakun."',noba='".$noba."',posting=".$stspost." where kodeorg='".$kdOrg."' and tanggal='".$tgl."' and karyawanid='".$krywnId."'";
				//exit('Warning: '.$sUp);
                if(mysql_query($sUp))
                        echo"";
                        else
	        ...
	        while($rBeban2=mysql_fetch_assoc($qBeban2)){
                ...
                $table.="
					<tr class=rowcontent>
						...
						<td><select id=tpLmbr name=tpLmbr style='width:90px' onchange='getLembur(0,0)'>".$optTipelembur."</select></td>
						<td><select id=jam name=jam style='width:100px' onchange='getUangLem()'><option value=''>".$_SESSION['lang']['pilihdata']."</option></select></td>
						<td><select id=beban name=beban style='width:110px'>".$optbeban."</select></td>
						<td><input type='text' class='myinputtextnumber' id='uang_lbhjm' name='uang_lbhjm' style='width:85px' onkeypress='return angka_doang(event)' value=0 disabled /></td>
						<td><input type='text' class='myinputtextnumber' id='uang_trnsprt' name='uang_trnsprt' style='width:85px' onkeypress='return angka_doang(event)' value=0  /></td>
						<td><input type='text' class='myinputtextnumber' id='uang_mkn' name='uang_mkn' style='width:85px' onkeypress='return angka_doang(event)' value=0 /></td>
						<td><input type='text' class='myinputtext' id='noba' name='noba' maxlength=30 style='width:180px'></td>
