...
foreach($tmpRes as $key=>$row) {
    ...
    $data[$i-1]['kredit']=number_format($data[$i-1]['kredit'],2);
    $data[$i-1]['debet']=number_format($data[$i-1]['debet'],2);
    $i++;
}
...
$title = 'Summary Memorial Journal';
 ...
 if($proses=='excel')$table = "<table class='sortable' border=1 cellspacing=1>";
        ...
        $table .= "<td align='right'>".number_format($totalDebet,2)."</td>";
        $table .= "<td align='right'>".number_format($totalKredit,2)."</td>";
        $table .= "</tr>";
        $table .= "</tbody>";
        $table .= "</table>";
#=== Formatting ===
