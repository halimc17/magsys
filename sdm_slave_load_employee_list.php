...
else if(trim($_SESSION['empl']['tipelokasitugas'])=='KANWIL')
{
		...
		$whrd="regional='".$_SESSION['empl']['regional']."'";
		//print_r($whrd);
    if($_SESSION['empl']['regional']=='SUMSEL'){
	    $whrd="regional in ('".$_SESSION['empl']['regional']."','LAMPUNG')";
    }
$str="select a.*,b.namajabatan,c.namagolongan,d.tipe from ".$dbname.".datakaryawan a, 
      ".$dbname.".sdm_5jabatan b, ".$dbname.".sdm_5golongan c,  ".$dbname.".sdm_5tipekaryawan d where 
	  a.kodejabatan=b.kodejabatan and a.kodegolongan=c.kodegolongan
	  and d.id=a.tipekaryawan and a.tipekaryawan!=0 and lokasitugas in (select distinct kodeunit from ".$dbname.".bgt_regional_assignment where ".$whrd.")
	  ".$where."
	  limit ".$maxdisplay.",".$getrows
	  ;   
...
if($numrows<1)
{
	echo "<tr class=rowcontent>
		<td colspan=16 style='text-align:center'>".$_SESSION['lang']['datanotfound']."</td>
		</tr>";
}
else
{
