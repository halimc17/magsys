...
<?php
	...
	$sPeriode="select distinct periode from ".$dbname.".sdm_5periodegaji where sudahproses=1 order by periode desc";
	...
	$sTipe="select id,tipe from ".$dbname.".sdm_5tipekaryawan where id NOT IN ('5','6') order by tipe asc";
	//echo $sTipe;
	...
	$sJab="select kodejabatan,namajabatan from ".$dbname.".sdm_5jabatan order by namajabatan asc";
	$qJab=mysql_query($sJab) or die(mysql_error());
	while($rJab=mysql_fetch_assoc($qJab)){
	...
	$arr="".$dr."##periode##tipeKary##jabatan##sistemGaji";
?>
...
<div>
	<fieldset style="float: left;">
		...
		<table cellspacing="1" border="0" >
			...
			<tr><td><label><?php echo $_SESSION['lang']['unit']?></label></td><td><?php echo $optOrg?></td></tr><?php } ?>
			...
			<tr><td><label><?php echo $_SESSION['lang']['jabatan']?></label></td><td><select id="jabatan" name="jabatan" style="width:150px"><?php echo $optJab?></select></td></tr>
			...
			<tr><td colspan="2">
					<button onclick="zPreview('sdm_slave_2daftarIuran_jamsostek','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button>
					<button onclick="zPdf('sdm_slave_2daftarIuran_jamsostek','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button>
					<button onclick="zExcel(event,'sdm_slave_2daftarIuran_jamsostek.php','<?php echo $arr?>')" class="mybutton" name="preview" id="preview">Excel</button>
