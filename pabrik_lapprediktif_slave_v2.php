...
if(is_array($listmesin)){
	...
	foreach ($listmesin as $stasiun=>$row){
		if(in_array($stasiun, $liststasiun)) {
			$ttjmlharga=0;
			$stream.="<thead>";
			$stream.="<tr class=rowheader><td align=left colspan=8><b>Station : ".$stasiun." - ".$nmOrg[$stasiun]."</td></tr>";
			$stream.="<tr class=rowheader><td align=left colspan=8><b>Pabrik : ".$pabrik." - ".$nmOrg[$pabrik]."</td></tr>";
			$stream.="</thead><tbody>";
			/*
			...
			foreach($row as $mesin=>$row2){
				...
				$stream.="<td align=center colspan=8><b>".$mesin." - ".$nmOrg[$mesin]."</td>";
				$stream.="</tr>";
...
######EXCEL	
	case 'excel':
		...
		$tglSkrg=date("Ymd");
		$nop_="LAPORAN_PREDIKTIF_II_".$tglSkrg;
		if(strlen($stream)>0){
