...
<?
	...
	if($_SESSION['empl']['tipelokasitugas'] == 'HOLDING'){
		...
		$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where kodeorganisasi in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['induk']."') and length(kodeorganisasi)=4 order by namaorganisasi";
	}
	$res=mysql_query($str);
	...
	echo"<fieldset><legend>DAFTAR FILE</legend>
			<select style='width:175px;' name='kodeorg1'  id='kodeorg1'>".$optOrg."</select>
	...
	echo "	<div style='height:140px;width:100%;overflow:scroll;'>
	...
	if($_SESSION['empl']['tipelokasitugas'] == 'HOLDING'){
		...
		$str1="select a.*,b.namakaryawan from ".$dbname.".rencana_gis_file a
				left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
				where a.unit in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['induk']."') ".$where."
				order by a.lastupdate desc";
	}
	...
	echo" <iframe name=frame id=frame  frameborder=0 width=100% height=330px></iframe><br>";
	CLOSE_BOX();
	echo close_body();
?>
