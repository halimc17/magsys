...
//exit("Error".$jmAwal);
        switch($proses)
        {
                ...
                $ql2="select count(*) as jmlhrow from ".$dbname.".pabrik_timbangan where nokontrak!='' ".$vOrg." order by tanggal desc";// echo $ql2;
                ...
                $tab.="<td>".$_SESSION['lang']['action']."</td>";     
                $tab.="</tr></thead><tbody>";
                $sData="select notransaksi,tanggal,beratbersih,nokendaraan,nokontrak,kodebarang,kgpembeli from ".$dbname.".pabrik_timbangan where nokontrak!='' ".$vOrg." order by tanggal desc limit ".$offset.",".$limit."";
                //echo $sData;
                $qData=mysql_query($sData) or die(mysql_error($conn));
                $row=mysql_num_rows($qData);
                if($row>0)
                ...
                $tab.="<td>".$_SESSION['lang']['action']."</td>";     
                $tab.="</tr></thead><tbody>";
                $sData="select notransaksi,tanggal,beratbersih,nokendaraan,nokontrak,kodebarang,kgpembeli from ".$dbname.".pabrik_timbangan where nokontrak!='' ".$where." ".$vOrg." order by tanggal desc limit ".$offset.",".$limit."";
                //echo $sData;
                $qData=mysql_query($sData) or die(mysql_error($conn));
                $row=mysql_num_rows($qData);
                if($row>0)
