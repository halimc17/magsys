...
while($bar=mysql_fetch_object($res))
{
        ...
        $hp=$bar->nohp;
        $passpor=$bar->no_keluarga;
        $ktp=$bar->noktp;
        $tdarurat=$bar->notelepondarurat;
        $tmasuk=tanggalnormal($bar->tanggalmasuk);
        $tpengangkatan=tanggalnormal($bar->tanggalpengangkatan);
	...
	$years=(date('Y',$tdate)-date('Y',$dob))-(date('m',$tdate)<date('m',$dob) or (date('m',$tdate)==date('m',$dob) and date('d',$tdate)<date('d',$dob)) ?1 :0);
...
//=============pdf
	...
	$pdf->Cell(30,5,$_SESSION['lang']['tanggalpengangkatan'],0,0,'L');
        if($tpengangkatan=='00-00-0000')
        {
            $pdf->Cell(40,5,':',0,0,'L');
        }
        else
        {
            $pdf->Cell(40,5,': '.$tpengangkatan,0,0,'L');
        }
        $pdf->SetX(125);	//
        $pdf->Cell(25,5,'Masa Kerja',0,0,'L');
        $pdf->Cell(40,5,':'.$lamaKerja,0,1,'L');
    ...
    $pdf->Cell(32,5,$_SESSION['lang']['jms'],0,0,'L');
                $pdf->Cell(60,5,': '.$jms,0,0,'L');	
	$pdf->Cell(32,5,$_SESSION['lang']['statusakad'],0,0,'L');
                $pdf->Cell(70,5,': '.$txtAkad,0,1,'L');
    $pdf->Ln();	
}
//=======================Riwayat Pekerjaan
        $pdf->SetFont('Arial','B',10);		
...
//=======================Riwayat Kursus
    $pdf->Ln();
        $pdf->SetFont('Arial','B',10);		
        $pdf->Cell(25,5,'4. '.strtoupper($_SESSION['lang']['traininginternal']),0,1,'L');
        $pdf->SetFont('Arial','',7);												

    $pdf->Cell(6,4,'No',1,0,'L',1);
    $pdf->Cell(30,4,$_SESSION['lang']['jeniskursus'],1,0,'C',1);
    $pdf->Cell(50,4,$_SESSION['lang']['legend'],1,0,'C',1);	
    $pdf->Cell(45,4,$_SESSION['lang']['penyelenggara'],1,0,'C',1);		
    $pdf->Cell(20,4,$_SESSION['lang']['tanggalmulai'],1,0,'C',1);
    $pdf->Cell(20,4,$_SESSION['lang']['tanggalselesai'],1,0,'C',1);	
    $pdf->Cell(15,4,$_SESSION['lang']['sertifikat'],1,1,'C',1);			
//loop isinya
         ...
         $no+=1;	
             $pdf->Cell(6,4,$no,1,0,'L',0);
             $pdf->Cell(30,4,$bar->jnsTraining,1,0,'L',0);
             $pdf->Cell(50,4,$bar->judultraining,1,0,'L',0);	
             $pdf->Cell(45,4,$bar->penyelenggara,1,0,'L',0);		
             $pdf->Cell(20,4,tanggalnormal($bar->tanggalmulai),1,0,'L',0);
             $pdf->Cell(20,4,tanggalnormal($bar->tanggalselesai),1,0,'L',0);	
             $pdf->Cell(15,4,$bar->bersertifikat,1,1,'L',0);			
         }	
...
//loop isinya
             ...
             $pdf->Cell(40,4,substr($bar->darikodeorg." (".$kamusjabatan[$bar->darikodejabatan],0,40).")",1,0,'L',0);
             $pdf->Cell(40,4,substr($bar->kekodeorg." (".$kamusjabatan[$bar->kekodejabatan],0,40).")",1,0,'L',0);	
                                  }else
