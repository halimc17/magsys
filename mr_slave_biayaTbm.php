...
$_POST['intiplasma']==''?$intiplasma=$_GET['intiplasma']:$intiplasma=$_POST['intiplasma'];
...
// preview excel pdf    
    ...
    $str="SELECT tahuntanam, sum(luasareaproduktif) as luas FROM ".$dbname.".setup_blok
        WHERE substr(kodeorg,1,4) ".$kodeorg." ".$addInti." group by tahuntanam order by tahuntanam";
    $tt[0]=0;
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    ...
    $str="SELECT tahuntanam, substr(noakun,1,5) as noakun, sum(debet) as debet, sum(kredit) as kredit, periode FROM ".$dbname.".keu_jurnalsum_blok_vw
          ...
          group by tahuntanam, substr(noakun,1,5),periode";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {        
...
//                $luastotal+=$luas[$tata];
        ...
        if(!empty($adatt))foreach($adatt as $tata){
            if($tata==0){
                $tab.="<td align=center ".$bg." >s/d Tahun Ini</td>";
            }else{
                $tab.="<td align=center ".$bg." >s/d Tahun Ini</td>
