...
switch($method){
	...
	case 'postingdetail':
		//Cari Kontrak
		$i="select a.*,b.kodebarang from ".$dbname.".pmn_traderdt a 
		...
		while($d=mysql_fetch_assoc($n)){
			...
			if($tutupbuku==0){
				...
				values ('".$d['notransaksi']."','".$d['tanggal']."','".$kodeorg."','".$kodecustomer."','".$d['kodebarang']."','".$d['nokontrak']."','0','".$d['nosipb']."','".$_SESSION['standard']['username']."','".$kodeorg."','".$d['beratbersih']."')";
				if(mysql_query($str)){
