...
switch($proses){
	case'loadData':
		...
		$offset=$page*$limit;
		$str="select a.*,b.namaorganisasi,c.namakaryawan,e.bjr,round(if(d.luasareaproduktif=0,0,d.jumlahpokok/d.luasareaproduktif),0) as sph
	...
	case'saveData':
		if($addedit=='update'){
			...
			$strx="insert into ".$dbname.".kebun_cekancak
				(kodeorg,tanggal,karyawanid,pokok,brondolan,janjang,keterangan,lastuser,lastdate)
				values('".$kodeorg."','".$tanggal."','".$diperiksa."','".$pokok."','".$brondolan."','".$janjang."','".$keterangan."'
