...
switch($proses) {
    ...
    case 'showHeadList':    
		...
		$header = array(
			...
			$_SESSION['lang']['tgl_kirim'],'Release',$_SESSION['lang']['franco'],
			$_SESSION['lang']['tanggaltiba']
		);
		
		# Content
		$cols = "nosj,kodept,tanggal,tanggalkirim,postingby,franco,tanggaltiba,posting";
		$order="nosj desc";
		...
		foreach($data as $key=>$row) {
			...
			$data[$key]['franco'] = $optFranco[$row['franco']];
		}
		...
		$tHeader->addAction('postingData','Release','images/'.$_SESSION['theme']."/posting.png");
...
function formHeader($mode,$data) {
	...
	$els[] = array(
	makeElement('penerima','hidden',$_SESSION['lang']['penerima']),
	makeElement('penerima','hidden',$data['penerima'],array('style'=>'width:150px','disabled'=>'disabled'))
    );
	$els[] = array(
	...
	$els[] = array(
	makeElement('franco','label','Gudang Tujuan'),
