...
switch($proses){
	...
	case'saveData':
		if($addedit=='update'){
			...
			$sstblok="select bjr from ".$dbname.".kebun_5bjr where kodeorg='".$kodeorg."' order by tahunproduksi desc limit 1";
			//exit('Warning: '.$sstblok);
			$qstblok=mysql_query($sstblok) or die(mysql_error($conn));
			$bjr=0;
			while($rstblok=mysql_fetch_assoc($qstblok)){
	...
	case'getKary':
		...
		if($unit!=''){
			$sKary="select karyawanid,namakaryawan,nik from ".$dbname.".datakaryawan 
					...
					and kodejabatan in (4,6,10,11,283,330,331,332,333) order by kodeorganisasi";
			//exit('Warning: '.$sKary);
