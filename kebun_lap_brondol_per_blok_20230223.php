<?php
	...
	for ($x=1; $x<=$tgl_akhir; $x++) {
		$stream.="	<td colspan=5 align=center>".$periode.'-'.sprintf("%02d",$x)."</td>";
	}
	$stream.="		<td colspan=5 align=center>Total</td>
				</tr>";
		...
		foreach($kodeblok as $kdblok=>$blok){
			...
			foreach($kdunit as $kd_unit=>$unit){
				...
				for ($x=1; $x<=$tgl_akhir; $x++){
					...
					$brondolrestan[$unit.$blok.$tgl]=$restanlalu+$brondolan[$unit.$blok.$tgl]+$brondolrawat[$unit.$blok.$tgl]+$brondolborong[$unit.$blok.$tgl]-$brondolkirim[$unit.$blok.$tgl];
