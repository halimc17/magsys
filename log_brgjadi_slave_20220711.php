		...
		while($rdata=mysql_fetch_assoc($qdata)){
			...
			if($rsnum>0){
				...
				$ssdbl="insert into ".$dbname.".log_5saldobulanan 
						...
						('".$kodept."','".$rdata['kodebarang']."',".$rdata['jumlah'].",'".$hargasatuan."','".$_SESSION['standard']['userid']."',now(),'".substr($tanggal,0,7)."','".$totalharga."','".$kodegudang."','".$rdata['kodebarang']."',".$rdata['jumlah'].",'0','".$totalharga."','0','0','0','0')";
			}
			//exit('Warning: '.$ssdbl.' harga='.$hargasatuan);
			if(!mysql_query($ssdbl)){
				//Rollback GR dan GI
