...
$tmpPeriod = explode('-',$periodeData);
$optNm=makeOption($dbname, 'organisasi', 'kodeorganisasi,namaorganisasi');
$where=" kodeunit='".$kdUnit."' and tahunbudget='".$tmpPeriod[0]."'";
...
while($rPanen2=  mysql_fetch_assoc($qPanen2))
{
	...
	while($rAdj=mysql_fetch_assoc($qAdj)){
		$dtKdOrg[$rAdj['kodeorg'].$rAdj['tanggal']]=$rAdj['kodeorg'];
...
if($proses=='excel')
{
 ...
 $brd=1;
        ...
        {
            $tab.="<td ".$bgcolordt." align=center colspan=10>".substr($isi,-2,2)."</td>";
        }
        $tab.="</tr><tr>";
        ...
        foreach($dtKdOrg as $isi)
        {
            ...
            foreach($arrTgl as $dtTgl=>$ertDt)
            {
                ...
                if($ard==1)
                {
					...
					$tab.="<td align=right>".number_format($jjgTemuan_BKM[$isi.$ertDt])."</td>";
					...
					$tab.="<td align=right>".number_format($jjgHilang_Pokok[$isi.$ertDt])."</td>";
		...
		$tab.="<td align=right>".number_format($tjjgreslalu)."</td>";
		foreach($arrTgl as $dtTgl=>$ertDt)
		{
