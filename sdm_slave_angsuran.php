		...
		if($kdOrg=='' or $kdOrg=='0')
		{
			$str1="select * from ".$dbname.".datakaryawan
				 where (tanggalkeluar = '0000-00-00' or tanggalkeluar > ".$_SESSION['org']['period']['start'].") 
				  and tipekaryawan not in ('0','7','8') 
				  and lokasitugas in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['kodeorganisasi']."')
	...
	case'loadData':
		...
		if($kdOrg=='' or $kdOrg=='0')
		{
			$str="select a.*,u.namakaryawan,u.tipekaryawan,u.lokasitugas,u.nik from ".$dbname.".sdm_angsuran a left join ".$dbname.".datakaryawan u on a.karyawanid=u.karyawanid
			  ...
			  u.lokasitugas in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['kodeorganisasi']."' and kodeorganisasi not like '%HO%')
			  order by namakaryawan";	
		}
		else
		{
