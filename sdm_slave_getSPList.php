...
if(substr($_SESSION['empl']['lokasitugas'],2,2)=='HO')
{
	...
	$str=" select karyawanid,namakaryawan,bagian,subbagian,lokasitugas,tipekaryawan from ".$dbname.".datakaryawan
       where (tanggalkeluar = '0000-00-00' or tanggalkeluar > '".date("Y-m-d")."') and ".$tip." and lokasitugas='".$lok."' order by namakaryawan";	
}
else if($_SESSION['empl']['tipelokasitugas']=='KANWIL')
{
       if($tip=='%')
           $tip='tipekaryawan in(1,2,3,6)';
       else if($tip=='0')
...
{
        $optkar.="<option value='".$bar->karyawanid."'>".$bar->namakaryawan." | ".$tkar[$bar->tipekaryawan]." | ".$bar->lokasitugas." | ".$bar->subbagian."</option>";
}
 echo    $optkar;
    exit();
}
...
$str="select a.*,b.tipekaryawan from ".$dbname.".sdm_suratperingatan a
    left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
    where b.namakaryawan like '%".$notransaksi."%'
    and b.tipekaryawan in(1,2,3,6)  limit ".$offset.",20";
} 
  ...
  while($bar=mysql_fetch_object($res))
  {
	...
	echo"<tr class=rowcontent>
	  ...
	  <td align=center>";
		 ...
		 &nbsp <img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"editSP('".$bar->nomor."','".$bar->karyawanid."','".$bar->jenissp."');\">";                  
                            } 
                    echo"</td>
	  </tr>";
  }
