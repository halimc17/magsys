...
require_once('config/connection.php');
	...
	$fax		=$_POST['fax'];
        ...
        $statusintext     =$_POST['statusintext'];
        $email		=$_POST['email'];
	...
	switch($method){
		...
		case 'insert':
                    if($namasupplier=='')                    {
                        echo"warning: Silakan isi nama supplier/kontraktor"; exit();
                    }
                    
                    $scek="select * from ".$dbname.".log_5supplier 
                           where namasupplier='".$namasupplier."' and alamat='".$alamat."' ";
                    ...
                    npwp,supplierid,fax,email,statusintext)
                    values('".$kelompok."','".$namasupplier."','".$alamat."','".$kota."','".$telp."','".$cperson."',
                            ".$plafon.",'".$npwp."','".$idsupplier."','".$fax."','".$email."','".$statusintext."')";	   			 
		break;
            case'updStatus':
                if($_POST['status']==1){
...
$str=" select * from ".$dbname.".log_5supplier where kodekelompok='".$kelompok."' order by supplierid desc";
...
//                }
                $opt='';    
                    ...
                    if($bar->status==0)
                    {
                            $opt.="<input type=checkbox id=".$bar->supplierid." title='Click to activate' onclick=\"updateStatus('".$bar->supplierid."','".$bar->status."');\">";
                            $bg="bgcolor=orange";
                    }
                    else
                    {
                            $opt.="<input type=checkbox id=".$bar->supplierid." checked  title='Click to deActivate' onclick=\"updateStatus('".$bar->supplierid."','".$bar->status."');\">";
                    }
		echo"<tr ".$bg.">
		     <td ".$bger.">".$kelompok."</td>
...
//                    <td><img src=images/application/application_delete.png class=resicon  title='Delete' onclick=\"delSupplier('".$bar->supplierid."','".$bar->namasupplier."');\"></td>
		echo "  <td><img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"editSupplier('".$bar->supplierid."','".$bar->namasupplier."','".$bar->alamat."','".$bar->kontakperson."','".$bar->kota."','".$bar->telepon."','".$bar->fax."','".$bar->email."','".$bar->npwp."','".$bar->plafon."','".$bar->statusintext."');\"></td>
			 </tr>";
	}	 	   	
  }	
  else
