<?php
	...
	for ($x = 0; $x <= count($data); $x++) {
		$isi=explode("	", $data[$x]);
		$isitgl=substr($isi[1],0,4).substr($isi[1],5,2).substr($isi[1],8,2);
		//echo '<br>'.$isitgl.' >= '.$tgl1.' and '.$isitgl.' <= '.$tgl2;
		//if(substr($isi[1],0,10)>='".$tgl1."' && substr($isi[1],0,10)<='".$tgl2."'){
		if(intval($isitgl) >= intval($tgl1) and intval($isitgl) <= intval($tgl2)){
			//echo "<br>".($x+1).'.';
				...
				if($nkary>0){
					while($rkary=mysql_fetch_assoc($qkary)){
						...
						if(mysql_num_rows($qabsdt)==0){
							...
							(kodeorg,tanggal,karyawanid,shift,absensi,jam,jamPlg,penjelasan,catu,penaltykehadiran,premi,insentif,fingerprint) values 
							...
							$pjldt='';
							$plgdt='';
							while($rabsdt=mysql_fetch_assoc($qabsdt)){
								$shidt=$rabsdt['shift'];
								$absdt=$rabsdt['absensi'];
								$upfdt=$rabsdt['fingerprint'];
								$pjldt=$rabsdt['penjelasan'];
								$plgdt=$rabsdt['jamPlg'];
							}
							//if($plgdt<$jampulang[$tgi][$idi]){
							//	echo "<br> ".$plgdt." <= ".$jampulang[$tgi][$idi];
							//	exit;
							//}
							if($shidt=='1' and $absdt=='H' and $upfdt=='1' and $pjldt=='UFP2' and $plgdt<$jampulang[$tgi][$idi]){
								$strd="update ".$dbname.".sdm_absensidt set jamPlg='".$jampulang[$tgi][$idi]."' 
								where tanggal='".$tgi."' and kodeorg='".$kodeorg."' and karyawanid='".$rkary['karyawanid']."' and shift='1' and absensi='H' and fingerprint='1' and penjelasan='UFP2'";
								if(!mysql_query($strd)){
									echo " Gagal, ".$strd.' '.addslashes(mysql_error($conn));
									exit;
								}
