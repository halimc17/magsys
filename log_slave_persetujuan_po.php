	...
	case 'list_new_data':
		...
		$str="SELECT * FROM ".$dbname.".log_poht
			where (persetujuan1='".$_SESSION['standard']['userid']."' or
				   ...
				   persetujuan3='".$_SESSION['standard']['userid']."')
				   ".$where."
			ORDER BY `nopo` DESC";
		if($res=mysql_query($str)) {
			...
			while($bar=mysql_fetch_assoc($res)) {
				...
				for ($a=1;$a<4;$a++) {	
                    if($bar['persetujuan'.$a]!='') {
                        if(($bar['persetujuan'.$a]==$_SESSION['standard']['userid'])&&($bar['hasilpersetujuan'.$a]!='')) {
                            echo"<td><button class=mybutton disabled onclick=\"get_data_po('".$bar['nopo']."')\">
									".$_SESSION['lang']['disetujui']."</button></td>
                                <td><button class=mybutton disabled onclick=rejected_po('".$bar['nopo']."') >
									".$_SESSION['lang']['ditolak']."</button></td>";                           
                        } else if(($bar['persetujuan'.$a]==$_SESSION['standard']['userid'])&&($bar['hasilpersetujuan'.$a]=='')) {
							echo"<td><button class=mybutton onclick=\"get_data_po('".$bar['nopo']."','".$a."')\">
									".$_SESSION['lang']['disetujui']."</button></td>
								<td><button class=mybutton onclick=rejected_po('".$bar['nopo']."','".$a."') >
									".$_SESSION['lang']['ditolak']."</button></td>";
                        }
                    }
                }
				...
				echo"<input type=hidden id=nopo_".$no." name=nopo_".$no." value='".$bar['nopo']."' />
					</tr>";
			}
		} else {
