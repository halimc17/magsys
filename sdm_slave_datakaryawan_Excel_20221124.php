...
if($orgsearch!=''){
	...
	if(trim($_SESSION['empl']['tipelokasitugas'])=='HOLDING'){
		if($ptsearch!=''){
			...
			$where.=" and left(a.subbagian,4) in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$ptsearch."')";
		}
		if($regional!=''){
			if($regional=='JAKARTA'){
...
function getAge($tgldate,$tgldata){
	...
	$thn=(date('Y',$tdate)-date('Y',$dob))-(date('m',$tdate)<date('m',$dob) or (date('m',$tdate)==date('m',$dob) and date('d',$tdate)<date('d',$dob)) ? 1 : 0);
