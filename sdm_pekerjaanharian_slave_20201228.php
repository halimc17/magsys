...
$nomor=tanggalsystem(checkPostGet('nomor',''));
...
switch($proses){
		...
		while($bar=mysql_fetch_object($res)){
			...
			if($bar->posting!='1'){
				echo"<td align=center>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".tanggalnormal($bar->tanggal)."','".$bar->pekerjaan."','".$bar->target."','".$bar->aktual."','".$bar->correction."','".$bar->rencanakerja."','".$bar->catatan."','".$bar->atasan."','".$bar->stspekerjaan."')\">&nbsp
	...
	case'saveData':
		$stbl="select * from ".$dbname.".sdm_pekerjaanharian 
				where karyawanid='".$_SESSION['standard']['userid']."' and tanggal='".$tanggal."' and pekerjaan='".$pekerjaan."'";
		...
		//if($addedit=='update'){
			...
			$strx="insert into ".$dbname.".sdm_pekerjaanharian
					(karyawanid,tanggal,pekerjaan,target,aktual,correction,rencanakerja,catatan,atasan,stspekerjaan,lastuser,lastdate)
					values('".$_SESSION['standard']['userid']."','".$tanggal."','".$pekerjaan."','".$target."','".$aktual."','".$correction."'
						,'".$rencanakerja."','".$catatan."','".$atasan."','".$stspekerjaan."','".$_SESSION['standard']['username']."',now())";
		}
		if(!mysql_query($strx)){
