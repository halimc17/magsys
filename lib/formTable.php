...
class uForm {
    ...
    /*
     */
    function addEls($cId,$cName,$cCont=null,$cType=null,$cAlign=null,$cLength=null,$cRefer=null,$cCont2=null,$cTSatuan=null,$cTHarga=null,$cIdGudang=null,$tSaldo=null) {
	$this->_elements[] = new uElement($cId,$cName,$cCont,$cType,$cAlign,$cLength,$cRefer,$cCont2,$cTSatuan,$cTHarga,$cIdGudang,$tSaldo);
    }
}
...
class uFormTable {
	...
	public $_afterCrud; // JS Function to be executed after CRUD
	public $_afterEditMode; // JS Function to be executed after Edit Mode Clicked
	...
	foreach($newEls as $row) {
		...
		if(empty($this->_detailFieldset)) {
			...
			if($els->_type=='hidden' or $els->_type=='hid') {
				$formTab .= "<td></td>";
			} else {
				$formTab .= "<td>:</td>";
			}
	...
	foreach($this->_form->_elements as $cols) {
		if($cols->_type!='hidden' and $cols->_type!='hid') {
	...
	foreach($this->_table->_data as $key=>$row) {
		...
		if(!empty($this->_afterEditMode)) {
			$formTab .= ";".$this->_afterEditMode."(".$key.")";
		}
		$formTab .= "\"></td>";
	...
	if(!empty($this->_afterCrud)) {
		$formTab .= "<script>theFT.afterCrud='".$this->_afterCrud."';</script>";
	}
