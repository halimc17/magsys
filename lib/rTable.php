...
class rTable {
	...
	if($this->_print) {
		...
		if($this->_printXls) {
			$theTable .= "&nbsp;&nbsp;<img class='zImgBtn' src='images/".$_SESSION['theme']."/excel.jpg'".
			"style='cursor:pointer' onclick=\"printXLS(event";
			if(!is_null($this->_printAttr)) {
				foreach($this->_printAttr as $attr) {
					$theTable .= ",'".$attr."'";
				}
			}
			$theTable .= ")\" title='Print XLS' />";
		}
	...
	if(empty($this->_content)) {
		...
		foreach($row as $id=>$val) {
		    if($id!='switched' and $id!='noSwitchList' and $id!='noAction') {
			$theTable .= "<td align='".$this->_align[$ct].
			    "' id='".$id."_".$key."' value='".$val."'>".
			    $this->_contentShow[$key][$id]."</td>";
			$ct++;
		    }
		}
		# Actions
