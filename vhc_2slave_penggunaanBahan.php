...
<td ".$bgBelakang.">".$_SESSION['lang']['kodevhc']."</td>
    <td ".$bgBelakang.">".$_SESSION['lang']['jenisvch']." - ".$_SESSION['lang']['namajenisvhc']."</td>
        <td ".$bgBelakang.">".$_SESSION['lang']['tahunperolehan']."</td>
...
//    if($listDataVhc=='BG8401FM')echo $sData;
	...
	while($rData=mysql_fetch_assoc($qData))
    {
        ...
        $tab.="<td align=right>".$jenisVhc[$listDataVhc]." - ".$nmJenis[$jenisVhc[$listDataVhc]]."</td>";
        
        $tab.="<td align=right>".$tahunperolehan[$listDataVhc]."</td>";
...
//        $tab.="<td align=right>".number_format($rData['hrgTotal'],0)."</td>";
    ...
    $tab.="<tr class=rowcontent>";
    $tab.="<td colspan=8 align=right>".$_SESSION['lang']['subtotal']." ".$listDataVhc."</td>";
    $tab.="<td align=right colspan=2>".number_format($hrgSemua,0)."</td>";
    $tab.="</tr>";
    $grandTotal+=$hrgSemua;
}
$tab.="<tr class=rowcontent>";
$tab.="<td colspan=9 align=right>".$_SESSION['lang']['grnd_total']."</td>";
	 ...
	 class PDF extends FPDF
        ...
        $height = 12;
            ...
            $pdf->SetFont('Arial','',7);
                ...
                { 
                       $hrgSemua=0;
                       $sData="select sum(jumlah) as jumlah,sum(hargatotal) as hrgTotal,kodevhc,namabarang,kodebarang from ".$dbname.".log_bahan_kendaraan_vw where kodevhc='".$listDataVhc."' and substr(tanggal,1,7) = '".$periode."' group by kodevhc,kodebarang ";
                        $qData=mysql_query($sData) or die(mysql_error());
                        while($rData=mysql_fetch_assoc($qData))
                        {                           
                            $no+=1;
