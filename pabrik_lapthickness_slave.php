<?php
	...
	foreach($kodemesin as $mdid=>$mdval)
    {
		...
		for ($tangval = 1; $tangval <= 12; $tangval++){
			...
			if(($max_tebal[$mdid][$bulan]+$min_tebal[$mdid][$bulan])>0){
				$fontcolor0="";
			}else{
				$fontcolor0=" style='color:#e0ecfc' ";
			}
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','excel',event)>".@number_format($max_tebal[$mdid][$bulan],2)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($min_tebal[$mdid][$bulan],2)."</td>";
		}
