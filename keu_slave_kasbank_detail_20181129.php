...
switch($proses) {
	...
	case 'addFromPerdin':
        ...
        foreach($resData as $bar) {
			...
			if($param['kodeorg']!=$kodeorg){
				exit('Warning : Hutang Unit pada header form belum dicentang karena Unit Berbeda...!');
			}
  ...
  if($dibayar==0){
 			if($jlhHari == 1){
				...
				$sUangMuka="select a.*,b.id,b.keterangan as namajenis from ".$dbname.".sdm_5uangmukapjd a left join ".$dbname.".sdm_5jenisbiayapjdinas b on b.id=a.jenis 
				where a.regional='".$reg."' and a.kodegolongan='".$hgolongan."' and a.jenis in (2,6,9,10,11) order by a.jenis";
			}
			...
			if($rUangMuka) {
				...
				while($bUangMuka=mysql_fetch_object($rUangMuka)) {
					...
					if($jmlkali!=0){
						...
						$keterangan2=$bUangMuka->namajenis.' '.$jmlkali.' '.$_SESSION['lang']['hari'].' x '.
					...
					while($bardt=mysql_fetch_object($rreimburs)){ 
						...
						$keterangan2=$bardt->namajenis.' '.$bardt->keterangan.$titik;
