...
else
{
    ...
    $str="select nojurnal,tanggal,debet,kredit from ".$dbname.".keu_jurnal_tidak_balance_vw where kodeorg = '".$_SESSION['empl']['lokasitugas']."'
          ...
          and nojurnal not like '%/CLSM/%' and nojurnal not like '%HIS%'";
    $res=mysql_query($str);
    if(mysql_num_rows($res)>0)
    {
...
//         exit("error: ".$query);
       ...
       if(mysql_num_rows($res)==0) {
        ...
        $query = selectQuery($dbname,'keu_jurnaldt_vw','substr(nojurnal,10,4) as kodeorg,sum(jumlah) as jumlah',
            "substr(nojurnal,10,4)='".$param['kodeorg']."' and tanggal between '".$currstart."' and '".$currend."'
             and noakun!=0 and noakun>='".$akunRAT."'").
       ...
       #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Insert Header
        ...
        if($headErr=='') {
            ...
            if($detailErr=='') {
                ...
                if(!mysql_query($queryUpd)) {
                    ...
                    #periode history
                    $prdhis=str_replace("-", "", $param['periode'])."A";
                    $str="delete from ".$dbname.".keu_setup_watu_tutup where periode='".$prdhis."' and kodeorg='".$param['kodeorg']."'";
                    mysql_query($str);
                    $str="insert into ".$dbname.".keu_setup_watu_tutup(kodeorg,periode,username) values(
                          '".$param['kodeorg']."','".$prdhis."','".$_SESSION['standard']['username']."')";
                    mysql_query($str);  
        ...
        case'getPeriode':
            ...
            $periode2=($prd[0]+1)."-".$prd[1];
            $sdata="select count(periode) as maxrow from ".$dbname.".setup_periodeakuntansi 
                    where kodeorg='".$param['kodeorg']."' and periode between '".$param['periode']."'
                    and '".$periode2."' and tutupbuku=1";
            $qdata=mysql_query($sdata) or die(mysql_error($conn));
            $rdata=mysql_fetch_assoc($qdata);
            echo $rdata['maxrow']."####".$param['kodeorg']."####".$param['periode'];
        break;
        default:
        break;
}
...
function createSaldoAwal($dariperiode,$keperiode,$kodeorg)
{
    ...
    $str="select debet,kredit,noakun from ".$dbname.".keu_jurnalsum_vw 
          where periode='".$dariperiode."' and kodeorg='".$kodeorg."' and noakun!=0";
    $res=mysql_query($str);
    while($bar=mysql_fetch_object($res))
    {
