...
include('lib/nangkoelib.php');
	...
	$ppnba		=checkPostGet('ppnba','');
	$pphba		=checkPostGet('pphba','');
		...
		if($kodeCust!=''){
			...
			if($rows>0){
				if($posting=='0'){
					...
					$strx2="update ".$dbname.".log_spk_tax set nilai='".($ppnba)."' where notransaksi='".$notransaksi."' and noakun='1160100';";
					if(mysql_query($strx2)){
					}else{
						echo " Gagal,".addslashes(mysql_error($conn));
					}
					$strx3="update ".$dbname.".log_spk_tax set nilai='".($pphba)."' where notransaksi='".$notransaksi."' and noakun='2120200';";
					if(mysql_query($strx3)){
					}else{
						echo " Gagal,".addslashes(mysql_error($conn));
					}
				...
				$strx1="insert into ".$dbname.".log_spkht
					...
					values('".$kodeorg."','".$notransaksi."','".$tanggal."','".$kodeorg."','".$trpcode."','0','".$jmlharga."','0','".$nospp."'
							,'".$tglmin."','".$tglmax."','IDR')";
				if(mysql_query($strx1)){
					$strx2="insert into ".$dbname.".log_spk_tax values('".$kodeorg."','".$notransaksi."','1160100','".($ppnba)."')";
					if(mysql_query($strx2)){
						$strx3="insert into ".$dbname.".log_spk_tax values('".$kodeorg."','".$notransaksi."','2120200','".($pphba)."')";
						if(mysql_query($strx3)){
