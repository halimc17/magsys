...
$peti=	isset($_POST['peti'])? $_POST['peti']: '';
$serah=	isset($_POST['serah'])? $_POST['serah']: '';
	...
	case'goCariPo':
		...
		while ($d=mysql_fetch_assoc($n))
		{
			...
			$aCek="	select a.nopo,a.kodebarang,sum(a.jumlah) as jumlah from ".$dbname.".log_packingdt a
					where a.nopo='".$d['nopo']."' and a.kodebarang='".$d['kodebarang']."' group by a.nopo,a.kodebarang
					union
					select b.nopo,b.kodebarang,sum(b.jumlah) as jumlah from ".$dbname.".log_suratjalandt b
					where b.jenis='PO' and b.nopo='".$d['nopo']."' and b.kodebarang='".$d['kodebarang']."' group by b.nopo,b.kodebarang
					union
					select c.nopo,c.kodebarang,sum(c.jumlah) as jumlah from ".$dbname.".log_konosemendt c
					where c.jenis='PO' and c.nopo='".$d['nopo']."' and c.kodebarang='".$d['kodebarang']."' group by c.nopo,c.kodebarang";
			$bCek=mysql_query($aCek) or die (mysql_error($conn));
			$cCek=mysql_fetch_assoc($bCek);
			
			if(isset($nobpb[$d['nopo']]) and isset($jumlah[$nobpb[$d['nopo']]])) {
			...
			$no=0;
			if(isset($nobpb[$d['nopo']]) and !empty($jumlah[$nobpb[$d['nopo']]]))
			{
				echo $trKlik;
				$no+=1;
				echo"
	...
	case 'loadDetail';/*<td align=center>".$_SESSION['lang']['jumlah']." BPB</td>
		...
		while($c=mysql_fetch_assoc($b))
		{
			...
			$xCek="	select a.nopo,a.kodebarang,sum(a.jumlah) as jumlah from ".$dbname.".log_packingdt a
					...
					//echo $xCek;
			$yCek=mysql_query($xCek) or die (mysql_error($conn));
