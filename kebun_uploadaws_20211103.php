	...
	if (isset($_POST['submit'])){
		...
		if($stsupload>0){
			...
			$srain="select kodeorg,if(waktu<'06:00',tanggal-interval 1 day,tanggal) as tglhitung
					...
					where tanggal BETWEEN '".substr(min($tgl),0,10)." 06:00' and '".substr(max($tgl),0,10)." 05:59'+interval 1 day
			...
			while($rrain=mysql_fetch_assoc($qrain)){
				$shujan="select kodeorg,tanggal	from ".$dbname.".kebun_curahhujan 
					where kodeorg='".$rrain['kodeorg']."' and tanggal='".$rrain['tglhitung']."'";
				$qhujan=mysql_query($shujan) or die(mysql_error($conn));
				$numhujan=mysql_num_rows($qhujan);
				if($numhujan==0){
					$curah="INSERT into ".$dbname.".kebun_curahhujan values('".$rrain['kodeorg']."','".$rrain['tglhitung']
