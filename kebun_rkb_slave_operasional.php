...
switch($proses) {
	...
	$totalRow = getTotalRow($dbname,'kebun_rkb_aktifitas',$where);
	if(!empty($data)) {
	    $whereKarRow = "";
	    $notFirst = false;
    ...
    # Form Edit Header
    case 'showEdit':
	$query = selectQuery($dbname,'kebun_rkb_aktifitas',"*","notransaksi='".$param['notransaksi']."'");
	$tmpData = fetchData($query);
	$data = $tmpData[0];
	$data['tanggal'] = tanggalnormal($data['tanggal']);
	setIt($_SESSION['tmp']['kebun']['tipeTrans'],'RKB');
	...
	$fQuery = selectQuery($dbname,'kebun_rkb_aktifitas','notransaksi',$fWhere);
	...
	$data['lastupdate']=date('Y-m-d h:m:s');
	$cols = array('notransaksi','kodeorg','tanggal','nikmandor',
	    'nikmandor1','nikasisten','keranimuat','tipetransaksi','updateby','lastupdate');
	$query = insertQuery($dbname,'kebun_rkb_aktifitas',$data,$cols);
	//exit('Warning: '.$query);
	...
	$data['tanggal'] = tanggalsystem($data['tanggal']);
        $data['updateby']=$_SESSION['standard']['userid'];
	$query = updateQuery($dbname,'kebun_rkb_aktifitas',$data,$where);
	if(!mysql_query($query)) {
	    echo "DB Error : ".mysql_error();
	}
	break;
    ...
    case 'delete':
		...
		$query = "delete from `".$dbname."`.`kebun_rkb_aktifitas` where ".$where;
		if(!mysql_query($query)) {
			echo "DB Error : ".mysql_error();
			exit;
		}
...
function formHeader($mode,$tipe,$data) {
 ...
 //  $whereKary = "lokasitugas='".$_SESSION['empl']['lokasitugas']."' and tipekaryawan<>1";
    ...
    $iKary = "select a.karyawanid,a.namakaryawan,a.nik,b.namajabatan from ".$dbname.".datakaryawan a ".
		"left join ".$dbname.".sdm_5jabatan b on a.kodejabatan=b.kodejabatan where (b.namajabatan like '%assistant%' or b.namajabatan like '%clerk%') 
		and ".$whereKary.
