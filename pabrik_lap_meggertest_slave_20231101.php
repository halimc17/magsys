<?php
	...
	#ambil data Megger Test
	if($stasiun!=''){
		$whr=" and a.kodemesin like '".$stasiun."%'";
	}else{
		$whr=" ";
	}
	$strtb="select kodemesin,a.ukur1,a.ukur2,a.ukur3,a.temp from ".$dbname.".pabrik_meggertest a 
			where a.kodeorg = '".$kdOrg."'
			".$whr." 
			and a.temp<>0 and tanggal<'".$periode."-01-01'
			order by a.kodemesin,a.tanggal,a.temp";
	$restb=mysql_query($strtb);
	while($bartb=mysql_fetch_object($restb)){
		$tebalsisa[$bartb->kodemesin]=$bartb->ukur1;
	}
	$str="select a.kodeorg,a.kodemesin,a.tanggal,avg(a.temp) as temp,avg(a.ukur1) as ukur1,avg(a.ukur2) as ukur2,avg(a.ukur3) as ukur3
		,b.namaorganisasi as namamesin,c.namaorganisasi as namastasiun,d.kw from ".$dbname.".pabrik_meggertest a 
	...
	while($bar=mysql_fetch_object($res)){
		...
		if($bar->ukur2>0 and $tebalkecil[$bar->kodemesin][$bulan]>0){
			$tebalkecil[$bar->kodemesin][$bulan]=($bar->ukur2<$tebalkecil[$bar->kodemesin][$bulan] ?  $bar->ukur2 : $tebalkecil[$bar->kodemesin][$bulan]);
		}
		if($bar->ukur3>0 and $tebalkecil[$bar->kodemesin][$bulan]>0){
			$tebalkecil[$bar->kodemesin][$bulan]=($bar->ukur3<$tebalkecil[$bar->kodemesin][$bulan] ?  $bar->ukur3 : $tebalkecil[$bar->kodemesin][$bulan]);
		}
	...
	$bgclr="align='center'";
	if($proses=='excel'){
		$stream="<h2>Laporan Megger Test ".$kdOrg." Periode: ".$periode."</h2>";
		$brd=1;
	...
	for ($x = 1; $x <= 12; $x++){
		...
		//$qwe=date('D', strtotime($isi));
		//if($qwe=='Sun'){
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."U-V"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."U-W"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."V-W"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."Temp"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."Critical"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."Abnormal"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."Good"."</font></td>"; 
		$stream.="<td align=center width=5px rowspan='1' ".$bgclr.">".(($libur) ? "<font color=red>" : "")."VeryGood"."</font></td>"; 
	}
	...
	foreach($kodemesin as $mdid=>$mdval)
    {
		...
		for ($tangval = 1; $tangval <= 12; $tangval++){
			...
			if(($standard[$mdid][$bulan]+$ukurrata2[$mdid][$bulan])>0){
				$fontcolor0="";
			}else{
				$fontcolor0=" style='color:#e0ecfc' ";
			}
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur1[$mdid][$bulan],0)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur2[$mdid][$bulan],0)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($ukur3[$mdid][$bulan],0)."</td>";
			$stream.="<td ".$fontcolor0." title='Click untuk cetak detail.' align=right style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>".@number_format($temp[$mdid][$bulan],2)."</td>";
			if($tebalkecil[$mdid][$bulan]<5){
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>"."&radic;"."</td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
			}else if($tebalkecil[$mdid][$bulan]<10){
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>"."&radic;"."</td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
			}else if($tebalkecil[$mdid][$bulan]<50){
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)>"."&radic;"."</td>";
				$stream.="<td ".$fontcolor0." title='Click untuk melihat detail.' align=center style=\"cursor: pointer\" onclick=showpopup('".$mdid."','".$bln."','".$kdOrg."','".$stasiun."','',event)></td>";
	...
	*/
	switch($proses){
        ...
        case 'excel':
            $nop_="Laporan_Megger_Test_".$kdOrg."_".$periode."__".date("His");
            if(strlen($stream)>0)
            {
