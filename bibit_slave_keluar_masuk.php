        ...
        case'postData2':
			...
			if($rcek['post']=='0')
			{
				...
				if(substr($kdOrgTjn,6,2)=='MN') {
					...
					$paramJurnal = $cJournal->getParam($_SESSION['org']['induk'],$kodeTrans,$kodeJurnal);
					if(empty($paramJurnal)) exit("Warning: Parameter Jurnal untuk ".$kodeJurnal." belum ada");
					$strAkun = "'".$paramJurnal['noakundebet']."','".
						$paramJurnal['noakunkredit']."','".
						$paramJurnal['sampaidebet']."','".
						$paramJurnal['sampaikredit']."'";
					
					// Get Jurnal
					$qJurnal = "SELECT SUM(jumlah) as nilai FROM ".$dbname.
						".keu_jurnaldt WHERE LEFT(tanggal,7)<='".substr($tanggal,0,6).
						"' and noakun in (".$strAkun.")";
					$resJurnal = fetchData($qJurnal);
					$sumX = $resJurnal[0]['nilai'];
					
					// Get Saldo Bibit
					$qBibit = "SELECT SUM(jumlah) as nilai FROM ".$dbname.
						".bibitan_mutasi WHERE kodeorg='".$kdOrg."'";
					$resBibit = fetchData($qBibit);
					$sumY = 0;
					if(!empty($resBibit)) $sumY = $resBibit[0]['nilai'];
					...
					$dataRes['detail'][0] = array(
						...
						'keterangan'=>'Transplanting dari '.$kdOrg." ke ".$kdOrgTjn,
						'jumlah'=>$biaya,
						'matauang'=>'IDR',
						'kurs'=>'1',
					...
					$cJournal->doJournal($_SESSION['org']['kodeorganisasi'],$kodeJurnal,
										 $dataRes,$counter+1,"Transplanting PN ke MN");
				}
