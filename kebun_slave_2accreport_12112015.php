...
foreach($resData as $row) {
	if($level < 3) {
		...
		else $data3[substr($row['noakun'],0,5)][$row['noakun']]['todate']['upah'] += $row['jumlah'];
		...
		if(substr(str_replace('-','',$row['tanggal']),0,6) == substr($tgl,0,6)) {
			...
			else $data3[substr($row['noakun'],0,5)][$row['noakun']]['today']['upah'] += $row['jumlah'];
		...
		else $data4[$row['kodeblok']]['todate']['upah'] += $row['jumlah'];
		...
		if(substr(str_replace('-','',$row['tanggal']),0,6) == substr($tgl,0,6)) {
			...
			else $data4[$row['kodeblok']]['today']['upah'] += $row['jumlah'];
...
if($level==3) {
	$qMat2 = "SELECT x.tanggal, LEFT(x.kodekegiatan,7) as akun,a.kodebarang,
		...
		INNER JOIN ".$dbname.".setup_blok b ON a.kodeblok = b.kodeorg
				...
				if(!$allPanen[$sts.$tt]) {
					...
					foreach($optAkun as $akun=>$namaakun) {
						if(substr($akun,0,3) != '611') {
							...
							$tab .= "<td align=right>".number_format($data[$sts][$tt][$akun]['month'],2)."</td>";  #disini#
		...
		foreach($optAkun as $akun=>$namaakun) {
			...
			foreach($optAkun7 as $akun7=>$namaakun7) {
				if(substr($akun7,0,5) == $akun) {
					...
					// Hitung Total
					$sTotal[0] += $dataPres[$akun][$akun7]['today']['hk'];
					$sTotal[1] += $data3[$akun][$akun7]['today']['upah'];
					$sTotal[2] += $dataPres[$akun][$akun7]['todate']['hk'];
					...
					$gTotal[0] += $dataPres[$akun][$akun7]['today']['hk'];
					$gTotal[1] += $data3[$akun][$akun7]['today']['upah'];
					$gTotal[2] += $dataPres[$akun][$akun7]['todate']['hk'];
