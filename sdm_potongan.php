...
<?php
	...
	$idOrg=substr($_SESSION['empl']['lokasitugas'],0,4);
        if($_SESSION['empl']['tipelokasitugas']=='KANWIL'){
            $sql="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where 
                  kodeorganisasi in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['kodeorganisasi']."') 
	...
	while($res=mysql_fetch_assoc($query)){
        ...
        #list potongan
        $sTipePot="select distinct id,name from ".$dbname.".sdm_ho_component where name like 'pot.%' or name like 'potongan%' order by name asc";
