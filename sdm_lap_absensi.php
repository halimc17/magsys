<?
	...
	if($_SESSION['empl']['tipelokasitugas']=='HOLDING'){
		...
		//		where b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' order by namakaryawan";
	...
	//Pilih Tipe Karyawan
	$sTipe="select id,tipe from ".$dbname.".sdm_5tipekaryawan order by id";
	...
	while($rTipe=mysql_fetch_assoc($qTipe)){
		$optTipe.="<option value=".$rTipe['id'].">".$rTipe['id']." - ".$rTipe['tipe']."</option>";
	}

	$arr="##kodept##kodeunit##karyawanid##tipekaryawan##tanggal1##tanggal2";
?>
...
<script>
	function getUnit(){
	    ...
	    tujuan='sdm_lap_absensi_slave.php';
	...
	function getKary(){
		...
		param='kodept='+kodept+'&kodeunit='+kodeunit+'&tipekaryawan='+tipekaryawan+'&proses=getKary';
	    tujuan='sdm_lap_absensi_slave.php';
		post_response_text(tujuan, param, respog);
	    function respog(){
			if(con.readyState==4){
	...
	<fieldset style="float: left;">
		...
		<table cellspacing="1" border="0" >
			...
			<tr>
				...
				<td><select id="kodeunit" name="kodeunit" onchange=getKary()><?php echo $optUnit;?></select></td>
			</tr>
			<tr>
				<td><label><?php echo $_SESSION['lang']['tipekaryawan'];?></label></td>
				<td><select id="tipekaryawan" name="tipekaryawan" onchange=getKary()><?php echo $optTipe;?></select></td>
			</tr>
			<tr>
			...
			<tr><td colspan="2">
					<button onclick="zPreview('sdm_lap_absensi_slave','<?php echo $arr;?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button>
					<button onclick="zExcel(event,'sdm_lap_absensi_slave.php','<?php echo $arr;?>')" class="mybutton" name="preview" id="preview">Excel</button>
