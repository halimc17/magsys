...
switch($method){
	...
	case 'simpandetail':
		...
		$i="select right(notransaksi,2) as maxno from ".$dbname.".pmn_traderdt where notransaksi like '".$kodecustomer.substr($tanggaldet,0,6)."%' order by right(notransaksi,2) desc limit 1";
		//exit('Warning: '.$i);
		$n=mysql_query($i) or die (mysql_error($conn));
		$notransaksidet=$kodecustomer.substr($tanggaldet,0,6).'01';
		while($d=mysql_fetch_assoc($n)){
			$notransaksidet=$kodecustomer.substr($tanggaldet,0,6).sprintf("%02d",substr($d['maxno'],0,2)+1);
		}
	...
	case 'postingdetail':
		//Cari Kontrak
		$i="select a.*,b.kodebarang,b.kodeorg from ".$dbname.".pmn_traderdt a 
		...
		while($d=mysql_fetch_assoc($n)){
			...
			$sbuku="select * from ".$dbname.".setup_periodeakuntansi where periode='".substr($d['tanggal'],0,7)."' and kodeorg='".$kodeorg."'";
			...
			if($tutupbuku==0){
				...
				values ('".$d['notransaksi']."','".$d['tanggal']."','".$kodeorg."','".$kodecustomer."','".$d['kodebarang']."','".$d['nokontrak']."','0','".$d['nosipb']."','".$_SESSION['standard']['username']."','".$kodemill."','".$d['beratbersih']."')";
				if(mysql_query($str)){
