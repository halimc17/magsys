...
switch($proses) {
    ...
    case 'showDetail':
        ...
        if($blokStatus=='bibit'){
		   ...
		   $whereDiv = " tipe='BIBITAN'";
                ...
                $whereOrg = " kodeorganisasi in (select distinct kodeorg from ".$dbname.".setup_blok where left(kodeorg,4)='".$param['afdeling']."' and luasareaproduktif>0 and detail=1)
		...
		$cols = "kodekegiatan,kodeorg,kodesegment,hasilkerja,jumlahhk,upahkerja,umr,upahpremi";
	...
	case 'updateGudang':
		...
		}elseif(substr($param['material'],4,1)=='8'){
			$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where kodeorganisasi like '".substr($param['material'],0,4)."9".substr($param['material'],5,1)."%' AND tipe like '%GUDANG%'";
		}else{
