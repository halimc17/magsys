...
switch($proses){
		...
		while($bar=mysql_fetch_object($res)){
			...
			echo"<tr class=rowcontent >
					...
					<td align=center width='6%'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodeorg."','".$bar->kodebarang."','".$bar->kodeinventaris."','".$bar->namainventaris."','".$bar->merkinventaris."','".$bar->tipeinventaris."','".$bar->ketinventaris."','".$bar->ukuran."','".$bar->warna."','".$bar->bahan."','".tanggalnormal($bar->tglperolehan)."','".$bar->hargaperolehan."','".$bar->nopo."','".$bar->kodesupplier."','".$bar->nik."','".$bar->namakaryawan."','".tanggalnormal($bar->tgluserterima)."','".$bar->kondisi."','".$bar->divisi."','".$bar->lokasi."','".$bar->ruangan."')\">&nbsp
	...
	case'saveData':
		...
		if($kodeorglama!=$kodeorg or $kodebrglama!=$kodebarang or $kodeinvlama!=$kodeinv){
		...
		if($addedit=='update'){
			$strx="update ".$dbname.".log_invbarang set kodeorg='".$kodeorg."',kodebarang='".$kodebarang."',kodeinventaris='".$kodeinv."',namainventaris='".$namainv."'
					...
					where kodeorg='".$kodeorglama."' and kodebarang='".$kodebrglama."' and kodeinventaris='".$kodeinvlama."'";
		}else{
