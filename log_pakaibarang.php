...
if($_SESSION['empl']['tipelokasitugas']=='KANWIL' and substr($_SESSION['empl']['subbagian'],-2)!='PK'){
   ...
   $str="select namaorganisasi,kodeorganisasi from ".$dbname.".organisasi 
      ...
      and tipe like 'GUDANG%'";// and kodeorganisasi not in ('SENE10', 'SKNE10', 'SOGE30') order by namaorganisasi";
}
$res=mysql_query($str);
...
$frm[0].="<table cellspacing=1 border=0>
	 ...
	 <tr>
	 <td>".$_SESSION['lang']['untukunit']."</td><td><select id=untukunit onchange=loadSubunit(this.options[this.selectedIndex].value,'0') style='width:200px;'>".$optlokasitujuan."</select></td>
	 <td>".$_SESSION['lang']['subunit']."</td><td><select id=subunit onchange=loadBlock(this.options[this.selectedIndex].value)></select>
 	    <input type=hidden value='insert' id=method>
	 </td>
	 </tr>
	 <tr>
	 <!--<td>".$_SESSION['lang']['penerima']."</td><td><input type=text id=penerima class=myinputtext onkeypress=\"return tanpa_kutip(event);\" maxlength=45 size=35></td>-->
	 <td>".$_SESSION['lang']['penerima']."</td>
         <td><select id=penerima></select>
	 </td>    
    ...
    <fieldset>
	   ...
	   <table class=sortable cellspacing=1 border=0>
		   ...
		   <tr class=rowheader>
			...
			<td>Blok / Mesin PKS</td>
			<td>".$_SESSION['lang']['segment']."</td>
			<td>Kend / Alat Berat / Mesin</td>
			<td>".$_SESSION['lang']['kegiatan']."</td>
			   ...
			   <tbody>
				   <tr class=rowcontent>
					...
					<td>".makeElement('segment','select','',array('style'=>'width:100px'),$optSegment)."</td>
