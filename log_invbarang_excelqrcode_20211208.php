...
<?php
	...
	while($barz=mysql_fetch_object($resz)){
		...
		$karyawan[$unit][$no]		=$barz->karyawan;
...
".$barz->nopo;
	...
	for ($x = (($baris-1)*$kolom)+1; $x <= $baris*$kolom; $x++) {
		$stream2.="<td align=center style='width:188px;height:10px;'></td>";
		if($x<$baris*$kolom){
			$stream2.="	<td align=center style='border:none;width:18px;'></td>";
		}
	} 
	$stream2.="</tr>";
	$stream2.="<tr>";
	for ($x = (($baris-1)*$kolom)+1; $x <= $baris*$kolom; $x++) {
		...
		if($qrbrg==''){
			$stream2.="	<td align=center style='width:188px;height:188px;'></td>";
		}else{
	...
	if($_GET['type']!='excel'){
		...
		$nop_="Inventaris_Karyawan_QRCode".$namakaryawan."_".date("His");
        if(strlen($stream2)>0){
