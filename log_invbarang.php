...
echo open_body();
?>
<script language=javascript1.2 src='js/log_invbarang.js'></script>
<?php
include('master_mainMenu.php');

OPEN_BOX('',"<b>".'Inventaris Barang'."</b>");
//get org
if($_SESSION['empl']['tipelokasitugas']=='HOLDING'){
	//$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where detail=1 and tipe='HOLDING' and length(kodeorganisasi)=4";
	//$sKry="select nik,namakaryawan from ".$dbname.".datakaryawan where (tanggalkeluar='0000-00-00' or tanggalkeluar>CURDATE()) and lokasitugas like '%HO' and tipekaryawan not in ('4','5') order by namakaryawan";
	$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where detail=1 and length(kodeorganisasi)=4";
	$sKry="select nik,namakaryawan,lokasitugas from ".$dbname.".datakaryawan where (tanggalkeluar='0000-00-00' or tanggalkeluar>CURDATE()) and tipekaryawan not in ('4','5') order by namakaryawan";
	...
	$sKry="select nik,namakaryawan,lokasitugas from ".$dbname.".datakaryawan where (tanggalkeluar='0000-00-00' or tanggalkeluar>CURDATE()) and lokasitugas not like '%HO' and lokasitugas in (select kodeorganisasi from ".$dbname.".organisasi where detail=1 and induk='".$_SESSION['empl']['induk']."') and tipekaryawan not in ('4','5') or kodejabatan=200 order by namakaryawan";
}else{
	...
	$sKry="select nik,namakaryawan,lokasitugas from ".$dbname.".datakaryawan where (tanggalkeluar='0000-00-00' or tanggalkeluar>CURDATE()) and lokasitugas='".$_SESSION['empl']['lokasitugas']."' and tipekaryawan not in ('4','5') or kodejabatan=200 or kodejabatan=348 order by namakaryawan";
}
//exit('Warning: '.$str);
$res=mysql_query($str);
...
$sBrg="select kodebarang,namabarang,satuan from ".$dbname.".log_5masterbarang where inactive='0' 
and (left(kodebarang,3)='382' 
or left(kodebarang,3)='383' 
or left(kodebarang,1)='9') order by namabarang";
$qBrg=mysql_query($sBrg);
$optbrg="<option value=''></option>";
while($rBrg=mysql_fetch_object($qBrg)){
	$optbrg.="<option value='".$rBrg->kodebarang."'>".$rBrg->namabarang." - [".$rBrg->kodebarang."]</option>";
}
...
while($rKry=mysql_fetch_object($qKry)){
	$optKry.="<option value='".$rKry->nik."'>".$rKry->namakaryawan." [".$rKry->nik."][".$rKry->lokasitugas."]</option>";
}
				...
				<td valign=top>  
					<table>
						...
						<tr>
							...
							<td><input type=text class=myinputtext id=tgldiuser size=10 onmousemove=setCalendar(this.id) onkeypress=\"return false;\"></td>
						</tr>
						<tr>
...
echo "<fieldset>
		...
		<table>
			<tr>
				<td>".$_SESSION['lang']['kodeorganisasi']."</td>
				<td><select id=carikodeorg style='width:160px'>".$cariorg."</select></td>
				<td>Inventaris</td>
				<td><input type=text id=carinamainv class=myinputtext maxlength=150 size=23></td>
				<td>Karyawan</td>
				<td><input type=text id=carikaryawan class=myinputtext maxlength=150 size=23></td>
				<td>Ruangan</td>
				<td><input type=text id=cariruangan class=myinputtext maxlength=50 size=23></td>
				<td><button class=mybutton title='Preview' onclick=loadData(0)>".$_SESSION['lang']['find']."</button></td>
				<td><button class=mybutton title='Print BA' onclick=cariinventaris('excel',event)>".$_SESSION['lang']['excel']."</button></td>
				<td><button class=mybutton title='Print QR Code' onclick=cariinventaris('pdf',event)>QR PDF</button></td>
				<td><button class=mybutton title='Print QR Code' onclick=cariinventaris('qrxls',event)>QR Xls A4</button></td>
				<td><button class=mybutton title='Print QR Code' onclick=cariinventaris('qr2xls',event)>QR ".$_SESSION['lang']['excel']."</button></td>
			</tr>
		</table>
	</fieldset>";
CLOSE_BOX();
