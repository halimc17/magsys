	...
	switch($method){
		...
		case 'update':
			$strx="update ".$dbname.".log_5masterbarang set 
			       namabarang='".saveKutip($namabarang)."',
		...
		case 'insert':
			...
			values('".$kelompok."','".$kodebarang."','"
			         .saveKutip($namabarang)."','".$satuan."',".$minstok.",
					 '".$nokartu."',".$konversi.")";	   
		break;
		default:
        break;	
...
$txtfind= isset($_POST['txtcari'])? trim($_POST['txtcari']): '';
	...
	while($bar=mysql_fetch_object($res))
	{
		...
		<td>
			<img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".$bar->kelompokbarang."','".substr($bar->kodebarang,0,5)."','".$bar->kodebarang."','".changeKutipChar($bar->namabarang)."','".$bar->satuan."','".$bar->minstok."','".$bar->nokartubin."','".$bar->konversi."');\"> 
