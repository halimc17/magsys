...
switch($proses){
	case'loadData':
		...
		while($bar=mysql_fetch_object($res)){
			...
			echo"<tr class=rowcontent >
					...
					<td ".$drcl." align=right width='7%'>".number_format($bar->jamganti1,0,'.',',')."</td>
					<td ".$drcl." align=right width='7%'>".number_format($bar->jamganti2,0,'.',',')."</td>
					<td ".$drcl." align=right width='7%'>".number_format($bar->hmakhir,0,'.',',')."</td>
					<td ".$drcl." align=left>".$bar->keterangan."</td>
					<td align=center width='4%'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->kodemesin."','".$bar->jamganti1."','".$bar->jamganti2."','".$bar->hmakhir."','".$bar->keterangan."')\">&nbsp
	...
	case'saveData':
		if($addedit=='update'){
			$strx="update ".$dbname.".pabrik_hm_setup set jamganti1='".$jamganti1."',jamganti2='".$jamganti2."',hmakhir=".$hmakhir."
					,keterangan='".$keterangan."',lastuser='".$_SESSION['standard']['username']."',lastdate=now()
					where kodemesin='".$mesin."'";
		}else{
			$strx="insert into ".$dbname.".pabrik_hm_setup (kodemesin,jamganti1,jamganti2,hmakhir,keterangan,lastuser,lastdate)
					values('".$mesin."','".$jamganti1."','".$jamganti2."','".$hmakhir."','".$keterangan."','".$_SESSION['standard']['username']."',now())";
		}
		if(!mysql_query($strx)){
